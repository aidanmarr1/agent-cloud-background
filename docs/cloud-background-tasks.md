...
- OpenRouter/Gemini 3.6 Flash: depends on model usage. The pinned route is `google/gemini-3.6-flash:exacto` with x-high reasoning (mapped by OpenRouter to Google's highest thinking level); actual provider cost is recorded from OpenRouter usage.
