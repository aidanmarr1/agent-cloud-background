...
- E2B sandboxes: E2B Hobby has no monthly base price and includes one-time $100 usage credits. E2B bills running sandboxes per second and points users to its usage calculator for exact CPU/RAM/runtime estimates. Hobby is enough to start if tasks fit within its limits; E2B Pro is $150/month plus usage when you need longer continuous runtimes, higher concurrency, or larger sandbox resources. Keep `AGENT_E2B_PAUSE_ON_TASK_END=true` so completed tasks stop accumulating sandbox runtime.
...
13. Start with `AGENT_E2B_PAUSE_ON_TASK_END=true` so idle sandboxes do not keep running.
...
npm run cloud:worker-smoke:local
npm run cloud:check
```
...
`cloud:worker-smoke:local` is the strongest pre-deploy check. It starts a production Next server and the real task worker on a generated queue such as `local-smoke-...`, then calls the signed background-worker smoke endpoint. Because it forces a unique `AGENT_TASK_QUEUE_NAME`, it cannot claim production jobs. It writes small diagnostic rows to Turso but does not call the LLM or start E2B.

If the production build already exists, use:

```bash
npm run cloud:worker-smoke:local -- --skip-build
```
...
The expensive part is usually LLM usage plus sandbox runtime. Keep `AGENT_E2B_PAUSE_ON_TASK_END=true`, keep command timeouts bounded, and start with E2B's default CPU/RAM before paying for heavier sandbox resources.
