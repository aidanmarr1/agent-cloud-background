...
async function assertSourceContracts() {
  ...
  assert.match(agentLoop, /FINAL_DELIVERABLE_WRITE_TOOLS = new Set\(\['create_file', 'create_website', 'append_file', 'edit_file', 'export_pdf', 'package_files'\]\)/, 'final deliverable completion must recognize file, website, PDF, and package updates')
  ...
  assert.match(agentLoop, /activeTools = toolRegistry\.getActiveDefinitions\(state\)[\s\S]*FILE REVISION REQUIRED:[\s\S]*The most direct recovery is to read that exact file[\s\S]*Use another available tool only when it is genuinely the better recovery/, 'stale final-report edits must recommend the direct read while preserving the healthy tool set')
