...
assert.match(e2bSandbox, /async function claimPersistedSandboxCandidate[\s\S]*provider_sandbox_id = excluded\.provider_sandbox_id[\s\S]*then agent_cloud_sandboxes\.lifecycle_generation[\s\S]*else agent_cloud_sandboxes\.lifecycle_generation \+ 1[\s\S]*lifecycle_state = 'active'[\s\S]*lifecycle_generation = \?/, 'E2B candidate publication must compare-and-swap the expected durable generation without advancing a same-sandbox reconnect')
...
assert.match(taskJobs, /async flush\(\): Promise<void> \{[\s\S]*flushPendingEventPersistence\(this\.job\)[\s\S]*const persistenceBoundary = this\.job\.persistChain[\s\S]*await persistenceBoundary[\s\S]*TaskJobClaimLostError/, 'durable emitters must snapshot, flush, and ownership-fence the pre-action event boundary')
const taskJobEmitterFlush = taskJobs.match(/class TaskJobEmitter[\s\S]*?async flush\(\): Promise<void> \{[\s\S]*?\n  \}\n\n  heartbeat/)?.[0] || ''
assert.doesNotMatch(taskJobEmitterFlush, /awaitTaskJobPersistenceIdle/, 'pre-action flushes must not chase browser frames or usage events that arrive after their durability boundary')
