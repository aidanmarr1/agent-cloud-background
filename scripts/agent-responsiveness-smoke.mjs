...
assert.match(config, /export const STREAM_REQUEST_TIMEOUT_MS = 60_000/, 'streaming model requests must tolerate GPT-5.6 Luna startup while remaining bounded')
...
assert.match(agentLoop, /const FAST_SOURCE_ACTION_REQUEST_TIMEOUT_MS = 45_000/, 'source action turns must tolerate GPT-5.6 Luna provider startup without stacked retries')
assert.match(agentLoop, /const FAST_ACTION_REQUEST_TIMEOUT_MS = 45_000/, 'non-source action turns must tolerate GPT-5.6 Luna provider startup without stacked retries')
