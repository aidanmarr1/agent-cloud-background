...
assert.match(config, /export const STREAM_REQUEST_TIMEOUT_MS = 60_000/, 'streaming model requests must tolerate routed Qwen startup while remaining bounded')
...
assert.match(agentLoop, /const FAST_SOURCE_ACTION_REQUEST_TIMEOUT_MS = 45_000/, 'source action turns must tolerate Qwen provider startup without stacked retries')
assert.match(agentLoop, /const FAST_ACTION_REQUEST_TIMEOUT_MS = 45_000/, 'non-source action turns must tolerate Qwen provider startup without stacked retries')
...
assert.match(agentLoop, /HOT PATH SOURCE ACTION TURN:[\s\S]*parallel batch of up to 2 source extraction calls/, 'source/action turns should retain a fast model-selected pair of independent source reads')
assert.match(agentLoop, /sourceBatchRoomBeforeNarration[\s\S]*NARRATION_REQUEST_AFTER_VISIBLE_ACTIONS - state\.visibleToolActionsSinceLastNarration[\s\S]*sourceBatchRoomBeforeNarration >= 2[\s\S]*Math\.min\(2, sourceBatchRoomBeforeNarration\)[\s\S]*parallel_tool_calls:\s*allowParallelSourceToolCalls/, 'parallel source calls must reserve action three for the LLM-authored narration request')
