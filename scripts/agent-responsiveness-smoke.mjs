...
assert.match(config, /iterationTimeoutMs:\s*IS_OLLAMA \? 600_000 : 60_000/, 'API stream iterations must tolerate xhigh reasoning while remaining bounded')
assert.match(config, /checkIntervalMs:\s*150/, 'timeout watchdog must check often enough for responsive recovery')
assert.match(config, /export const STREAM_REQUEST_TIMEOUT_MS = 60_000/, 'streaming model requests must tolerate Gemini 3.6 Flash xhigh startup while remaining bounded')
...
assert.match(agentLoop, /const FAST_SOURCE_ACTION_REQUEST_TIMEOUT_MS = 45_000/, 'source action turns must tolerate Gemini 3.6 Flash xhigh provider startup without stacked retries')
assert.match(agentLoop, /const FAST_ACTION_REQUEST_TIMEOUT_MS = 45_000/, 'non-source action turns must tolerate Gemini 3.6 Flash xhigh provider startup without stacked retries')
assert.match(agentLoop, /const FAST_SOURCE_ACTION_INACTIVITY_TIMEOUT_MS = 15_000/, 'source action turns must tolerate xhigh reasoning and provider jitter instead of buying a repair turn')
assert.match(agentLoop, /const FAST_ACTION_INACTIVITY_TIMEOUT_MS = 15_000/, 'non-source action turns must tolerate xhigh reasoning and provider jitter instead of buying a repair turn')
