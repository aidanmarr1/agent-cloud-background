...
assert.match(config, /inactivityTimeoutMs:\s*IS_OLLAMA \? 120_000 : 2_000/, 'agent inactivity timeout must recover quickly from invisible provider stalls')
assert.match(config, /iterationTimeoutMs:\s*IS_OLLAMA \? 600_000 : 25_000/, 'API stream iterations must stay bounded without minute-scale frozen turns')
assert.match(config, /checkIntervalMs:\s*150/, 'timeout watchdog must check often enough for responsive recovery')
assert.match(config, /export const STREAM_REQUEST_TIMEOUT_MS = 3_500/, 'streaming model requests must stay short while allowing the configured provider to start')
...
assert.match(config, /export const WEB_SEARCH_TOOL_TIMEOUT_MS = .*8_000/, 'web search needs a bounded timeout that covers Serper-backed search calls')
assert.match(config, /export const BROWSER_TOOL_TIMEOUT_MS = .*2_500/, 'browser actions need their own fast bounded timeout')
assert.match(config, /export const DOCUMENT_TOOL_TIMEOUT_MS = .*8_000/, 'document reads need enough time to return extracted content or an internal recovery result')
...
assert.match(agentLoop, /const FAST_ACTION_REQUEST_TIMEOUT_MS = 3_200/, 'fast action turns should give the provider enough first-token room to avoid retry loops')
assert.match(agentLoop, /const FAST_ACTION_INACTIVITY_TIMEOUT_MS = 500/, 'fast action turns should recover quickly from invisible provider stalls')
