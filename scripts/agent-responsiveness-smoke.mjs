...
assert.match(agentLoop, /const FAST_SOURCE_ACTION_REQUEST_TIMEOUT_MS = 4_500/, 'source action turns must use a realistic provider-start window without invisible retry waits')
assert.match(agentLoop, /const FAST_ACTION_REQUEST_TIMEOUT_MS = 4_500/, 'non-source action turns must use a realistic provider-start window without invisible retry waits')
assert.match(agentLoop, /const FAST_SOURCE_ACTION_INACTIVITY_TIMEOUT_MS = 1_100/, 'source action turns should recover quickly without aborting normal tool-call argument pauses')
...
assert.match(streamProcessor, /if \(delta\.tool_calls\) \{[\s\S]*lastVisibleActivityTime = Date\.now\(\)/, 'tool-call argument chunks must count as stream activity before a visible pill can be emitted')
