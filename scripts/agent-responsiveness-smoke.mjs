...
assert.match(config, /export const STREAM_REQUEST_TIMEOUT_MS = 12_000/, 'streaming model requests must stay bounded while allowing Gemini 3.5 Flash Lite Nitro to start reliably')
...
assert.match(agentLoop, /const FAST_SOURCE_ACTION_REQUEST_TIMEOUT_MS = 10_000/, 'source action turns must use a realistic Gemini 3.5 Flash Lite provider-start window without stacked retries')
assert.match(agentLoop, /const FAST_ACTION_REQUEST_TIMEOUT_MS = 10_000/, 'non-source action turns must use a realistic Gemini 3.5 Flash Lite provider-start window without stacked retries')
