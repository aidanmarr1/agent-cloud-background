...
async function assertSourceContracts() {
  ...
  assert.match(agentLoop, /FAST_SOURCE_ACTION_REQUEST_TIMEOUT_MS = 2_200/, 'source-action starts must keep a tight fast window')
  assert.match(agentLoop, /FAST_ACTION_REQUEST_TIMEOUT_MS = 3_200/, 'non-source action starts must avoid provider-start churn while staying bounded')
  assert.match(agentLoop, /FAST_ACTION_RETRY_REQUEST_TIMEOUT_MS = 4_500/, 'hot action retries must get a slightly wider provider-start window instead of looping through narration')
  ...
  assert.match(agentLoop, /FORCED_NARRATION_REQUEST_TIMEOUT_MS = 2_800/, 'progress narration must avoid short provider-start churn while staying bounded')
  ...
  assert.match(agentLoop, /FORCED_NARRATION_REQUEST_TIMEOUT_MS = 2_800[\s\S]*FORCED_NARRATION_MAX_TOKENS = 48/, 'tiny progress narration turns must stay compact while avoiding provider-start churn')
