...
async function assertSourceContracts() {
  ...
  assert.match(agentConfig, /MODEL_MAX_PROMPT_TOKENS = 983_040[\s\S]*MODEL_MAX_COMPLETION_TOKENS = 65_536[\s\S]*MAX_CONTEXT_MESSAGES = 4_096/, 'Gemini 3.7 Flash must use its published context and the application-selected completion cap instead of an eight-message cap')
  ...
  assert.match(llm, /PINNED_OPENROUTER_PROVIDER\s*=\s*'google-vertex'[\s\S]*exactOpenRouterProviderRoute/, 'runtime must pin Gemini to OpenRouter\'s exact Google Vertex provider route')
