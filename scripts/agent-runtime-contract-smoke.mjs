...
async function assertSourceContracts() {
  ...
  assert.match(prompts, /Every non-empty plan must end with a concise, task-specific delivery step/, 'the planning model must always author a task-specific final delivery step')
  assert.match(planManager, /ensureFinalDeliveryStep[\s\S]*Deliver \$\{target\}/, 'runtime plan normalization must preserve the final delivery invariant when planner output is malformed')
  assert.ok(
  ...
  assert.match(planManager, /PLANNER_CONTROL_REASONING = \{ effort: 'none' as const, exclude: true \}/, 'planner JSON control calls must use Qwen’s lowest configured reasoning')
  assert.match(planManager, /PLANNER_ACK_REASONING = \{ effort: 'none' as const, exclude: true \}/, 'startup acknowledgement must use Qwen’s lowest configured reasoning')
  ...
  assert.match(llm, /reasoning:\s*\{[\s\S]*effort:\s*'none'[\s\S]*exclude:\s*true/, 'every OpenRouter call must use Qwen’s lowest configured reasoning and exclude internal traces')
  ...
  assert.match(agentLoop, /MINIMAL_THINKING_REASONING = \{ effort: 'none' as const, exclude: true \}/, 'agent control turns must use Qwen’s lowest configured reasoning mode')
  assert.match(agentLoop, /TASK_REASONING = \{ effort: 'none' as const, exclude: true \}/, 'Qwen action turns must use the lowest configured reasoning so required tools remain compatible')
