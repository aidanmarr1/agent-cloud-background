...
async function assertSourceContracts() {
  ...
  assert.match(planManager, /PLANNER_ACK_MAX_TOKENS = 320/, 'planner acknowledgement generation must leave room after Muse mandatory minimal reasoning for a complete visible sentence')
  ...
  assert.match(planManager, /res = await createCompletion\(\{[\s\S]*response_format:\s*\{\s*type:\s*'json_object'\s*\}/, 'fast and strict planner calls must request structured JSON on the first provider attempt')
  ...
  assert.match(normalPlannerStartup, /scheduleAcknowledgementCall\(\)[\s\S]*plannerDeadlineAtMs = Date\.now\(\) \+ PLANNER_OVERALL_DEADLINE_MS[\s\S]*this\.planPromise = start\(\)[\s\S]*this\.attemptPlanCall\(0,\s*true\)/, 'normal startup must run its model-authored acknowledgement independently beside the planner')
  assert.match(normalPlannerStartup, /scheduleAcknowledgementCall/, 'normal startup must launch an independently metered acknowledgement alongside planner JSON')
  assert.match(normalPlannerStartup, /await this\.acknowledgementRequestStartedPromise[\s\S]*this\.attemptPlanCall\(0,\s*true\)/, 'normal startup must dispatch acknowledgement generation before planner generation without a fixed display timer')
  ...
  assert.match(planManager, /const stream = await createStreamingCompletion[\s\S]*for await \(const chunk of stream\)[\s\S]*isSafeStreamingPlannerAckDraft\(draft, request\)[\s\S]*this\.emitter\.textDelta\(draft\)/, 'the worker-owned acknowledgement must release the first complete task-specific model draft while the provider stream is active')
  ...
  assert.match(planManager, /startPlanCall\(\): void \{[\s\S]*scheduleAcknowledgementCall\(\)[\s\S]*attemptPlanCall\(0,\s*true\)/, 'normal startup must start the acknowledgement before its plan call')
  ...
  assert.match(planManager, /PLANNER_CONTROL_REASONING = \{ effort: 'minimal' as const, exclude: true \}/, 'planner JSON control calls must use the fastest supported reasoning mode')
  assert.match(planManager, /PLANNER_ACK_REASONING = \{ effort: 'minimal' as const, exclude: true \}/, 'startup acknowledgement must use the fastest supported reasoning mode')
  ...
  assert.match(agentConfig, /MODEL_MAX_PROMPT_TOKENS = 917_504[\s\S]*MODEL_MAX_COMPLETION_TOKENS = 131_072[\s\S]*MAX_CONTEXT_MESSAGES = 4_096/, 'Muse must preserve its one-million-token prompt budget and full product completion budget')
  ...
  assert.match(llm, /DEFAULT_OPENROUTER_MODEL/, 'runtime must route through the centralized OpenRouter model')
  ...
  assert.match(llm, /PINNED_OPENROUTER_PROVIDER\s*=\s*'meta'[\s\S]*exactOpenRouterProviderRoute/, 'runtime must pin Muse to OpenRouter\'s exact Meta provider endpoint')
  ...
  assert.match(llm, /ASSISTANT_REASONING_EFFORT = 'minimal'[\s\S]*function providerReasoningPayload[\s\S]*effort:\s*ASSISTANT_REASONING_EFFORT[\s\S]*exclude:\s*true/, 'every Muse request lane must use its fastest supported hidden reasoning mode')
  assert.doesNotMatch(llm, /temperature:\s*_temperature/, 'Muse must preserve its supported per-call temperature setting')
  assert.match(llm, /const hasNativeTools = Array\.isArray\(rest\.tools\)[\s\S]*hasNativeTools \? \{ tool_choice: 'auto' as const \} : \{\}/, 'Muse tool turns must use the live Meta endpoint\'s supported automatic tool-choice mode while keeping all healthy tools available')
  assert.match(llm, /usage:\s*\{\s*include:\s*true\s*\}/, 'OpenRouter calls must explicitly request usage data for compatibility')
  assert.match(llm, /provider:\s*exactOpenRouterProviderRoute\(\)/, 'OpenRouter calls must use only the exact Meta provider endpoint')
  ...
  assert.match(agentLoop, /PRESENTATION_REASONING = \{ effort: 'minimal' as const, exclude: true \}/, 'acknowledgement, narration, and handoff turns must use minimal reasoning')
  assert.match(agentLoop, /FAST_ACTION_REASONING = \{ effort: 'minimal' as const, exclude: true \}/, 'fast action selection must use minimal reasoning')
  assert.match(agentLoop, /TASK_REASONING = \{ effort: 'minimal' as const, exclude: true \}/, 'ordinary task turns must use minimal reasoning')
  assert.match(agentLoop, /DEEP_TASK_REASONING = \{ effort: 'minimal' as const, exclude: true \}/, 'final synthesis and build turns must use minimal reasoning without reducing output or iteration budgets')
