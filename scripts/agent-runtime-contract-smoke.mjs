...
async function assertSourceContracts() {
  ...
  assert.match(eventDispatcher, /visibleStartedRecovery[\s\S]*isHiddenInternalToolResult\(event\.name,\s*event\.result\) && !visibleStartedRecovery/, 'internal recovery results must not remove already-visible source action pills')
