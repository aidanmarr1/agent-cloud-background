...
async function assertSourceContracts() {
  ...
  assert.match(chatRoute, /`Map \$\{subject\} angles`[\s\S]*`Read current \$\{subject\} sources`/, 'route startup research plans must be subject-specific instead of generic framing')
  assert.doesNotMatch(chatRoute, /Frame the key questions|Gather current evidence/, 'route startup plans must not emit generic research placeholders')
