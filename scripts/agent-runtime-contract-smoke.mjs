...
async function assertSourceContracts() {
  ...
  assert.match(llm, /ASSISTANT_SUPPORTS_VIDEO_INPUT = false/, 'the pinned DeepSeek route must reject unsupported native video input')
  assert.match(llm, /ASSISTANT_SUPPORTS_FILE_INPUT = false/, 'the pinned DeepSeek route must reject unsupported generic file input')
  assert.match(llm, /ASSISTANT_SUPPORTS_AUDIO_INPUT = false/, 'the pinned DeepSeek route must reject unsupported native audio input')
  ...
  assert.match(llm, /DEFAULT_ASSISTANT_MODEL/, 'runtime must route through the centralized assistant model')
  ...
  assert.match(llm, /export const DEFAULT_MODEL = DEFAULT_ASSISTANT_MODEL/, 'stale environment or client model IDs must not change the pinned model')
  assert.match(llm, /ASSISTANT_BASE_URL\s*=\s*'https:\/\/api\.deepseek\.com'/, 'runtime must use only DeepSeek\'s direct provider endpoint')
  ...
  assert.match(llm, /getAssistantApiKey[\s\S]*trimmedEnv\(process\.env\.DEEPSEEK_API_KEY\)/, 'provider credentials must be trimmed before request headers are built')
  assert.doesNotMatch(llm, /process\.env\.OPENROUTER_API_KEY|openrouter\.ai/, 'active provider runtime must not retain OpenRouter routing')
  assert.match(llm, /ASSISTANT_TOOL_REASONING_EFFORT = 'low'[\s\S]*function providerReasoningPayload[\s\S]*hasNativeTools && reasoning\?\.effort !== 'none'[\s\S]*thinking:\s*\{ type: 'enabled' \}[\s\S]*reasoning_effort: ASSISTANT_TOOL_REASONING_EFFORT[\s\S]*thinking:\s*\{ type: 'disabled' \}/, 'only tool-selection turns may use DeepSeek low reasoning; narration and chat must disable thinking')
  assert.doesNotMatch(llm, /temperature:\s*_temperature/, 'DeepSeek must preserve its supported per-call temperature setting')
  assert.match(llm, /const hasNativeTools = Array\.isArray\(rest\.tools\)[\s\S]*hasNativeTools \? \{ tool_choice: 'auto' as const \} : \{\}/, 'DeepSeek tool turns must use supported automatic tool choice while keeping all healthy tools available')
  assert.match(llm, /stream_options:\s*\{\s*include_usage:\s*true\s*\}/, 'DeepSeek calls must explicitly request terminal usage data')
  assert.doesNotMatch(llm, /provider:\s*exactOpenRouterProviderRoute\(\)/, 'direct DeepSeek calls must not contain an OpenRouter provider route')
