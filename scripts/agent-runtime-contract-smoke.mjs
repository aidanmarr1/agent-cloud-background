...
async function assertSourceContracts() {
  ...
  assert.match(planManager, /PLANNER_CONTROL_REASONING = \{ effort: 'low' as const, exclude: true \}/, 'planner JSON control calls must use low reasoning')
  ...
  assert.match(agentConfig, /MODEL_MAX_PROMPT_TOKENS = 983_040[\s\S]*MODEL_MAX_COMPLETION_TOKENS = 65_536[\s\S]*MAX_CONTEXT_MESSAGES = 4_096/, 'Muse Spark 1.2 must use its published context and the full configured completion capacity instead of an eight-message cap')
  ...
  assert.doesNotMatch(llm, /process\.env\.OPENROUTER_REASONING_EFFORT|process\.env\.OPENROUTER_REASONING_EXCLUDE/, 'stale environment variables must not override Muse per-turn reasoning levels')
  ...
  assert.match(llm, /function providerReasoningPayload[\s\S]*requestedEffort === 'xhigh'[\s\S]*requestedEffort === 'none'[\s\S]*_reasoning\?\.enabled === false[\s\S]*reasoning:\s*\{[\s\S]*effort,[\s\S]*exclude:\s*true/, 'Muse calls must use supported native reasoning levels and hide internal traces')
  assert.match(llm, /const hasNativeTools = Array\.isArray\(rest\.tools\)[\s\S]*hasNativeTools \? \{ tool_choice: 'auto' as const \} : \{\}/, 'Muse tool turns must normalize required and named choices to the only mode accepted by the exact Meta endpoint')
  ...
  assert.match(agentLoop, /TASK_REASONING = \{ effort: 'low' as const, exclude: true \}/, 'ordinary task turns must receive low reasoning')
  assert.match(agentLoop, /DEEP_TASK_REASONING = \{ effort: 'medium' as const, exclude: true \}/, 'final synthesis and build turns must receive medium reasoning')
  ...
  assert.match(agentLoop, /const requestReasoning = useCompactNarration[\s\S]*PRESENTATION_REASONING[\s\S]*isFinalInlineAnswerTurn[\s\S]*DEEP_TASK_REASONING[\s\S]*fastActionTurn[\s\S]*FAST_ACTION_REASONING[\s\S]*TASK_REASONING/, 'reasoning must adapt by turn type without slowing visible narration')
