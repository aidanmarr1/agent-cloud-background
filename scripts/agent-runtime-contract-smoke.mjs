...
async function assertSourceContracts() {
  ...
  assert.match(agentConfig, /MODEL_MAX_PROMPT_TOKENS = 922_000[\s\S]*MODEL_MAX_COMPLETION_TOKENS = 128_000[\s\S]*MAX_CONTEXT_MESSAGES = 4_096/, 'GPT-5.6 Luna must use its published prompt and completion capacity instead of an eight-message context cap')
  ...
  assert.match(llm, /PINNED_OPENROUTER_PROVIDER\s*=\s*'openai'\s+as const/, 'runtime must pin the exact standard OpenAI provider endpoint')
  ...
  assert.match(llm, /reasoning:\s*\{[\s\S]*effort:\s*'none'[\s\S]*exclude:\s*true/, 'every OpenRouter call must disable reasoning and exclude internal traces')
  assert.match(llm, /usage:\s*\{\s*include:\s*true\s*\}/, 'OpenRouter calls must explicitly request usage data for compatibility')
  assert.match(llm, /order:\s*\[PINNED_OPENROUTER_PROVIDER\][\s\S]*only:\s*\[PINNED_OPENROUTER_PROVIDER\][\s\S]*allow_fallbacks:\s*false/, 'every OpenRouter call must use only the exact standard OpenAI endpoint with no provider fallback')
  ...
  assert.match(agentLoop, /FAST_SOURCE_ACTION_REQUEST_TIMEOUT_MS = 45_000/, 'source-action starts must tolerate GPT-5.6 Luna provider startup')
  assert.match(agentLoop, /FAST_ACTION_REQUEST_TIMEOUT_MS = 45_000/, 'non-source action starts must tolerate GPT-5.6 Luna provider startup')
  ...
  assert.match(agentLoop, /const FINAL_INLINE_ANSWER_REQUEST_TIMEOUT_MS = 60_000/, 'final inline answers must tolerate GPT-5.6 Luna provider startup')
  ...
  assert.match(agentLoop, /FINAL_SAVED_DELIVERABLE_REQUEST_TIMEOUT_MS = 60_000/, 'saved final deliverables must have a bounded startup window that tolerates GPT-5.6 Luna provider variance')
