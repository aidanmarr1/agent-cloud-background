...
async function assertSourceContracts() {
  ...
  assert.match(planManager, /PLANNER_ACK_REASONING = \{ effort: 'minimal' as const, exclude: true \}/, 'startup acknowledgement must use minimal thinking while staying bounded')
  ...
  assert.match(agentLoop, /NARRATION RETRY:[\s\S]*Do not call tools, do not advance, and do not switch to normal work/, 'ordinary narration model-start timeouts must retry narration instead of silently releasing to normal work')
