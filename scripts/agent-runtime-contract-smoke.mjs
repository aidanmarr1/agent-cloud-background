...
async function assertSourceContracts() {
  ...
  assert.match(panelHeader, /startup:\s*'Preparing a fresh computer'/, 'computer panel startup copy must describe the real cold-start work')
  assert.match(typingIndicator, /Preparing a fresh computer/, 'startup indicator must describe the real cold-start work')
  ...
  assert.match(chatTaskRunner, /DIRECT_CHAT_MAX_TOKENS = 1536/, 'direct chat should keep concise answers on a smaller completion cap')
  ...
  assert.match(chatTaskRunner, /DIRECT_CHAT_CONTINUATION_MAX_TOKENS = 768/, 'direct chat continuations should stay compact')
  ...
  assert.match(agentLoop, /FINAL_SAVED_DELIVERABLE_INITIAL_MAX_TOKENS = 2_400/, 'initial markdown deliverable writes need enough budget for one coherent report')
  ...
  assert.match(agentLoop, /if \(isLastStep\) \{[\s\S]*?recoveryMinimum = Math\.min\(FINAL_AUTOSAVE_DRAFT_MIN_CHARS, requestedMinimum\)[\s\S]*?text\.length >= recoveryMinimum && taskNeedsSavedFinalArtifact\(state, messages\)[\s\S]*?\}/, 'only saved-artifact tasks should recover text into a file, while concise artifacts may use their truthful requested minimum')
  assert.match(agentLoop, /textSavedDeliverableTarget\(state, this\.options\.messages\)[\s\S]*?lastStreamToolCallPolicy\.textSavedDeliverable/, 'text-only provider recovery must retain one stable real file target from stream through save')
  assert.match(streamProcessor, /textSavedDeliverable\.started = true[\s\S]*?fileContentStart[\s\S]*?fileContentDelta\(textSavedDeliverable\.id, content\)[\s\S]*?immediate: true/, 'text-only provider recovery must route actual content chunks into the immediate live file lane')
  ...
  assert.match(agentLoop, /const FINAL_SAVED_DELIVERABLE_MAX_TOKENS = 2_400/, 'saved final deliverables must have enough room for coherent in-place report revisions')
  ...
  assert.match(agentLoop, /state\.toolJsonRecoveryCount >= 2 \|\| state\.consecutiveNoToolCalls >= 1/, 'one ignored required file-tool turn must switch to the bounded live text-save fallback')
  assert.match(agentLoop, /rejectedUnsavedFinalDeliverableDraft[\s\S]*!lastStreamToolCallPolicy\.textSavedDeliverable[\s\S]*finalSavedDeliverableTurn[\s\S]*Held back an unsaved final-deliverable draft/, 'unsaved final-file prose must stay hidden instead of repeating in chat before live text-save recovery')
  assert.match(agentLoop, /streamedTarget && state\.consecutiveNoToolCalls >= 2[\s\S]*could not produce enough valid content to save the requested file[\s\S]*return 'ERROR'/, 'repeated unusable live text-save drafts must terminate at a hard bounded fence instead of opening endless file previews')
