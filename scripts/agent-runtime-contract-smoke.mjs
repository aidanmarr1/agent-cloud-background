...
async function assertSourceContracts() {
  ...
  assert.match(chatRoute, /const rawMessages = validation\.data\.messages[\s\S]*const directChat = false[\s\S]*const scopedTaskMessages = scopeAgentTaskMessages\(safeRawMessages\)[\s\S]*const messagesPromise = useExternalWorker \|\| directChat \|\| !hasUnhydratedAttachments\(scopedTaskMessages\)/, 'chat route should keep every new message on the full Agent path while scoping payloads before attachment hydration')
