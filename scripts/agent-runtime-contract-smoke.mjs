...
async function assertSourceContracts() {
  ...
  assert.match(chatRoute, /taskStartPromise = enqueueTaskJob\([\s\S]*initialEvents,[\s\S]*\}\)\.then/, 'external-worker route must enqueue the durable job immediately after first paint setup')
