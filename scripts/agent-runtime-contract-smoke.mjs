...
async function assertSourceContracts() {
  ...
  assert.match(webSearchSource, /WEB_SEARCH_RESULT_COUNT\s*=\s*5/, 'web_search must request and display at most 5 results')
  assert.match(webSearchSource, /num:\s*WEB_SEARCH_RESULT_COUNT/, 'Serper web_search request size must use the shared 5-result count')
