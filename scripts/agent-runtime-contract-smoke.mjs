...
async function assertSourceContracts() {
  ...
  assert.match(planManager, /PLANNER_ACK_MAX_TOKENS = 128/, 'planner acknowledgement generation must stay short enough for a prompt visible opening')
  ...
  assert.match(planManager, /await this\.recordUsage\?\.\(normalized,/, 'planner usage recording must complete before acknowledgement, plan emission, or first tool startup can become durable')
  ...
  assert.match(llm, /ASSISTANT_SUPPORTS_IMAGE_INPUT = true/, 'the pinned GLM route must advertise native image input')
  assert.match(llm, /ASSISTANT_SUPPORTS_VIDEO_INPUT = true/, 'the pinned GLM route must advertise native video input')
  assert.match(llm, /ASSISTANT_SUPPORTS_FILE_INPUT = false/, 'the pinned GLM route must not advertise unsupported native file input')
  assert.match(llm, /ASSISTANT_SUPPORTS_AUDIO_INPUT = false/, 'the pinned GLM route must not advertise unsupported native audio input')
  ...
  assert.match(agentLoop, /function assistantSupportsNativeAttachment[\s\S]*ASSISTANT_SUPPORTS_IMAGE_INPUT[\s\S]*ASSISTANT_SUPPORTS_VIDEO_INPUT[\s\S]*ASSISTANT_SUPPORTS_AUDIO_INPUT[\s\S]*ASSISTANT_SUPPORTS_FILE_INPUT/, 'native attachment routing must match GLM’s advertised modalities')
  ...
  assert.match(planManager, /const response = await createCompletion[\s\S]*sanitizePlannerAck\(response\.choices\[0\]\?\.message\?\.content \|\| ''\)[\s\S]*recordCompletionUsage\(response\.usage, 'ack'\)[\s\S]*isUsablePlannerAck\(sanitizedAck, request\)[\s\S]*this\.emitter\.textDelta\(sanitizedAck \+ '\\n\\n'\)/, 'the precomputed-plan acknowledgement must be validated as one complete model candidate and usage-debited before it is shown')
  ...
  assert.match(agentConfig, /MODEL_MAX_PROMPT_TOKENS = 917_504[\s\S]*MODEL_MAX_COMPLETION_TOKENS = 131_072[\s\S]*MAX_CONTEXT_MESSAGES = 4_096/, 'GLM 5.3 Flash must preserve its one-million-token context and exact endpoint completion cap')
  ...
  assert.match(llm, /PINNED_OPENROUTER_PROVIDER\s*=\s*'z-ai\/fp8'[\s\S]*exactOpenRouterProviderRoute/, 'runtime must pin GLM to OpenRouter\'s exact Z.AI FP8 provider endpoint')
  assert.doesNotMatch(llm, /process\.env\.OPENROUTER_REASONING_EFFORT|process\.env\.OPENROUTER_REASONING_EXCLUDE/, 'stale environment variables must not override code-pinned medium reasoning')
  ...
  assert.match(llm, /ASSISTANT_REASONING_EFFORT = 'medium'[\s\S]*ASSISTANT_FAST_REASONING_EFFORT = 'minimal'[\s\S]*function providerReasoningPayload[\s\S]*reasoning\?\.effort === ASSISTANT_FAST_REASONING_EFFORT[\s\S]*exclude:\s*true/, 'GLM task calls must stay medium while explicit startup control calls may use hidden minimal reasoning')
  assert.doesNotMatch(llm, /temperature:\s*_temperature/, 'GLM must preserve its supported per-call temperature setting')
  assert.match(llm, /const hasNativeTools = Array\.isArray\(rest\.tools\)[\s\S]*hasNativeTools \? \{ tool_choice: 'auto' as const \} : \{\}/, 'GLM tool turns must use the exact endpoint\'s automatic tool-choice mode while keeping all healthy tools available')
  assert.match(llm, /usage:\s*\{\s*include:\s*true\s*\}/, 'OpenRouter calls must explicitly request usage data for compatibility')
  assert.match(llm, /provider:\s*exactOpenRouterProviderRoute\(\)/, 'OpenRouter calls must use only the exact Z.AI FP8 provider endpoint')
  ...
  assert.match(agentLoop, /FINAL_SAVED_DELIVERABLE_INITIAL_MAX_TOKENS = MODEL_MAX_COMPLETION_TOKENS/, 'initial markdown deliverables must receive the provider\'s full completion budget')
  ...
  assert.doesNotMatch(streamProcessor, /fileActionLabelFallback|args\.action_label\s*=\s*runtimeVisibleActionLabel/, 'provisional streaming must wait for the model-authored action label instead of inventing one')
  ...
  assert.match(agentLoop, /PRESENTATION_REASONING = \{ effort: 'medium' as const, exclude: true \}/, 'acknowledgement, narration, and handoff turns must use medium reasoning')
  assert.match(agentLoop, /FAST_ACTION_REASONING = \{ effort: 'medium' as const, exclude: true \}/, 'fast action selection must use medium reasoning')
  assert.match(agentLoop, /TASK_REASONING = \{ effort: 'medium' as const, exclude: true \}/, 'ordinary task turns must use medium reasoning')
  assert.match(agentLoop, /DEEP_TASK_REASONING = \{ effort: 'medium' as const, exclude: true \}/, 'final synthesis and build turns must use medium reasoning')
  ...
  assert.match(agentLoop, /const FINAL_SAVED_DELIVERABLE_MAX_TOKENS = MODEL_MAX_COMPLETION_TOKENS/, 'saved final deliverables must retain the provider\'s full completion budget for coherent revisions')
