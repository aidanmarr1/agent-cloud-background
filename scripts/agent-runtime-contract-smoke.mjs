...
async function assertSourceContracts() {
  ...
  assert.match(agentLoop, /const hasPriorOpenedEvidence[\s\S]*currentStepIdx > 0[\s\S]*visitedUrls\.size > state\.stepVisitedUrls\.size[\s\S]*stepResearchCallCount >= 1[\s\S]*stepSourceDomainCounts\.size >= 2[\s\S]*stepFailureCount >= 1/, 'later ordinary research phases must preserve prior opened evidence when the current multi-domain packet has a failed source route')
