...
async function assertSourceContracts() {
  ...
  assert.match(agentConfig, /MODEL_MAX_PROMPT_TOKENS = 917_504[\s\S]*MODEL_MAX_COMPLETION_TOKENS = 131_072[\s\S]*MAX_CONTEXT_MESSAGES = 4_096/, 'DeepSeek must preserve its one-million-token prompt budget and full product completion budget')
  ...
  assert.match(llm, /DEFAULT_DEEPSEEK_MODEL/, 'runtime must route through the centralized DeepSeek model')
  assert.match(llm, /ASSISTANT_PROVIDER\s*=\s*'deepseek'\s+as const/, 'runtime provider must be statically pinned to DeepSeek')
  assert.match(llm, /export const DEFAULT_MODEL = DEFAULT_DEEPSEEK_MODEL/, 'stale model IDs must not change the pinned preview')
  assert.match(llm, /getAssistantApiKey[\s\S]*trimmedEnv\(process\.env\.DEEPSEEK_API_KEY\)/, 'DeepSeek credentials must be trimmed')
  assert.doesNotMatch(llm, /process\.env\.OPENROUTER_API_KEY|openrouter\.ai/, 'runtime must not retain another provider route')
  assert.match(llm, /ASSISTANT_REASONING_EFFORT = 'low'[\s\S]*thinking: \{ type: 'enabled' \}[\s\S]*reasoning_effort: ASSISTANT_REASONING_EFFORT/, 'all lanes must use low enabled thinking')
  assert.match(llm, /stream_options: \{ include_usage: true \}/, 'DeepSeek streams must return token usage')
