...
async function assertSourceContracts() {
  ...
  assert.match(chatRoute, /;\[, messages, access\] = await Promise\.all\(\[[\s\S]*creditsPromise,[\s\S]*messagesPromise,[\s\S]*accessPromise,[\s\S]*\]\)/, 'chat route must not wait for worker heartbeat readiness before opening the external task stream')
  ...
  assert.match(chatRoute, /workerAvailabilityPromise\.then\(async \(unavailableWorker\)[\s\S]*throw await taskWorkerUnavailableStreamError\(unavailableWorker\)[\s\S]*enqueueTaskJob/, 'external worker unavailability must become a visible stream error instead of a blank pre-stream wait')
