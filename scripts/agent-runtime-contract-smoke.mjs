...
async function assertSourceContracts() {
  ...
  assert.match(chatRoute, /Promise\.all\(\[[\s\S]*accessPromise,[\s\S]*workerAvailabilityPromise,[\s\S]*\]\)\.then\(async \(\[accessResult, unavailableWorker\]\)[\s\S]*throw await taskAccessStreamError\(accessResult\.response\)[\s\S]*throw await taskWorkerUnavailableStreamError\(unavailableWorker\)[\s\S]*enqueueTaskJob/, 'task access and worker availability must still gate enqueue after first paint')
