...
async function assertSourceContracts() {
  ...
  assert.match(planManager, /PLANNER_JSON_REQUEST_TIMEOUT_MS = 4_200/, 'strict planner JSON calls must have a bounded but realistic timeout without causing a visible startup failure')
  ...
  assert.match(planManager, /PLANNER_OVERALL_DEADLINE_MS = 9_000/, 'planner startup must have an overall bound so plans do not sit past the expected window')
