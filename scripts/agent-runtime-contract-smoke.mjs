...
async function assertSourceContracts() {
  ...
  assert.match(chatRoute, /const startupPlan = createFastStartupPlan\(\{ messages \}\)/, 'external-worker route must create a fast startup plan without a second route model call')
  assert.match(chatRoute, /const initialEvents:[\s\S]*heartbeatEvent[\s\S]*type: 'plan', items: startupPlan\.items/, 'external-worker route must persist the startup plan with initial events for reconnect replay')
  assert.match(chatRoute, /taskStartPromise = Promise\.resolve\(\)\.then\(\(\) => \{[\s\S]*startupPlanExpected: false[\s\S]*payload: queuedTaskPayload/, 'external-worker route must enqueue immediately with startup plan or explicit no-plan release flag')
