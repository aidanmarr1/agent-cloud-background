...
async function assertSourceContracts() {
  ...
  assert.match(planManager, /PLANNER_ACK_REASONING = \{ enabled: false as const, exclude: true \}/, 'startup acknowledgement must use no-thinking generation while staying bounded')
  ...
  assert.match(agentLoop, /FAST_SOURCE_ACTION_REQUEST_TIMEOUT_MS = 10_000/, 'source-action starts must tolerate Gemini 3.6 provider selection without becoming long stalls')
  assert.match(agentLoop, /FAST_ACTION_REQUEST_TIMEOUT_MS = 10_000/, 'non-source action starts must tolerate Gemini 3.6 provider selection without becoming long stalls')
  ...
  assert.match(agentLoop, /launchNarrationSidecarIfDue[\s\S]*compactForcedNarrationMessages\([\s\S]*max_tokens: NARRATION_SIDECAR_MAX_TOKENS[\s\S]*reasoning: NO_THINKING_REASONING[\s\S]*retryMaxAttempts: 0/, 'cadence narration must use a compact, minimal-thinking, no-retry sidecar')
  ...
  assert.match(agentLoop, /const FINAL_INLINE_ANSWER_REQUEST_TIMEOUT_MS = 12_000/, 'final inline answers must get a realistic Gemini 3.6 provider-start window without falling into deadline finalization')
  ...
  assert.match(agentLoop, /FINAL_SAVED_DELIVERABLE_REQUEST_TIMEOUT_MS = 12_000/, 'saved final deliverables must have a bounded startup window that tolerates Gemini 3.6 provider variance')
  ...
  assert.match(agentLoop, /const response = await createCompletion\(\{[\s\S]*messages: requestMessages,[\s\S]*reasoning: NO_THINKING_REASONING/, 'compact narration sidecar calls must not share the action tool-selection request')
