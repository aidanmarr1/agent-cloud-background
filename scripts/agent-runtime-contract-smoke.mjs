...
async function assertSourceContracts() {
  ...
  assert.ok(agentLoop.includes("return ASSISTANT_PROVIDER !== 'openrouter' || model.trim().length > 0"), 'the pinned OpenRouter balanced route must retain provider-required native tool calls')
  ...
  assert.match(agentConfig, /MODEL_MAX_PROMPT_TOKENS = 983_616[\s\S]*MODEL_MAX_COMPLETION_TOKENS = 65_536[\s\S]*MAX_CONTEXT_MESSAGES = 4_096/, 'Qwen 3.7 Flash must use its published prompt and completion capacity instead of an eight-message context cap')
  ...
  assert.match(prompts, /coherent professional structure suited to the request[\s\S]*Executive Summary or overview[\s\S]*substantive topic-specific sections[\s\S]*References\/Sources/, 'research report deliverables must receive flexible professional-structure guidance')
  assert.match(prompts, /guidance, not a rigid template/, 'report structure guidance must leave topic-specific headings and numbering to the agent')
  ...
  assert.match(taskStrategy, /one coherent professional report[\s\S]*opening synthesis[\s\S]*substantive topic-specific sections[\s\S]*Adapt headings and numbering/, 'research strategy must guide professional reports without imposing one fixed skeleton')
  assert.match(await readFile(join(root, 'src/agent/guards/stepMessages.ts'), 'utf8'), /coherent professional Markdown structure suited to the request[\s\S]*Executive Summary or overview[\s\S]*Choose natural headings and numbering/, 'final research step guidance must preserve agent judgement inside a professional report shape')
  ...
  assert.match(outputVerifier, /Executive Summary\|Summary\|Overview\|Key Findings/, 'report verification must accept a natural opening synthesis heading instead of one mandatory label')
  ...
  assert.match(toolPipeline, /web_search was skipped because the user supplied the exact target[\s\S]*Call browser_navigate with that exact URL now and author a fresh action_label/, 'explicit URL web_search attempts must return to the model for a semantically correct direct-navigation action and label')
  ...
  assert.match(agentLoop, /FAST_SOURCE_ACTION_REQUEST_TIMEOUT_MS = 45_000/, 'source-action starts must tolerate Qwen 3.7 Flash provider selection')
  assert.match(agentLoop, /FAST_ACTION_REQUEST_TIMEOUT_MS = 45_000/, 'non-source action starts must tolerate Qwen 3.7 Flash provider selection')
  ...
  assert.match(agentLoop, /FORCED_NARRATION_REQUEST_TIMEOUT_MS = 5_000/, 'progress narration must use no-thinking generation with a realistic provider-start window')
  ...
  assert.match(agentLoop, /remainingVisibleActions = Math\.max\([\s\S]*state\.visibleToolActionsSinceLastNarration - visibleActionFrontier[\s\S]*acceptProgressNarration\(state,\s*content,\s*\{[\s\S]*remainingVisibleActions/, 'valid progress narration must preserve overflow action-pill cadence after late narration')
  ...
  assert.match(agentLoop, /FAST PROGRESS NARRATION ONLY[\s\S]*Size the update to the evidence:[\s\S]*Lead with the genuinely new result or progress and choose the wording and structure freely/, 'the narration sidecar must size and structure updates naturally around new evidence')
  assert.match(agentLoop, /asynchronous narration-only request has no selected next action[\s\S]*keep it result-only/, 'the narration sidecar must not invent an immediate action it did not select')
  assert.doesNotMatch(agentLoop, /Distinct upcoming focus/, 'the narration sidecar must not turn the next plan phase into a Next sentence')
  assert.match(agentLoop, /Treat it as a progressive evidence trace: select the newest delta[\s\S]*choose the wording and structure freely/, 'the narration sidecar must request varied, progressive finding narration')
  assert.match(agentLoop, /Cadence narration has its own tiny LLM request so action selection never[\s\S]*waits for prose/, 'overdue narration must run independently without blocking tool-call turns')
  ...
  assert.match(agentLoop, /launchNarrationSidecarIfDue[\s\S]*compactForcedNarrationMessages\([\s\S]*max_tokens: NARRATION_SIDECAR_MAX_TOKENS[\s\S]*reasoning: MINIMAL_THINKING_REASONING[\s\S]*retryMaxAttempts: 0/, 'cadence narration must use a compact, provider-compatible minimal-thinking, no-retry sidecar')
  assert.match(agentLoop, /this\.emitter\.progressUpdate\(review\.text,\s*\{[\s\S]*stepIndex: recordStepIdx,[\s\S]*afterToolId,[\s\S]*remainingVisibleActions/, 'accepted narration must be placed exactly after its completed action cluster')
  assert.match(agentLoop, /LLM narration sidecar failed without blocking task actions/, 'narration failures must never block task progress')
  assert.match(agentLoop, /never make tool selection wait for narration[\s\S]*const cadenceNarrationInMainTurn = false/, 'ordinary action selection must remain independent of narration generation')
  ...
  assert.match(agentLoop, /lastToolResults = await toolPipeline\.executeAll\([\s\S]*launchNarrationSidecarIfDue\(\)/, 'completed tool clusters must immediately arm the independent narration sidecar')
  ...
  assert.doesNotMatch(agentLoop, /enabled: false as const, exclude: true/, 'Qwen 3.7 Flash requests must keep minimal reasoning enabled')
  ...
  assert.match(agentLoop, /NARRATION_SIDECAR_REQUEST_TIMEOUT_MS = 4_000[\s\S]*NARRATION_SIDECAR_MAX_TOKENS = 64/, 'tiny progress narration sidecar calls must stay compact while allowing provider startup variance')
  assert.match(agentLoop, /launchNarrationSidecarIfDue[\s\S]*acceptProgressNarration\(state,\s*content,[\s\S]*review\.status !== 'accepted'[\s\S]*retryNarrationCadenceAttemptWithoutNewAction/, 'sidecar narration must be validated before resetting cadence so unusable or repetitive text cannot pass')
  ...
  assert.match(agentLoop, /SOURCE OPENING REQUIRED[\s\S]*Do not call web_search again[\s\S]*parallel batch of up to 3 read_document\/http_request calls/, 'source-saturated compact research turns must allow up to three independent supported URLs instead of searching again')
  ...
  assert.match(agentLoop, /const FINAL_INLINE_ANSWER_REQUEST_TIMEOUT_MS = 60_000/, 'final inline answers must tolerate Qwen 3.7 Flash provider startup')
  ...
  assert.match(agentLoop, /Existing saved deliverable verified after final text confirmation/, 'a valid saved artifact must close the final plan after the model confirms it in text')
  assert.match(agentLoop, /readFileInSandbox\(conversationId, existingDeliverablePath\)[\s\S]*outputVerifier\.verify[\s\S]*state\.currentStepIdx = state\.currentPlanItems\.length/, 'the final text boundary must read back and verify the real saved file before completing')
  ...
  assert.match(agentLoop, /FINAL_SAVED_DELIVERABLE_REQUEST_TIMEOUT_MS = 60_000/, 'saved final deliverables must have a bounded startup window that tolerates Qwen 3.7 Flash provider variance')
  ...
  assert.match(agentLoop, /const requestReasoning = useCompactNarration[\s\S]*MINIMAL_THINKING_REASONING[\s\S]*isFinalInlineAnswerTurn[\s\S]*TASK_REASONING/, 'all task and final-answer turns must use minimal reasoning')
  ...
  assert.match(agentLoop, /const response = await createCompletion\(\{[\s\S]*messages: requestMessages,[\s\S]*reasoning: MINIMAL_THINKING_REASONING/, 'compact narration sidecar calls must use a separate provider-compatible request from action tool selection')
...
export async function runLedgerSmoke() {
  ...
  assert.ok(
    ...
    'overdue narration must not block a useful tool call while the async narration sidecar is pending',
  )
