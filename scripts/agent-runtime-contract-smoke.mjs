...
async function assertSourceContracts() {
  ...
  assert.match(planManager, /PLANNER_JSON_REQUEST_TIMEOUT_MS = 3_800/, 'planner JSON calls must have a bounded timeout without causing the visible startup failure')
  assert.match(planManager, /PLANNER_RELAXED_JSON_REQUEST_TIMEOUT_MS = 4_600/, 'planner startup must have a short model-owned primary path before strict JSON recovery')
  assert.match(planManager, /PLANNER_REPAIR_REQUEST_TIMEOUT_MS = 3_500/, 'planner repair calls must have a bounded timeout that does not surface the startup toast')
  assert.match(planManager, /requestTimeoutMs:\s*relaxedJsonMode[\s\S]*PLANNER_RELAXED_JSON_REQUEST_TIMEOUT_MS[\s\S]*PLANNER_JSON_REQUEST_TIMEOUT_MS/, 'initial planner calls must pass the bounded planner timeout and the lean retry timeout')
  assert.match(planManager, /this\.planPromise = start\.then\(\(\) => this\.attemptPlanCall\(0,\s*true\)\)/, 'planner startup must begin with the faster model-owned planning path')
  ...
  assert.match(agentLoop, /function supportsProviderRequiredToolChoice\(\): boolean \{\s*return true\s*\}/, 'tool-action turns must use provider-required tool choice first so research does not spin on text-only replies')
  ...
  assert.match(agentConfig, /COMPLEXITY_ITERATION_BONUS\s*=\s*\{\s*1:\s*0,\s*2:\s*24,\s*3:\s*64\s*\}/, 'complex research tasks need expanded but not runaway iteration bonus')
  assert.match(agentConfig, /MIN_RESEARCH_CALLS_BY_COMPLEXITY\s*=\s*\{\s*1:\s*3,\s*2:\s*6,\s*3:\s*11\s*\}/, 'complex research phases need a serious evidence-call floor without overworking default tasks')
  assert.match(agentConfig, /MIN_OPENED_SOURCE_BREADTH_BY_COMPLEXITY\s*=\s*\{\s*1:\s*2,\s*2:\s*4,\s*3:\s*6\s*\}/, 'complex research phases need source diversity without forcing excessive default breadth')
  assert.match(agentConfig, /COMPLEXITY_BUDGET_MULTIPLIERS\s*=\s*\{\s*1:\s*0\.6,[\s\S]*2:\s*1\.0,[\s\S]*3:\s*1\.3,/, 'task effort should preserve quality headroom while speed fixes remove stalls instead of reducing depth')
  ...
  assert.match(planManager, /PLANNER_QUALITY_REPAIR_ATTEMPTS = 3/, 'planner quality failures need multiple model-authored repair attempts before giving up')
  ...
  assert.doesNotMatch(toolPipeline, /derivedActionLabel|actionLabelFromToolIntent/, 'tool pipeline must not invent deterministic action labels; live labels must be model-authored')
  assert.match(toolPipeline, /action_label must be model-authored/, 'tool pipeline must ask the model to retry missing action labels instead of manufacturing them')
  ...
  assert.match(agentIdentity, /AGENT_IDENTITY_DISCLOSURE_RESPONSE = "I'm Agent, made by an AI agent lab\. I can't disclose anything further\."/, 'identity disclosure questions must use the required fixed Agent identity response')
  ...
  assert.match(chatRoute, /latestUserAskedAgentIdentityDisclosure\(messages\)[\s\S]*emitter\.textDelta\(AGENT_IDENTITY_DISCLOSURE_RESPONSE\)[\s\S]*emitter\.done\(\)/, 'direct chat identity disclosure must bypass provider generation')
  assert.match(agentLoop, /latestUserAskedAgentIdentityDisclosure\(messages\)[\s\S]*this\.emitAgentIdentityDisclosureAnswer\(\)/, 'misrouted identity disclosure tasks must stop before planning or tools')
  assert.match(agentLoop, /directives\.some\(directive => isAgentIdentityDisclosureQuestion\(directive\.content\)\)[\s\S]*this\.emitAgentIdentityDisclosureAnswer\(\)/, 'live identity disclosure follow-ups must not enter the model/tool loop')
  ...
  assert.match(chatRoute, /const rawMessages = validation\.data\.messages[\s\S]*const directChat = shouldUseDirectChat\(rawMessages\)[\s\S]*const messagesPromise = directChat \|\| !hasUnhydratedAttachments\(rawMessages\)[\s\S]*hydrateMessageAttachmentsForUser\(rawMessages,\s*userId\)[\s\S]*const creditsPromise = assertServerCreditsAvailable\(userId\)[\s\S]*Promise\.all\(\[[\s\S]*creditsPromise[\s\S]*messagesPromise[\s\S]*accessPromise[\s\S]*workerAvailabilityPromise/, 'chat route should route before attachment hydration and run startup checks in parallel without deterministic bypasses')
  ...
  assert.match(webSearchSource, /WEB_SEARCH_RESULT_COUNT\s*=\s*15/, 'web_search must request and display 15 results')
  ...
  assert.match(planManager, /this\.acknowledgementPromise = this\.emitModelGeneratedAcknowledgement\('task'\)[\s\S]*this\.planPromise = start\.then\(\(\) => this\.attemptPlanCall\(0,\s*true\)\)/, 'model acknowledgement and planner generation must start in parallel so the plan appears quickly after the acknowledgement')
  ...
  assert.match(chatRoute, /skipStartupAcknowledgement:\s*shouldUseExternalTaskWorker\(\)/, 'external workers must skip their duplicate startup acknowledgement while the route-owned acknowledgement streams first')
  ...
  assert.doesNotMatch(prompts, /quickly\|quick\|brief\|short\|concise/, 'concise output style must not downgrade research depth by itself')
  ...
  assert.match(agentLoop, /FORCED_NARRATION_REQUEST_TIMEOUT_MS = 2_800/, 'progress narration must have enough start time to stream reliably without feeling slow')
  assert.match(agentLoop, /FORCED_NARRATION_INACTIVITY_TIMEOUT_MS = 900/, 'progress narration must not fail on normal sub-second provider pauses')
  ...
  assert.doesNotMatch(activityDescriber, /actionLabelFromToolIntent/, 'visible action wording must not use deterministic topic-label builders')
  assert.doesNotMatch(toolPipeline, /actionLabelFromToolIntent\(toolName,\s*topic\)/, 'tool metadata repair must not use deterministic topic-label builders')
  ...
  assert.match(agentConfig, /MIN_RESEARCH_CALLS_BY_COMPLEXITY = \{ 1: 3, 2: 6, 3: 11 \}/, 'research evidence thresholds must stay above shallow three-action phases while keeping default effort a notch lower')
  assert.match(agentConfig, /MIN_OPENED_SOURCE_BREADTH_BY_COMPLEXITY = \{ 1: 2, 2: 4, 3: 6 \}/, 'substantive research phases must require source diversity without overcharging default tasks')
  ...
  assert.match(agentConfig, /inactivityTimeoutMs:\s*IS_OLLAMA \? 120_000 : 1_200/, 'invisible model stalls should recover quickly instead of making the UI look frozen')
  ...
  assert.match(agentLoop, /compactResearchPhaseCanAdvance[\s\S]*compactResearchEvidenceComplete\(state\)[\s\S]*PHASE EVIDENCE READY[\s\S]*Do not call more tools for this phase/, 'compact research must close a phase once the evidence floor is met instead of offering extra research tools')
  ...
  assert.match(agentLoop, /FORCED_NARRATION_REQUEST_TIMEOUT_MS = 2_800[\s\S]*FORCED_NARRATION_MAX_TOKENS = 48/, 'tiny progress narration turns must stay compact without self-cancelling before the provider can start')
  assert.match(agentLoop, /processedCompactNarrationTurn[\s\S]*lastStreamResult\.toolCalls\.size === 0[\s\S]*lastStreamResult\.assistantContent\.trim\(\)[\s\S]*visibleToolActionsSinceLastNarration = 0/, 'accepted compact narration turns must reset cadence so the agent returns to tool work instead of repeating status text')
  ...
  assert.match(agentLoop, /SOURCE OPENING REQUIRED[\s\S]*Do not call web_search again[\s\S]*read_document[\s\S]*browser_navigate/, 'source-saturated compact research turns must tell the model to open or extract an existing source instead of searching again')
  ...
  assert.match(agentLoop, /compactResearchOpenedSourceToolsForState[\s\S]*new Set\(\['read_document'\]\)[\s\S]*!hasRenderedBrowserContext\(state\)[\s\S]*allowed\.delete\('browser_get_content'\)/, 'source-saturated compact research turns must prefer text extraction and only expose browser content after a real browser page exists')
  ...
  assert.match(agentLoop, /const FINAL_INLINE_ANSWER_REQUEST_TIMEOUT_MS = 2_800/, 'final inline answers must not get a long silent startup window')
  assert.match(agentLoop, /const FINAL_SAVED_DELIVERABLE_MAX_TOKENS = 900/, 'saved final deliverables must start with a small write chunk instead of one slow giant report/code call')
  ...
  assert.match(agentLoop, /FINAL_SAVED_DELIVERABLE_REQUEST_TIMEOUT_MS = 3_600/, 'saved final deliverables must have a bounded startup window that still tolerates provider variance')
  assert.match(agentLoop, /FINAL_SAVED_DELIVERABLE_ITERATION_TIMEOUT_MS = 5_000/, 'saved final deliverables must have enough stream runway to finish a real file write')
  ...
  assert.match(agentLoop, /const requestReasoning = useCompactNarration[\s\S]*isFinalInlineAnswerTurn[\s\S]*reasoning:\s*\{\s*effort:\s*'minimal' as const,\s*exclude:\s*true\s*\}/, 'final chat-answer turns should use minimal hidden reasoning to avoid invisible stalls')
  ...
  assert.match(toolPipeline, /After 4 completed visible actions it becomes a hard gate/, 'visible tool preflight must hard-gate the fifth visible action until narration is accepted')
  assert.match(toolPipeline, /this visible tool call was skipped because 4 visible actions/, 'visible tool preflight must block the fifth visible action when narration is overdue')
...
export async function runLedgerSmoke() {
  ...
  phaseEndAvailableEvidenceState.stepResearchCallCount = 6
  phaseEndAvailableEvidenceState.stepToolCallCount = 6
