...
async function assertSourceContracts() {
  ...
  assert.match(chatRoute, /workerStartupPlanPromise[\s\S]*routeStartupPlanPromise[\s\S]*taskStartPromise = workerStartupPlanPromise\.then/, 'external-worker route may wait only for the fast visible startup plan handoff before enqueue')
  assert.match(chatRoute, /const queuedPayload: TaskJobPayload[\s\S]*startupPlanExpected: false[\s\S]*return enqueueTaskJob\([\s\S]*payload: queuedPayload/, 'external-worker route must pass the visible route plan into the worker payload to avoid duplicate slow planning')
