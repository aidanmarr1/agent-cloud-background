...
async function assertSourceContracts() {
  ...
  assert.match(planManager, /PLANNER_CONTROL_REASONING = \{ effort: 'none' as const, exclude: true \}/, 'planner JSON control calls must disable hidden reasoning')
  assert.match(planManager, /PLANNER_ACK_REASONING = \{ effort: 'none' as const, exclude: true \}/, 'startup acknowledgement must not wait behind hidden reasoning')
  ...
  assert.match(llm, /DEFAULT_REASONING_EFFORT = normalizeReasoningEffort\([\s\S]*'none'[\s\S]*\)/, 'reasoning must default to OpenRouter’s lowest mode')
  ...
  assert.match(agentLoop, /lastStreamResult\.cadenceProgressUpdate[\s\S]*acceptProgressNarration\(state,[\s\S]*remainingVisibleActions: lastStreamResult\.cadenceProgressVisibleActionsAfter \|\| 0[\s\S]*resetCadence:\s*true/, 'valid native progress narration must preserve overflow action-pill cadence after late narration')
  ...
  assert.match(agentLoop, /cadenceNarrationMainTurnGuidance[\s\S]*Put one natural completed-result update in that field, sized to the newest evidence rather than a fixed template/, 'native action narration must size updates naturally around new evidence')
  ...
  assert.match(agentLoop, /MINIMAL_THINKING_REASONING = \{ effort: 'none' as const, exclude: true \}/, 'agent control turns must use OpenRouter’s lowest reasoning mode')
  assert.match(agentLoop, /TASK_REASONING = \{ effort: 'none' as const, exclude: true \}/, 'Qwen action turns must disable hidden reasoning so required tools remain compatible')
  ...
  assert.match(agentLoop, /lastStreamResult\.cadenceProgressUpdate[\s\S]*acceptProgressNarration\(state,[\s\S]*resetCadence:\s*true/, 'native narration must be validated after the billed stream before resetting cadence')
