...
async function assertSourceContracts() {
  ...
  assert.match(agentConfig, /BASE_ITERATIONS\s*=\s*44/, 'long tasks need a larger base iteration budget')
  assert.match(agentConfig, /MAX_ITERATIONS\s*=\s*180/, 'long tasks need a higher global iteration ceiling')
  assert.match(agentConfig, /COMPLEXITY_ITERATION_BONUS\s*=\s*\{\s*1:\s*0,\s*2:\s*32,\s*3:\s*84\s*\}/, 'complex research tasks need expanded but not runaway iteration bonus')
  assert.match(agentConfig, /MIN_RESEARCH_CALLS_BY_COMPLEXITY\s*=\s*\{\s*1:\s*3,\s*2:\s*7,\s*3:\s*12\s*\}/, 'complex research phases need a serious evidence-call floor without overworking default tasks')
  ...
  assert.match(agentConfig, /COMPLEXITY_BUDGET_MULTIPLIERS\s*=\s*\{\s*1:\s*0\.7,[\s\S]*2:\s*1\.1,[\s\S]*3:\s*1\.45,/, 'task effort should preserve quality headroom while speed fixes remove stalls instead of reducing depth')
  ...
  assert.match(chatRoute, /deferredPrefaceEvents\s*=\s*\[[\s\S]*routeStartupAcknowledgementPromise[\s\S]*queuedStartupPlanPromise[\s\S]*\]/, 'chat route must stream the model-generated acknowledgement and bounded route-owned plan')
  ...
  assert.match(chatRoute, /const queuedStartupPlanPromise = routeStartupPlanPromise\.then[\s\S]*plan\?\.items\?\.length \? plan : null/, 'external-worker route must use one bounded route-owned startup plan promise for both visible plan and worker queue payload')
  assert.match(chatRoute, /taskStartPromise = queuedStartupPlanPromise\.then\(\(plan\) => \{[\s\S]*startupPlanExpected: false[\s\S]*payload: queuedTaskPayload/, 'external-worker route must enqueue a payload that already carries the route plan or explicit no-plan release flag')
  assert.match(chatRoute, /startupPlanExpected:\s*!directChat && useExternalWorker/, 'worker must still briefly wait for the route-owned startup plan after claiming the already-queued job')
  assert.match(chatRoute, /queuedStartupPlanPromise\.then\(\(plan\) => \{[\s\S]*return \[\{ type: 'plan'/, 'route plan emission must share the same bounded promise used for the queued worker payload')
  ...
  assert.match(chatRoute, /if \(raced\.index === 0\)[\s\S]*if \(!ackPrefaceExpired\) emitEvents\(raced\.events\)[\s\S]*const planEvents = buffered\.get\(1\)[\s\S]*emitEvents\(planEvents\)[\s\S]*else if \(!acknowledgementSettled/, 'chat route must emit acknowledgement first, then release any buffered plan without requiring both to finish together')
  ...
  assert.match(agentLoop, /FAST_ACTION_REQUEST_TIMEOUT_MS = 1_500/, 'between-tool action turns must stay under the fast start budget')
  ...
  assert.match(agentLoop, /FORCED_NARRATION_REQUEST_TIMEOUT_MS = 1_500/, 'progress narration must start inside the fast 1-2 second window')
  assert.match(agentLoop, /FORCED_NARRATION_INACTIVITY_TIMEOUT_MS = 450/, 'progress narration must recover quickly from short provider stalls')
  ...
  assert.match(agentConfig, /MIN_RESEARCH_CALLS_BY_COMPLEXITY = \{ 1: 3, 2: 7, 3: 12 \}/, 'research evidence thresholds must stay above shallow three-action phases while keeping default effort a notch lower')
  ...
  assert.match(agentLoop, /FORCED_NARRATION_REQUEST_TIMEOUT_MS = 1_500[\s\S]*FORCED_NARRATION_MAX_TOKENS = 48/, 'tiny progress narration turns must stay compact without lingering before the provider starts')
