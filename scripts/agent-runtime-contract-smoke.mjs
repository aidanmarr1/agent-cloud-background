...
async function assertSourceContracts() {
  ...
  assert.match(streamProcessor, /contentOnlyStallMs[\s\S]*Math\.min\(5_000,\s*Math\.max\(1_500,\s*this\.tierTimeouts\.contentOnlyTimeoutMs\)\)/, 'content-only action stalls must respect tighter fast-action timeouts')
  ...
  assert.match(agentLoop, /FORCED_NARRATION_REQUEST_TIMEOUT_MS = 1_400[\s\S]*FORCED_NARRATION_MAX_TOKENS = 48/, 'tiny progress narration turns must stay compact without self-cancelling before the provider can start')
