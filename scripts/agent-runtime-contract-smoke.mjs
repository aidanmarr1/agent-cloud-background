...
async function assertSourceContracts() {
  ...
  assert.match(planManager, /PLANNER_FAST_JSON_REQUEST_TIMEOUT_MS = 2_200/, 'fast planner must not sit for multi-second startup delays')
  assert.match(planManager, /PLANNER_JSON_REQUEST_TIMEOUT_MS = 3_000/, 'strict planner JSON calls must have a bounded timeout without causing a long visible startup failure')
  assert.match(planManager, /PLANNER_RELAXED_JSON_REQUEST_TIMEOUT_MS = 2_800/, 'planner startup must have a short model-owned primary path before strict JSON recovery')
  assert.match(planManager, /PLANNER_REPAIR_REQUEST_TIMEOUT_MS = 2_200/, 'planner repair calls must have a short bounded timeout that does not surface the startup toast')
  ...
  assert.match(planManager, /requestTimeoutMs:\s*fastPlannerMode[\s\S]*PLANNER_FAST_JSON_REQUEST_TIMEOUT_MS[\s\S]*PLANNER_RELAXED_JSON_REQUEST_TIMEOUT_MS[\s\S]*PLANNER_JSON_REQUEST_TIMEOUT_MS/, 'initial planner calls must pass the fast bounded planner timeout and lean fallback timeouts')
  ...
  assert.match(agentConfig, /MIN_RESEARCH_CALLS_BY_COMPLEXITY\s*=\s*\{\s*1:\s*3,\s*2:\s*8,\s*3:\s*14\s*\}/, 'complex research phases need a serious evidence-call floor without overworking quick tasks')
  assert.match(agentConfig, /MIN_OPENED_SOURCE_BREADTH_BY_COMPLEXITY\s*=\s*\{\s*1:\s*2,\s*2:\s*5,\s*3:\s*7\s*\}/, 'complex research phases need source diversity without forcing excessive quick-task breadth')
  assert.match(agentConfig, /COMPLEXITY_BUDGET_MULTIPLIERS\s*=\s*\{\s*1:\s*0\.7,[\s\S]*2:\s*1\.2,[\s\S]*3:\s*1\.6,/, 'task effort should preserve quality headroom while speed fixes remove stalls instead of reducing depth')
  ...
  assert.match(planManager, /PLANNER_CONTROL_REASONING = \{ effort: 'high' as const, exclude: true \}/, 'planner JSON control calls must use the configured high reasoning effort')
  assert.match(planManager, /PLANNER_ACK_REASONING = \{ effort: 'high' as const, exclude: true \}/, 'startup acknowledgement must use the configured high reasoning effort')
  ...
  assert.match(llm, /DEFAULT_REASONING_EFFORT = normalizeReasoningEffort\([\s\S]*reasoningEnvForProvider\(ASSISTANT_PROVIDER\),[\s\S]*'high',/, 'reasoning must default to high effort unless explicitly overridden')
  ...
  assert.match(chatRoute, /ROUTE_STARTUP_ACK_REASONING = \{ effort: 'high' as const, exclude: true \}/, 'route startup acknowledgement must use the configured high reasoning effort')
  ...
  assert.match(agentLoop, /const fastActionTurn = activeTools\.length > 0 &&[\s\S]*!isPostCompletion &&[\s\S]*isFastActionToolTurn\(state,\s*this\.options\.messages\)/, 'active tool-selection turns must enter the fast action lane by default')
  ...
  assert.match(agentLoop, /fastActionTurn[\s\S]*reasoning:\s*\{\s*effort: 'high' as const,\s*exclude: true\s*\}/, 'between-tool action turns must use high reasoning effort with hidden output excluded')
  ...
  assert.match(agentConfig, /MIN_RESEARCH_CALLS_BY_COMPLEXITY = \{ 1: 3, 2: 8, 3: 14 \}/, 'research evidence thresholds must stay above shallow three-action phases while keeping quick tasks light')
  assert.match(agentConfig, /MIN_OPENED_SOURCE_BREADTH_BY_COMPLEXITY = \{ 1: 2, 2: 5, 3: 7 \}/, 'substantive research phases must require source diversity without overcharging quick tasks')
  ...
  assert.match(agentLoop, /reasoning:\s*\{\s*effort:\s*'high'/, 'forced narration turns must use the configured high reasoning effort')
  ...
  assert.match(agentLoop, /const requestReasoning = useCompactNarration[\s\S]*isFinalInlineAnswerTurn[\s\S]*reasoning:\s*\{\s*effort:\s*'high' as const,\s*exclude:\s*true\s*\}/, 'final chat-answer turns should use high reasoning effort with hidden output excluded')
