...
async function assertSourceContracts() {
  ...
  assert.match(chatRoute, /taskStartPromise = accessPromise\.then\(async \(accessResult\)[\s\S]*throw await taskAccessStreamError\(accessResult\.response\)[\s\S]*enqueueTaskJob/, 'task access must still gate enqueue after first paint')
  assert.doesNotMatch(chatRoute, /taskStartPromise = Promise\.all\(\[[\s\S]*workerAvailabilityPromise[\s\S]*enqueueTaskJob/, 'worker readiness checks must not hold the durable job out of the worker queue after first paint')
