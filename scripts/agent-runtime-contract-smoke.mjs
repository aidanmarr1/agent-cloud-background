...
async function assertSourceContracts() {
  ...
  assert.match(planManager, /PLANNER_ACK_MAX_TOKENS = 256/, 'planner acknowledgement generation must leave room after Muse mandatory minimal reasoning for a prompt visible opening')
  ...
  assert.match(planManager, /PLANNER_ACK_REQUEST_TIMEOUT_MS = 20_000/, 'precomputed-plan acknowledgement requests must tolerate routed provider startup without becoming unbounded')
