...
async function assertSourceContracts() {
  ...
  assert.ok(agentLoop.includes('return model.trim().length > 0'), 'the pinned Muse route must retain provider-required native tool calls')
  ...
  assert.match(llm, /ASSISTANT_REASONING_EFFORT = 'minimal'[\s\S]*function providerReasoningPayload[\s\S]*effort:\s*ASSISTANT_REASONING_EFFORT[\s\S]*exclude:\s*true/, 'every Muse request lane must use the endpoint\'s mandatory lowest reasoning effort and hide reasoning output')
  assert.doesNotMatch(llm, /temperature:\s*_temperature/, 'Muse must preserve its supported per-call temperature setting')
  assert.match(llm, /const hasNativeTools = Array\.isArray\(rest\.tools\)[\s\S]*hasNativeTools \? \{ tool_choice: 'auto' as const \} : \{\}/, 'Muse tool turns must use automatic tool choice while keeping all healthy tools available')
  assert.match(llm, /usage:\s*\{\s*include:\s*true\s*\}/, 'OpenRouter calls must explicitly request usage data')
