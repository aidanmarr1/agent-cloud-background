...
async function assertSourceContracts() {
  ...
  assert.match(agentLoop, /Keep the product default: research findings are saved as a concise Markdown deliverable/, 'bare overview custom override must preserve markdown-by-default research output')
  assert.match(agentLoop, /FAST-LANE RESEARCH OVERRIDE[\s\S]*Saved instructions asking for unusually deep research do not apply[\s\S]*product default still saves research findings as concise Markdown/, 'bare overview runtime must inject a high-priority fast-lane instruction while keeping markdown output')
  ...
  assert.match(toolPipeline, /maybeAutoOpenSearchResult/, 'web_search should be able to trigger source reading without a full extra model turn')
  ...
  assert.match(agentLoop, /FORCED_NARRATION_REQUEST_TIMEOUT_MS = 2_000[\s\S]*FORCED_NARRATION_MAX_TOKENS = 48/, 'tiny progress narration turns must stay compact without lingering before the provider starts')
...
export async function runLedgerSmoke() {
  ...
  assert.equal(bareResearchDepth.requiredCalls, 3, 'bare research prompts should avoid one-source shallow research without inheriting deep-report quotas: ' + JSON.stringify(bareResearchDepth))
  assert.equal(bareResearchDepth.requiredSourceBreadth, 2, 'bare research prompts should need compact multi-source breadth: ' + JSON.stringify(bareResearchDepth))
