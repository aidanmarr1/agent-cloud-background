...
async function assertSourceContracts() {
  ...
  assert.match(planManager, /PLANNER_ACK_STREAM_TIMEOUT_MS = 20_000/, 'startup acknowledgement must tolerate routed provider startup without becoming unbounded')
  ...
  assert.match(planManager, /plannerDeadlineAtMs = Date\.now\(\) \+ PLANNER_OVERALL_DEADLINE_MS[\s\S]*this\.planPromise = start\(\)[\s\S]*this\.attemptPlanCall\(0,\s*true\)[\s\S]*await this\.acknowledgementPromise/, 'planner and acknowledgement must run concurrently while plan completion remains fenced on acknowledgement accounting')
  ...
  assert.match(prompts, /Every non-empty plan must still culminate[\s\S]*last phase may naturally combine final work, verification, and handoff/, 'the planning model must complete the requested result without a stock final-phase shape')
  ...
  assert.match(llm, /ASSISTANT_SUPPORTS_FILE_INPUT = false/, 'the pinned GLM route must route files through extraction tools')
  assert.match(llm, /ASSISTANT_SUPPORTS_AUDIO_INPUT = false/, 'the pinned GLM route must route audio through extraction tools')
  ...
  assert.match(planManager, /createStreamingCompletion[\s\S]*for await \(const chunk of stream\)[\s\S]*sanitizePlannerAck\(firstBuffer \|\| ack\)[\s\S]*isUsablePlannerAck\(sanitizedAck, request\)[\s\S]*emitVisible\(sanitizedAck\)/, 'task acknowledgement must come from the model but be validated as one complete candidate before it is shown')
  ...
  assert.match(streamProcessor, /let visibleTextSegmentStarted = false[\s\S]*const segment = visibleTextSegmentStarted \? content : `\\n\\n\$\{content\}`/, 'each distinct model prose turn must begin at a paragraph boundary instead of concatenating with earlier narration')
  ...
  assert.match(agentConfig, /MODEL_MAX_PROMPT_TOKENS = 917_504[\s\S]*MODEL_MAX_COMPLETION_TOKENS = 131_072[\s\S]*MAX_CONTEXT_MESSAGES = 4_096/, 'GLM 5.3 Flash must use the exact Z.ai endpoint context and completion limits')
  ...
  assert.match(llm, /PINNED_OPENROUTER_PROVIDER\s*=\s*'z-ai'[\s\S]*exactOpenRouterProviderRoute/, 'runtime must pin GLM to OpenRouter\'s exact Z.ai provider route')
  ...
  assert.match(llm, /ASSISTANT_REASONING_EFFORT = 'minimal'[\s\S]*function providerReasoningPayload[\s\S]*effort:\s*ASSISTANT_REASONING_EFFORT[\s\S]*exclude:\s*true/, 'GLM calls must pin reasoning to minimal and hide internal traces')
  assert.match(llm, /const hasNativeTools = Array\.isArray\(rest\.tools\)[\s\S]*hasNativeTools \? \{ tool_choice: 'auto' as const \} : \{\}/, 'GLM tool turns must use the proven automatic tool-choice boundary')
  assert.match(llm, /usage:\s*\{\s*include:\s*true\s*\}/, 'OpenRouter calls must explicitly request usage data for compatibility')
  assert.match(llm, /provider:\s*exactOpenRouterProviderRoute\(\)/, 'OpenRouter calls must use only the exact Z.ai provider route')
  ...
  assert.match(planManager, /this\.acknowledgementPromise = this\.emitModelGeneratedAcknowledgement\('task'\)[\s\S]*this\.planPromise = start\(\)[\s\S]*this\.attemptPlanCall\(0,\s*true\)[\s\S]*await this\.acknowledgementPromise/, 'model acknowledgement and planner must start concurrently while completion still waits for acknowledgement accounting')
...
export async function runLedgerSmoke() {
  ...
  assert.equal(
    ...
    'Found the embedded media page did not expose a title or description from that URL.',
  )
  assert.equal(
