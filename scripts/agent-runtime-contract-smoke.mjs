...
async function assertSourceContracts() {
  ...
  assert.match(planManager, /PLANNER_ACK_MAX_TOKENS = 192/, 'planner acknowledgement repair must leave room for mandatory hidden reasoning while keeping startup generation bounded')
  ...
  assert.match(planManager, /PLANNER_CONTROL_REASONING = \{ effort: 'minimal' as const, exclude: true \}/, 'planner JSON control calls must use minimal reasoning')
  assert.match(planManager, /PLANNER_ACK_REASONING = \{ effort: 'minimal' as const, exclude: true \}/, 'startup acknowledgement must use minimal reasoning')
  ...
  assert.match(llm, /ASSISTANT_REASONING_EFFORT = 'minimal'[\s\S]*function providerReasoningPayload[\s\S]*effort:\s*ASSISTANT_REASONING_EFFORT[\s\S]*exclude:\s*true/, 'Muse calls must pin mandatory reasoning to minimal and hide internal traces')
  assert.doesNotMatch(llm, /temperature:\s*_temperature/, 'Muse must preserve its supported per-call temperature setting')
  assert.match(llm, /const hasNativeTools = Array\.isArray\(rest\.tools\)[\s\S]*hasNativeTools \? \{ tool_choice: 'auto' as const \} : \{\}/, 'Muse tool turns must use its only live-supported automatic tool-choice mode while keeping all healthy tools available')
  ...
  assert.match(agentLoop, /DEEP_TASK_REASONING = \{ effort: 'minimal' as const, exclude: true \}/, 'final synthesis and build turns must use minimal reasoning')
