...
async function assertSourceContracts() {
  ...
  assert.doesNotMatch(llm, /PINNED_OPENROUTER_PROVIDER|exactOpenRouterProviderRoute/, 'runtime must keep balanced routing across Gemini’s standard Google hosts')
  ...
  assert.match(llm, /reasoning:\s*\{[\s\S]*effort:\s*'minimal'[\s\S]*exclude:\s*true/, 'every OpenRouter call must use Gemini’s mandatory lowest reasoning and exclude internal traces')
  ...
  assert.match(agentLoop, /MINIMAL_THINKING_REASONING = \{ effort: 'minimal' as const, exclude: true \}/, 'agent control turns must use Gemini’s lowest reasoning mode')
  assert.match(agentLoop, /TASK_REASONING = \{ effort: 'minimal' as const, exclude: true \}/, 'Gemini action turns must use the lowest supported reasoning so required tools remain compatible')
