...
async function assertSourceContracts() {
  ...
  assert.match(prompts, /a narrow lookup or single action often needs 2/, 'planner prompt must keep simple task progress compact')
  assert.match(prompts, /ordinary research, comparison, or build work typically needs 3-4/, 'planner prompt must calibrate ordinary tasks to a small useful module set')
  assert.match(prompts, /deep multi-region, multi-artifact, illustrated, or PDF work often needs 5/, 'planner prompt must preserve room for deep multi-output work')
  assert.match(prompts, /These are tendencies, not caps or templates/, 'planner module counts must remain adaptive rather than becoming fixed templates')
  assert.match(prompts, /One visible module represents a meaningful workstream or outcome, not one source page, tool call, or internal micro-step/, 'planner must group internal actions beneath meaningful visible modules')
  assert.match(prompts, /End every non-empty plan with a user-facing completion module that names the concrete answer, action state, artifact, report, export, or handoff/, 'planner must make the user-facing completion outcome explicit')
  ...
  assert.ok(agentLoop.includes('return model.trim().length > 0'), 'the pinned DeepSeek route must retain provider-required native tool calls')
  ...
  assert.match(agentLoop, /toolCallsNeedStartupReady\(lastStreamResult\.toolCalls\)[\s\S]*await this\.awaitWithTaskSignal\(this\.options\.startupReadyPromise\)/, 'sandbox-dependent actions must retain their execution-time readiness fence')
  assert.match(chatTaskRunner, /startupReadyPromise = runClaimedPreChargeBootstrap\(\s*'sandbox_startup'/, 'parallel sandbox startup must retain typed durable-worker retry classification')
  assert.match(agentLoop, /isRetryableTaskInfrastructureInitializationError\(err\)\) throw err/, 'typed startup failures must escape the agent UI loop for durable worker recovery')
  ...
  assert.doesNotMatch(planManager, /requirements\.push\('Boot local preview'\)|requirements\.push\('Visually verify the current browser rendering'\)/, 'website plans must not force localhost or visual QA when another verification route is more appropriate')
  assert.match(planManager, /Choose verification methods that fit the website and the risks actually present/, 'website plans must leave the verification route to task-aware model judgment')
  ...
  assert.match(
    eventDispatcher,
    /settleHiddenComputerPanelItem[\s\S]*if \(!isBrowserTool\(event\.name\)\)[\s\S]*removeComputerPanelItem\(this\.conversationId,\s*panelId\)/,
    'hidden skipped non-browser tools must remove their streaming computer-panel placeholder',
  )
  ...
  assert.match(chatInput, /clampTaskInput/, 'main composer must clamp programmatic inserts, drafts, and paste')
  ...
  assert.match(chatInput, /startsNewTask\?: boolean/, 'composer must distinguish a new home task from a live instruction for the current task')
  assert.match(chatInput, /currentTaskStreaming = isStreaming && !startsNewTask/, 'a different running task must not take over the home composer')
  assert.match(chatInput, /optimisticTaskStarting = submitPending && !currentTaskStreaming && !isProcessingFiles/, 'composer must show a running state immediately after task submit before current-task stream state flushes')
  assert.match(chatInput, /showStopButton = \(currentTaskStreaming \|\| optimisticTaskStarting\)/, 'composer send button must switch to stop/running state only for its current task')
  assert.match(heroSection, /<ChatInput[\s\S]*startsNewTask/, 'the home composer must opt into independent new-task behavior')
  ...
  assert.match(agentLoop, /Choose the length and structure from the task and completed context[\s\S]*do not force the same layout across tasks/, 'final handoffs must let the model choose a task-shaped structure instead of inheriting a fixed acknowledgement layout')
  assert.match(agentLoop, /If one or more files are attached below[\s\S]*user can open those attachments/, 'artifact handoffs must naturally direct the user to attached deliverables')
  assert.match(prompts, /End every non-empty plan with a user-facing completion module[\s\S]*may combine final synthesis, writing, verification, export, and delivery/, 'the planning model must expose the concrete requested result without a stock handoff-only phase')
  assert.doesNotMatch(planManager, /preserveModelAuthoredPlan|ensureFinalDeliveryStep/, 'runtime plan handling must not rewrite the model-authored final phase')
  assert.doesNotMatch(planManager, /const deliveryTitle = `Deliver \$\{target\}`/, 'runtime must not append a fallback Deliver title')
  assert.match(prompts, /Research method is a model decision, not a fixed browser workflow/, 'the model must choose between extraction and full browser use')
  assert.match(prompts, /The full browser is optional unless rendered state/, 'ordinary research must not require opening the full browser')
  assert.ok(
  ...
  assert.match(
    messageSlice,
    /const lastIndex = messages\.length - 1[\s\S]*if \(current\?\.role === 'assistant'\)[\s\S]*messages\.push\(boundedUserMessage, continuation\)/,
    'live directive messages must split only the final assistant and keep the continuing turn after the clamped user redirect',
  )
  ...
  assert.match(agentMessage, /const completedGroupCount = taskGroups\.filter\(\(g\) => g\.status === 'done'\)\.length/, 'completion details must derive their count from all planned groups')
  assert.match(agentMessage, /completedSteps=\{completedGroupCount\}/, 'completion details must use the all-planned-groups completion count')
  ...
  assert.match(homeSubmit, /setStreamingStatus\('thinking'\)[\s\S]*startInitialAgentTask\(id\)[\s\S]*router\.push\(`\/chat\/\$\{id\}`\)/, 'home submit must show genuine thinking and open the task stream before routing to the chat page')
  ...
  assert.match(globalsCss, /--accent-blue:\s*#0068b8/, 'light theme must expose the consumer-facing blue app accent')
  assert.match(globalsCss, /--status-live:\s*#0068b8/, 'light theme must expose a dedicated blue live-status token')
  assert.match(globalsCss, /--status-live:\s*#4c9cff/, 'dark theme must expose a dedicated blue live-status token')
  ...
  assert.match(panelHeader, /startup:\s*'Initializing new computer'/, 'computer panel startup copy must describe a new computer initialization')
  assert.match(panelHeader, /planning:\s*'Thinking'/, 'legacy planning state must render as Thinking rather than a third startup label')
  assert.match(typingIndicator, /Initializing new computer/, 'startup indicator must describe a new computer initialization')
  assert.match(typingIndicator, /planning:\s*'Thinking'/, 'legacy planning state must render as Thinking rather than a third startup label')
  assert.doesNotMatch(planManager, /progressUpdate\('Planning…'\)/, 'planning must remain internal and never become a visible startup status')
  assert.doesNotMatch(typingIndicator, /Creating plan|elapsedMs|setInterval/, 'startup copy must not advance on a timer before a plan event arrives')
  ...
  assert.match(useAgentStream, /setConversationStreaming\(conversationId, true\)[\s\S]*?postTaskStartWithStableRunId\(/, 'new tasks must enter visible running state and reach the stable-run task-start helper without waiting on cached client credit preflight')
  assert.match(useAgentStream, /async function postTaskStartWithStableRunId\([\s\S]*?fetch\('\/api\/chat'/, 'the stable-run task-start helper must post new tasks to the chat API')
  ...
  assert.match(useAgentStream, /if \(!existingController && storedActiveRun && !isAutoSend\) \{[\s\S]*const serverActiveRun = await fetchServerActiveRun\(conversationId, \{[\s\S]*runId: storedActiveRun\.runId,[\s\S]*\}\)[\s\S]*else if \(serverActiveRun === null\) \{[\s\S]*TASK_START_CONFIRMATION_GRACE_MS[\s\S]*rollbackRejectedTaskStart\(conversationId, storedActiveRun\.runId\)[\s\S]*storedActiveRun = null/, 'follow-up sends must verify the stored run id, preserve ambiguous starts through the confirmation grace, and roll back only a definitive miss before routing text')
  ...
  assert.match(userMessage, /max-w-\[94%\][\s\S]*break-words/, 'user message bubbles must remain viewport-bounded and wrap safely on narrow mobile screens')
  ...
  assert.match(chatServerSync, /REFRESH_INTERVAL_MS\s*=\s*5_000/, 'background account-task polling must discover changes made on another device promptly')
  ...
  assert.match(chatServerSync, /function mergeNewerSummaryMetadata[\s\S]*serverSummary: true,[\s\S]*serverBodyStale: true[\s\S]*mergeNewerSummaryMetadata\(existing, conversation, true\)/, 'a newer metadata summary must fence an older loaded body until the newer server body is fetched')
  assert.match(chatServerSync, /!isServerSummaryConversation\(conversation\)/, 'a stale summary-fenced body must never be uploaded')
  assert.match(chatServerSync, /conversation\.serverBodyStale !== true/, 'the stale-body upload fence must survive an accidentally removed summary marker')
  ...
  assert.match(conversationsLib, /if \(!currentRow\) \{[\s\S]*insert into conversations[\s\S]*on conflict\(user_id, id\) do nothing[\s\S]*\} else \{[\s\S]*update conversations[\s\S]*where user_id = \? and id = \? and revision = \?/, 'conversation sync must insert or revision-CAS-update individual task rows')
  assert.match(conversationsLib, /const incomingCanReplaceCurrent = !currentRow \|\| \([\s\S]*currentIsActive[\s\S]*baseRevisionMatches \|\| currentIsPlaceholder \|\| \([\s\S]*requestedUpdatedAtMs >= \(Number\.isFinite\(currentUpdatedAtMs\)/, 'older browser saves must not overwrite newer active task rows')
  assert.match(conversationsLib, /:\s*baseRevisionMatches && \([\s\S]*requestedUpdatedAtMs >= \(Number\.isFinite\(currentDeletedAtMs\)/, 'browser saves must match the current revision and be newer than the tombstone before resurrecting a deleted task row')
  ...
  assert.match(browseView, /This source did not return readable text/, 'browse panel must render a truthful empty body when a page extraction returns no text')
  ...
  assert.match(prompts, /Cover each requested item, field, or choice without requiring redundant calls/, 'action plans must cover every requested outcome without requiring redundant interactions')
  assert.match(planManager, /Cover each requested item, field, or choice/, 'website action runtime rules must preserve each requested target')
  assert.match(planManager, /Remaining controls do not require more work/, 'action phases must finish on sufficient evidence instead of exhausting page controls')
  ...
  assert.match(eventDispatcher, /if \(!this\.startupAcknowledgment\) \{[\s\S]*this\.pendingStartupPlanItems = \[\.\.\.items\][\s\S]*return/, 'the client must keep an early plan hidden until the model-authored acknowledgement is visible')
  assert.match(eventDispatcher, /if \(this\.pendingStartupPlanItems\) \{[\s\S]*this\.captureStartupAcknowledgment\(\)[\s\S]*this\.activatePlan\(pendingPlan\)/, 'acknowledgement text must release a held precomputed or replayed plan')
  ...
  assert.match(chatInput, /Add from local files/, 'chat input must expose the supported local-file attachment entry point')
  assert.match(fileHandling, /FILE_ACCEPT = ''/, 'the local-file picker must expose every file type')
  ...
  assert.match(llm, /ASSISTANT_SUPPORTS_IMAGE_INPUT = true/, 'the pinned DeepSeek route must advertise native image input')
  assert.match(llm, /ASSISTANT_SUPPORTS_VIDEO_INPUT = false/, 'the preview must not advertise unsupported video input')
  assert.match(llm, /ASSISTANT_SUPPORTS_FILE_INPUT = false/, 'the preview must not advertise unsupported file input')
  assert.match(llm, /ASSISTANT_SUPPORTS_AUDIO_INPUT = false/, 'the preview must not advertise unsupported audio input')
  ...
  assert.match(agentLoop, /nativeAttachmentContentPart/, 'agent loop must convert native attachments into provider content parts')
  ...
  assert.match(agentLoop, /function assistantSupportsNativeAttachment[\s\S]*ASSISTANT_SUPPORTS_IMAGE_INPUT[\s\S]*ASSISTANT_SUPPORTS_VIDEO_INPUT[\s\S]*ASSISTANT_SUPPORTS_AUDIO_INPUT[\s\S]*ASSISTANT_SUPPORTS_FILE_INPUT/, 'native attachment routing must match DeepSeek’s advertised modalities')
  ...
  assert.match(planManager, /PLANNER_CONTROL_REASONING = \{ effort: 'none' as const, exclude: true \}/, 'planner JSON control calls must use non-thinking mode')
  assert.match(planManager, /PLANNER_ACK_REASONING = \{ effort: 'none' as const, exclude: true \}/, 'startup acknowledgement must use non-thinking mode')
  ...
  assert.match(planManager, /Model acknowledgement repair was still unusable[\s\S]*this\.suppressFurtherAcknowledgementDeltas = true[\s\S]*return false/, 'unusable model acknowledgement drafts must fail without exposing a plan-only or locally fabricated opening')
  ...
  assert.match(agentConfig, /MODEL_MAX_PROMPT_TOKENS = 983_040[\s\S]*MODEL_MAX_COMPLETION_TOKENS = 65_536[\s\S]*MAX_CONTEXT_MESSAGES = 4_096/, 'Gemini must reserve its published output ceiling within the model context window')
  ...
  assert.doesNotMatch(tools, /youtube_transcript|YouTube Transcript/, 'removed YouTube transcript tooling must not be exposed to the model')
  assert.doesNotMatch(toolRegistry, /youtube_transcript|getYouTubeTranscript/, 'removed YouTube transcript tooling must not have an executor')
  ...
  assert.match(modelToolSchemas, /stripSchemaDescriptions/, 'tool schema compaction should remove repeated parameter descriptions')
  assert.match(modelToolSchemas, /same tools, fields, required arguments,[\s\S]*validation constraints/, 'tool schema compaction must preserve model autonomy and argument contracts')
  ...
  assert.doesNotMatch(toolPipeline, /finalSynthesisCarryoverBlockReason/, 'tool pipeline must not hard-block research or browser tools solely because the model is in the final phase')
  ...
  assert.match(eventDispatcher, /preserveVisibleSourceRecoveryPanel[\s\S]*event\.name !== 'read_document' &&[\s\S]*event\.name !== 'http_request' &&[\s\S]*event\.name !== 'web_search' &&[\s\S]*event\.name !== 'image_search'[\s\S]*streaming: false/, 'blocked source extraction and accepted unavailable searches must remain as truthful current Computer panel items instead of jumping backwards')
  assert.match(panelMapper, /internalRecovery[\s\S]*Source extraction unavailable/, 'internal source recovery must render a user-facing source state instead of raw internal text')
  assert.doesNotMatch(panelMapper, /Source needs browser rendering/, 'the UI must not claim browser rendering is required when the model can choose another extraction route')
  ...
  assert.match(chatTaskRunner, /chargeServerTokenUsage\(userId,\s*conversationId,\s*creditRunId,\s*creditUsage,\s*`attempt:\$\{billingAttempt\}:direct:\$\{attempt \+ 1\}`\)/, 'direct chat continuation calls must be charged with attempt-scoped distinct server ledger ids')
  assert.match(chatTaskRunner, /DIRECT_CHAT_MAX_TOKENS = MODEL_MAX_COMPLETION_TOKENS/, 'direct chat must have the model provider\'s full completion budget when the answer genuinely needs it')
  ...
  assert.match(agentLoop, /hasSavedFinalDeliverableCandidate\(state\)[\s\S]*finalOutputRetryDecision\(state\.consecutiveNullStreams,[\s\S]*hasSavedFinalDeliverable && state\.deliverableVerified[\s\S]*terminalReason = 'saved_deliverable_model_start_timeout'/, 'saved deliverable final revision timeouts must share a bounded decision and require verified output before completion')
  ...
  assert.match(agentLoop, /finalDecision === 'error'[\s\S]*The final file write could not start quickly enough/, 'final saved deliverable model-start timeouts must error instead of looping when no file exists')
  ...
  assert.match(chatTaskRunner, /DIRECT_CHAT_CONTINUATION_MAX_TOKENS = MODEL_MAX_COMPLETION_TOKENS/, 'direct chat continuations must have the full provider budget when a long answer genuinely continues')
  assert.match(chatTaskRunner, /DIRECT_CHAT_MAX_CONTEXT_MESSAGES = MAX_CONTEXT_MESSAGES[\s\S]*DIRECT_CHAT_MAX_CONTEXT_CHARS = Math\.floor\(MODEL_MAX_PROMPT_TOKENS \* 3\.5\)/, 'context-dependent direct chat must use the model context capacity rather than an eight-message window')
  assert.match(chatTaskRunner, /directChatNeedsConversationContext/, 'direct chat should avoid paying for history on standalone questions')
  assert.match(chatTaskRunner, /return cleanMessages\.slice\(-1\)/, 'standalone direct chat should send only the latest user message')
  assert.match(chatTaskRunner, /DIRECT_CHAT_CONTEXT_REFERENCE_PATTERN/, 'context-dependent direct-chat follow-ups must still preserve history')
  assert.match(chatTaskRunner, /directChatNeedsTemporalContext/, 'direct chat should only pay for temporal context on date/time questions')
  assert.match(chatTaskRunner, /includeTemporalContext:\s*directChatNeedsTemporalContext\(messages\)/, 'direct chat temporal context must be gated by the latest user request')
  ...
  assert.match(chatTaskRunner, /DIRECT_CHAT_IDENTITY_SYSTEM_PROMPT[\s\S]*You are Agent, a general AI agent[\s\S]*do not disclose the model\/provider/, 'direct chat must identify as a general AI agent and apply identity guidance')
  ...
  assert.doesNotMatch(chatTaskRunner, /latestUserAskedAgentIdentityDisclosure\(messages\)[\s\S]*emitter\.textDelta|AGENT_IDENTITY_DISCLOSURE_RESPONSE/, 'direct chat must not bypass provider generation with a fixed identity answer')
  ...
  assert.match(directChatRouting, /compare\|write\|draft\|compose/, 'short inline writing requests should not initialize a sandbox or become saved artifacts')
  assert.match(chatTaskRunner, /For a purely social acknowledgement, reply proportionately and naturally; do not list capabilities unless the user asks\./, 'lightweight social turns should not repeat a capability menu')
  assert.match(chatRoute, /const rawMessages = validation\.data\.messages[\s\S]*const safeRawMessages = stripPersistedAttachmentBodies\(rawMessages\)[\s\S]*const directChat = shouldUseDirectChat\(safeRawMessages\)[\s\S]*const scopedTaskMessages = scopeAgentTaskMessages\(safeRawMessages\)[\s\S]*const messagesPromise = useExternalWorker \|\| directChat \|\| !hasUnhydratedAttachments\(scopedTaskMessages\)/, 'chat route should use one Agent experience with lightweight conversational turns while scoping payloads before attachment hydration')
  assert.match(chatRoute, /const useExternalWorker = shouldUseExternalTaskWorker\(\) && !directChat/, 'conversational Agent turns must not wait for or consume the paid background worker')
  assert.match(chatRoute, /await timedRoutePromise\('taskAccessReadyMs', assertTaskAccess\([\s\S]*if \(access && !access\.ok\) return access\.response/, 'chat route must finish task access authorization before task-start preflights')
  assert.match(chatRoute, /const creditsPromise = assertServerCreditsAvailable\(userId\)[\s\S]*const workerAvailabilityPromise = directChat[\s\S]*timedRoutePromise\('workerReadyMs', taskWorkerUnavailableResponse\(\)\)[\s\S]*const attachmentAccessPromise = timedRoutePromise[\s\S]*Promise\.all\(\[[\s\S]*creditsPromise,[\s\S]*messagesPromise,[\s\S]*workerAvailabilityPromise,[\s\S]*attachmentAccessPromise,/, 'credit, hydration, conditional worker, and attachment-access preflights should settle in parallel after authorization')
  assert.match(chatRoute, /const conversationInsertPromise = timedRoutePromise\([\s\S]*prepareConversationForTaskStartInsert\([\s\S]*conversationInsertPromise,[\s\S]*await enqueueTaskJob\(\{[\s\S]*conversationInsert,[\s\S]*await startTaskJob\(\{[\s\S]*conversationInsert,/, 'conversation placeholder persistence must be reserved atomically with either external or in-process task creation')
  ...
  assert.match(chatTaskRunner, /if \(conversationId\) \{[\s\S]*chargeServerTaskStart/, 'shared task runner should use one metering path without deterministic task-start bypasses')
  assert.match(chatTaskRunner, /meteredTaskStarted[\s\S]*chargeActiveCredit/, 'active-time credit charging should only finalize for metered task starts')
  ...
  assert.match(llm, /DEFAULT_OPENROUTER_MODEL/, 'runtime must route through the centralized Gemini model')
  assert.match(llm, /ASSISTANT_PROVIDER\s*=\s*'openrouter'\s+as const/, 'runtime provider must be statically pinned to OpenRouter')
  assert.match(llm, /export const DEFAULT_MODEL = DEFAULT_OPENROUTER_MODEL/, 'stale model IDs must not change the pinned preview')
  assert.match(llm, /getAssistantApiKey[\s\S]*trimmedEnv\(process\.env\.OPENROUTER_API_KEY\)/, 'OpenRouter credentials must be trimmed')
  assert.doesNotMatch(llm, /process\.env\.DEEPSEEK_API_KEY|api\.deepseek\.com/, 'runtime must not retain the expired provider route')
  assert.match(llm, /ASSISTANT_REASONING_EFFORT = 'low'[\s\S]*reasoning: \{ effort: ASSISTANT_REASONING_EFFORT/, 'all lanes must use low enabled thinking')
  assert.match(llm, /stream_options: \{ include_usage: true \}/, 'OpenRouter streams must return token usage')
  assert.match(agentLoop, /reasoning_content: result\.reasoningContent/, 'assistant history must preserve thinking')
  assert.match(agentLoop, /reasoning_content: lastStreamResult\.reasoningContent/, 'tool history must preserve thinking')
  assert.match(llm, /estimateUsageCost/, 'token usage must be normalized into estimated provider cost')
  ...
  assert.match(eventDispatcher, /targetNarrations\.some\(narration => narration\.position === safePosition\)/, 'client must reject multiple narration blocks in the same action gap')
  ...
  assert.doesNotMatch(agentLoop, /Failed to record planner token usage; continuing task/, 'planner token ledger failures must fail closed instead of continuing unbilled')
  assert.doesNotMatch(agentLoop, /Failed to record iteration token usage; continuing task/, 'iteration token ledger failures must fail closed instead of continuing unbilled')
  assert.match(agentLoop, /isTransientFailure as isTransientAssistantStreamError[\s\S]*from '.\/ExecutionControl'/, 'transient assistant stream fetch failures must be classified for recovery')
  ...
  assert.match(serverCredits, /where user_id = \? and round\(monthly_balance, 0\) >= \?/, 'server credit charges must use a whole-number atomic DB non-overdraw condition')
  ...
  assert.match(activeTasks, /export async function getActiveTaskLeaseForConversation/, 'legacy fallback discovery must bind a lease to the requested conversation')
  assert.match(activeTasks, /export async function getActiveTaskLeaseForUser/, 'legacy user-level active-task discovery must remain available for compatibility callers')
  ...
  assert.match(taskJobs, /let pollInFlight = false[\s\S]*if \(streamClosed \|\| pollInFlight\) return[\s\S]*pollInFlight = true[\s\S]*finally\(\(\) => \{[\s\S]*pollInFlight = false/, 'persisted task event replay must not overlap Turso polls and duplicate the same seq events')
  ...
  assert.match(useAgentStream, /classifyStreamSequence\(event, runId, highestDispatchedSeq\)[\s\S]*highestDispatchedSeq = seq/, 'client stream consumer must advance only a contiguous persisted seq after dispatch')
  ...
  assert.match(taskWorker, /await Promise\.all\(\[startupWarmupPromise, ensureAgentRuntimePreloaded\(\)\]\)[\s\S]*sendHeartbeat\('idle', true\)/, 'worker must finish runtime preload and sandbox readiness before advertising itself idle')
  ...
  assert.match(chatTaskRunner, /getE2BSandboxBillingDescriptor\(conversationId\)[\s\S]*activateServerE2BRuntimeBilling\([\s\S]*startedAtMs: descriptor\.startedAtMs/, 'E2B runtime charge must durably bind the actual sandbox billing start and ownership generation')
  ...
  assert.match(planManager, /startAcknowledgementCall\(\): void \{[\s\S]*this\.scheduleAcknowledgementCall\(\)/, 'a precomputed plan must still start its single worker-owned model acknowledgement request')
  assert.match(agentLoop, /if \(startupPlanUsed\) \{[\s\S]*planManager\.startAcknowledgementCall\(\)[\s\S]*\} else \{[\s\S]*planManager\.startPlanCall\(\)/, 'precomputed and normal starts must choose exactly one model request path for their opening')
  assert.doesNotMatch(planManager, /acknowledgementAbortController|acknowledgementCallSuperseded|cancelRedundantAcknowledgementCall/, 'startup must not abort a billable acknowledgement stream before its terminal provider usage arrives')
  assert.match(planManager, /const sanitized = typeof ack === 'string'[\s\S]*isUsablePlannerAck\(sanitized[\s\S]*this\.emitter\.textDelta\(sanitized \+ '\\n\\n'\)[\s\S]*return true[\s\S]*repairAcknowledgementCandidate/, 'a valid planner acknowledgement must render immediately without a display wait or duplicate model call')
  assert.doesNotMatch(planManager, /PLANNER_START_AFTER_ACK_WAIT_MS/, 'planner startup must not wait behind acknowledgement display')
  assert.doesNotMatch(planManager, /ackPriority|PLANNER_ACK_PRIORITY_WINDOW_MS/, 'planner startup must not use an unbounded acknowledgement priority gate')
  ...
  assert.match(chatRoute, /;\[, messages, unavailableWorker,[^\]]*\] = await Promise\.all\(\[[\s\S]*creditsPromise,[\s\S]*messagesPromise,[\s\S]*workerAvailabilityPromise,[\s\S]*attachmentAccessPromise,/, 'credits, attachment hydration, worker readiness, and attachment ownership must run concurrently and all settle before task acceptance')
  assert.ok(chatRoute.indexOf('if (access && !access.ok)') < chatRoute.indexOf('enqueueTaskJob({'), 'ownership validation must complete before durable enqueue')
  ...
  assert.doesNotMatch(chatRoute, /createPrefacedTaskJobEventStream|taskStartPromise/, 'task acceptance must not be deferred into an already-open response stream')
  assert.match(chatRoute, /error instanceof TaskConversationConflictError[\s\S]*CONVERSATION_TASK_ALREADY_RUNNING[\s\S]*status: 409/, 'atomic conversation conflicts must be returned before opening SSE')
  ...
  assert.match(agentLoop, /if \(startupPlanUsed\) \{[\s\S]*planManager\.startAcknowledgementCall\(\)[\s\S]*\} else \{[\s\S]*planManager\.startPlanCall\(\)/, 'precomputed plans must start the worker-owned model acknowledgement without starting a duplicate planner')
  ...
  const usePrecomputedPlanContract = planManager.match(/usePrecomputedPlan\([\s\S]*?\n  private emitPlanAfterAcknowledgement/)?.[0] || ''
  assert.doesNotMatch(usePrecomputedPlanContract, /this\.acknowledgementEmitted = true/, 'using a precomputed route plan must not pretend the visible acknowledgement already painted')
  assert.match(planManager, /if \(this\.skipAcknowledgement\) return true[\s\S]*if \(this\.acknowledgementEmitted\) \{[\s\S]*suppressFurtherAcknowledgementDeltas = true[\s\S]*return true/, 'acknowledgement emission must suppress a duplicate if a model-authored opening is already visible')
  ...
  assert.match(chatRoute, /CONVERSATION_TASK_ALREADY_RUNNING[\s\S]*status:\s*409/, 'same-conversation concurrent starts must return a scoped conflict without restoring an account-wide task limit')
  ...
  assert.match(activeChatRoute, /getActiveTaskLeaseForConversation/, 'active-run discovery route must read the exact conversation lease from the server')
  assert.match(activeChatRoute, /getActiveTaskLeaseForConversation\(userId, conversationId\)/, 'active-run discovery route must bind lease lookup to the requested conversation')
  ...
  assert.match(useAgentStream, /const storedRecord = persistedCursorRecord \|\| getStoredActiveRun\(conversationId\)[\s\S]*const serverRecord = await fetchServerActiveRun\(conversationId, \{[\s\S]*\.\.\.options,[\s\S]*runId: storedRecord\?\.runId,[\s\S]*\}\)[\s\S]*serverRecord === undefined \? storedRecord : serverRecord/, 'resume must prefer authoritative server discovery for the persisted run id while retaining local cursor state on transient lookup failure')
  ...
  assert.match(modal, /role="dialog"[\s\S]*aria-modal="true"/, 'modal dialogs must expose dialog semantics')
  assert.match(modal, /getFocusableElements[\s\S]*event\.key !== 'Tab'[\s\S]*initialFocus\?\.focus/, 'modal dialogs must trap keyboard focus and establish an accessible initial focus target')
  assert.match(modal, /previouslyFocused[\s\S]*previouslyFocused\.focus/, 'modal dialogs must restore focus when they close')
  const conversationSearchSource = await readFile(join(root, 'src/components/chat/ConversationSearch.tsx'), 'utf8')
  const commandPaletteSource = await readFile(join(root, 'src/components/ui/CommandPalette.tsx'), 'utf8')
  assert.match(conversationSearchSource, /inputRef\.current\?\.focus[\s\S]*previouslyFocused\.focus[\s\S]*role="search"/, 'task search must focus its query field, restore the prior focus on close, and expose search semantics')
  assert.match(commandPaletteSource, /inputRef\.current\?\.focus[\s\S]*previouslyFocused\.focus[\s\S]*role="dialog"/, 'command palette must focus search, restore the prior focus on close, and expose dialog semantics')
  ...
  assert.match(chatTaskRunner, /for \(let attempt = 0; attempt <= DIRECT_CHAT_MAX_CONTINUATIONS; attempt\+\+\) \{[\s\S]*await assertServerCreditsAvailable\(userId\)[\s\S]*createCompletion/, 'direct chat must preflight credit runway before every provider call')
  assert.match(chatTaskRunner, /retryMaxAttempts:\s*0[\s\S]*isAssistantRequestTimeout\(error\)[\s\S]*REQUEST RECOVERY:[\s\S]*requestTimeoutMs:\s*60_000/, 'a direct answer timeout must retry once through a fresh provider route instead of immediately failing the task')
  ...
  assert.match(planManager, /await this\.assertCreditRunway\('ack'\)[\s\S]*createStreamingCompletion/, 'planner acknowledgements must preflight before their streaming provider call')
  ...
  assert.doesNotMatch(policyEngine, /LOCAL VISUAL QA REQUIRED/, 'website QA must not force local visual preview or fixed viewports')
  ...
  assert.match(policyEngine, /reviewBrowserPhaseCompletion\([\s\S]*state\.lastBrowserObservation/, 'model browser advancement must be evaluated against retained page evidence')
  ...
  assert.match(prompts, /choose the structure that best serves the user's subject, audience, and requested depth[\s\S]*Do not force a universal section sequence/, 'research report deliverables must receive flexible professional-structure guidance')
  assert.doesNotMatch(prompts, /must use inline bracket citations|always use inline bracket citations/, 'research report deliverables must not force one citation style on every task')
  ...
  assert.match(taskStrategy, /Create a coherent professional report[\s\S]*Choose its title, organization, prose\/bullets\/tables, synthesis, conclusions, and source presentation[\s\S]*rather than a fixed template/, 'research strategy must guide professional reports without imposing one fixed skeleton')
  assert.match(await readFile(join(root, 'src/agent/guards/stepMessages.ts'), 'utf8'), /choose a coherent professional Markdown structure from the actual topic[\s\S]*do not force a universal sequence or citation style/, 'final research step guidance must preserve agent judgement inside a professional report shape')
  ...
  assert.doesNotMatch(outputVerifier, /researchMinimumWords/, 'research verifier must not impose a runtime-authored report length')
  assert.match(outputVerifier, /explicitWordTarget\(originalRequest\)/, 'research verifier must enforce a word target only when the user authored one')
  assert.match(outputVerifier, /explicitlyRequestsOpeningSynthesis\(originalRequest\)/, 'report verification must require an opening synthesis only when the user requests one')
  ...
  assert.match(outputVerifier, /savedMarkdownReport[\s\S]*taskDefaultsToMarkdownDeliverable\(originalRequest\)/, 'saved markdown reports must retain request-aware structural verification')
  assert.match(outputVerifier, /brief\|quick\|short\|concise/, 'research verifier must respect explicitly concise report requests')
  assert.match(agentLoop, /function shouldUseNaturalCadenceNarration[\s\S]*Final phases may still select tools[\s\S]*return true/, 'final deliverable work must remain eligible for non-blocking cadence narration without entering narration-only recovery')
  ...
  assert.match(agentLoop, /const malformedToolResults = lastToolResults\.filter\(isMalformedToolArgumentsRecovery\)[\s\S]*TOOL JSON RECOVERY/, 'malformed streamed tool args must trigger an immediate retry nudge after preserving any valid sibling results')
  ...
  assert.match(agentLoop, /FINAL_SAVED_DELIVERABLE_INITIAL_MAX_TOKENS = 8_192[\s\S]*FINAL_SAVED_DELIVERABLE_WRITE_MAX_CHARS = 24_000/, 'initial markdown deliverables must use a bounded native write chunk while preserving continuation capacity')
  ...
  assert.match(agentLoop, /FAST_SOURCE_ACTION_REQUEST_TIMEOUT_MS = 45_000/, 'source-action starts must tolerate routed provider startup')
  ...
  assert.match(agentLoop, /let fastActionTurn = \(!allowPhaseDecision \|\| isFastSourceActionToolTurn\(state, this\.options\.messages\)\) && activeTools\.length > 0 &&[\s\S]*!isPostCompletion &&[\s\S]*isFastActionToolTurn\(state,\s*this\.options\.messages\)/, 'active tool-selection turns must enter the fast lowest-reasoning lane by default')
  ...
  assert.match(agentLoop, /allowParallelSourceToolCalls[\s\S]*!sourceExtractionBatchConsumedForLatestSearch\(state\)/, 'only one model-selected parallel source batch may be taken from the latest search set')
  assert.match(toolPipeline, /MAX_PARALLEL_SOURCE_EXTRACTIONS = 3/, 'one model-selected source batch must stay within three actions')
  assert.match(toolPipeline, /PARALLEL_SOURCE_EXTRACTION_TOOLS[\s\S]*read_document[\s\S]*http_request/, 'only stateless source extraction tools should be eligible for parallel execution')
  ...
  assert.doesNotMatch(toolPipeline, /autoSourceExtractionBatchFromSearch|executeAutoSourceExtractionBatch|AUTO SOURCE EXTRACTION RESULTS/, 'the runtime must leave source selection to the agent instead of automatically mining a search result set')
  assert.match(toolPipeline, /Selecting a source batch is the decision boundary[\s\S]*stepLastSourceExtractionSearchCount = state\.stepSearchQueries\.size[\s\S]*Promise\.all\(executionBatch\.map/, 'a model-selected search-result batch must be consumed before reads execute so blocked or cached calls cannot reopen it')
  ...
  assert.match(agentLoop, /fastActionTurn[\s\S]*FAST_ACTION_REASONING/, 'between-tool action turns must use the shared bounded-reasoning fast lane')
  ...
  assert.match(prompts, /Do not narrate with fewer than 3 new visible actions except at phase completion, and never go past 3 visible actions/, 'runtime prompt must enforce a maximum three-action narration gap')
  ...
  assert.match(prompts, /Lead with a fact-dense outcome from the newest completed work and continue naturally from what the user has already seen/, 'runtime prompt must require outcome-first narration with natural continuity')
  assert.match(prompts, /When uncertainty, source disagreement, or an evidence gap materially changes the result, say so plainly/, 'runtime prompt must preserve material uncertainty and disagreement')
  assert.match(prompts, /Add an immediate next direction only when work is continuing and that direction helps orient the user/, 'runtime prompt must make the direction conditional rather than formulaic')
  assert.match(prompts, /direct factual subjects, first-person confirmations, and concise review or finding leads are all valid/, 'runtime prompt must allow varied natural narration syntax')
  assert.match(prompts, /A concrete source-action lead is also valid when it immediately states the extracted result or material provenance/, 'runtime prompt must allow concrete source-action leads')
  assert.doesNotMatch(prompts, /default to a clean, slightly fuller two-sentence paragraph|normally end with a short "Next, I'll|Manus-style progress paragraph/, 'runtime prompt must not turn style examples into canned wording')
  ...
  assert.match(prompts, /Cover each requested item, field, or choice without requiring redundant calls/, 'action planner must preserve target coverage without prescribing a phase per interaction')
  ...
  assert.match(agentLoop, /state\.visibleToolActionsSinceLastNarration < Math\.min/, 'forced narration must be based on visible action pills, not raw iterations')
  ...
  assert.match(modelToolSchemas, /Visible action title: verb \+ concrete subject \+ intended result/, 'compact model tool schemas must retain the action-title specificity contract')
  ...
  assert.doesNotMatch(streamProcessor, /defaultFileActionLabel|fileActionLabelFallback|args\.action_label\s*=\s*runtimeVisibleActionLabel/, 'provisional streaming must wait for the model-authored action label instead of deriving one from a file path')
  assert.doesNotMatch(activityDescriber, /defaultFileActionLabel/, 'the activity layer must not retain a generic path-derived file title fallback')
  assert.doesNotMatch(toolPipeline, /repairFileActionLabel/, 'file actions missing model-authored titles must return to model repair instead of receiving runtime wording')
  ...
  assert.match(toolCache, /DISPLAY_ONLY_ARG_KEYS = new Set\(\['action_label', 'plan_step_index', 'progress_update'\]\)/, 'tool cache keys must ignore display-only metadata')
  ...
  assert.match(toolCache, /toolName === 'read_document'[\s\S]*const target = typeof sortedArgs\.url[\s\S]*delete sortedArgs\.source/, 'read_document cache keys must canonicalize legacy source targets into url')
  assert.doesNotMatch(toolCache, /youtube_transcript/, 'removed YouTube transcript tooling must not retain a cache policy')
  ...
  assert.match(toolCache, /SENSITIVE_HTTP_HEADER_NAME = \/\(\?:authorization[\s\S]*cookie[\s\S]*credential[\s\S]*if \(SENSITIVE_HTTP_HEADER_NAME\.test\(key\)\) return null/, 'credentialed http_request calls must not be cached')
  ...
  assert.match(toolPipeline, /A cache hit replays evidence already counted[\s\S]*if \(RESEARCH_TOOLS\.has\(tc\.name\)\) \{[\s\S]*state\.suppressedResearchToolName = tc\.name/, 'cached browser reads must not inflate research-call depth or hide duplicate-source loops')
  ...
  assert.match(agentConfig, /inactivityTimeoutMs:\s*IS_OLLAMA \? 120_000 : 8_000/, 'invisible model stalls should tolerate routed-provider pauses while remaining bounded')
  ...
  assert.match(workingMemory, /this\.priority\(b, opts\?\.stepIdx\)/, 'working memory should prioritize relevant current-step facts when compacting')
  ...
  assert.match(browserView, /hasPageError =[\s\S]*!hasCurrentLiveEvidence[\s\S]*!\(safeResult\.recoverable && hasVisualFrame\)[\s\S]*safeResult\.success === false[\s\S]*isPageLevelBrowserError/, 'browser panel must not show stale or recoverable page errors over current visual frames')
  ...
  assert.match(prompts, /Progress narration is required every 3 completed visible action pills/, 'narration cadence must be based on visible user-facing action progress')
  assert.match(agentLoop, /shouldUseNaturalCadenceNarration[\s\S]*state\.visibleToolActionsSinceLastNarration < Math\.min/, 'narration cadence must use the central visible-action frontier')
  assert.match(agentLoop, /hardWindowOpen =[\s\S]*NARRATION_MAX_VISIBLE_ACTION_GAP[\s\S]*!hardWindowOpen/, 'internal guard turns must not extend model narration beyond the fourth visible action')
  ...
  assert.match(agentConfig, /NARRATION_MAX_VISIBLE_ACTION_GAP\s*=\s*3/, 'no more than three actions may run without narration')
  assert.match(agentLoop, /cadenceNarrationMainTurnGuidance[\s\S]*do not use an operation[\s\S]*as the outcome[\s\S]*source-action lead is valid[\s\S]*The current tool has not returned yet/, 'native narration must summarize preceding results without banning concrete source leads or speculating about the current action')
  ...
  assert.match(agentLoop, /Write progress_update as the FIRST argument[\s\S]*It is display-only[\s\S]*make the tool call/, 'overdue narration must precede long file arguments on the same action request')
  ...
  assert.match(agentLoop, /const cadenceNarrationInMainTurn =[\s\S]*shouldUseNaturalCadenceNarration\(state\)[\s\S]*beginNarrationCadenceAttempt\(state\)/, 'cadence narration must arm on the next ordinary action request')
  assert.doesNotMatch(agentLoop, /launchNarrationSidecarIfDue|NARRATION_SIDECAR_REQUEST_TIMEOUT_MS|LLM narration sidecar failed/, 'narration must not open a competing provider sidecar')
  ...
  assert.match(agentState, /isLastStep \|\| isCurrentSynthesisStep\(state\)[\s\S]*state\.currentPhase = 'deliver'/, 'non-final synthesis/write steps must switch to deliver phase instead of compact research')
  assert.match(agentLoop, /shouldUseCompactResearchTurn[\s\S]*isCurrentSynthesisStep\(state\)[\s\S]*return false/, 'compact research turns must not run on synthesis-only steps')
  ...
  assert.match(agentLoop, /pendingActionSelectionRepairPrompt[\s\S]*ACTION SELECTION REPAIR[\s\S]*const actionSelectionRepairPrompt = pendingActionSelectionRepairPrompt[\s\S]*pendingActionSelectionRepairPrompt = null[\s\S]*this\.callLLMWithRetry\([\s\S]*actionSelectionRepairPrompt/, 'a paid no-progress repair must be passed once into the exact next model request')
  assert.match(agentLoop, /actionSelectionRepairPrompt: string \| null = null[\s\S]*if \(actionSelectionRepairPrompt\)[\s\S]*role: 'system',[\s\S]*content: actionSelectionRepairPrompt/, 'compact model requests must preserve the one-turn action-selection repair instruction')
  assert.doesNotMatch(agentLoop, /ACTION SELECTION REPAIR[\s\S]{0,900}contextManager\.push/, 'action-selection repair must not rely on raw context that compact research requests replace')
  ...
  assert.doesNotMatch(toolPipeline, /synthesisPhaseResearchReason/, 'synthesis steps must remain free to make a targeted evidence call when the model identifies a real gap')
  assert.doesNotMatch(toolPipeline, /isFinalDeliverableStep[\s\S]*!DELIVERABLE_STEP_TOOLS\.has/, 'final steps must not use a static tool allowlist that blocks task-aware direction changes')
  ...
  assert.match(agentLoop, /PRESENTATION_REASONING = \{ effort: 'none' as const, exclude: true \}/, 'acknowledgement, narration, and handoff turns must use non-thinking mode')
  assert.match(agentLoop, /FAST_ACTION_REASONING = \{ effort: 'low' as const, exclude: true \}/, 'fast action selection must use the lowest tool reasoning mode')
  assert.match(agentLoop, /TASK_REASONING = \{ effort: 'low' as const, exclude: true \}/, 'ordinary tool-capable turns must use the lowest reasoning mode')
  assert.match(agentLoop, /DEEP_TASK_REASONING = \{ effort: 'low' as const, exclude: true \}/, 'deep tool-capable turns must use low reasoning without reducing output or iteration budgets')
  ...
  assert.match(agentLoop, /function shouldUseNaturalCadenceNarration[\s\S]*visibleToolActionsSinceLastNarration < Math\.min[\s\S]*Final phases may still select tools[\s\S]*return true/, '3-action cadence narration should open immediately after the visible action frontier, including saved deliverable work')
  ...
  assert.match(agentLoop, /function shouldUseNaturalCadenceNarration[\s\S]*state\.exactExtractionGuardPending[\s\S]*!hardWindowOpen/, 'exact extraction guard must be evaluated before compact cadence narration')
  ...
  assert.doesNotMatch(toolRegistry, /args\.path \|\| ['"]output\.md['"]/, 'file tools must reject missing paths instead of silently naming reports output.md')
  ...
  assert.match(agentLoop, /explicitlyRequestsShortLiteralAnswer[\s\S]*reply\|respond\|answer\|output\|return\|say\|write[\s\S]*exactly\|only[\s\S]*return explicitlyRequestsShortLiteralAnswer/, 'an explicitly requested exact or literal short reply should complete on its first final turn')
  assert.match(policyEngine, /explicitlyRequestsShortLiteralAnswer[\s\S]*reply\|respond\|answer\|output\|return\|say\|write[\s\S]*exactly\|only[\s\S]*return explicitlyRequestsShortLiteralAnswer/, 'policy completion must not loop on an explicitly requested short literal reply')
  assert.match(completionAudit, /if \(requiresFinalDeliverable\(state\) && !hasFinalDeliverable\(state\)\)/, 'completion audit must require a saved artifact whenever the task contract calls for one')
  ...
  assert.match(agentLoop, /const FINAL_INLINE_ANSWER_REQUEST_TIMEOUT_MS = 60_000/, 'final inline answers must tolerate routed provider startup')
  assert.match(agentLoop, /const FINAL_SAVED_DELIVERABLE_MAX_TOKENS = 6_144[\s\S]*boundFileWriteChunksForModel/, 'saved final deliverables must bound each native JSON write envelope and continue coherently through file chunks')
  ...
  assert.match(agentLoop, /shouldAutosaveTextOnlyDraft[\s\S]*hasSavedFinalDeliverableCandidate\(state\)\) return false/, 'a final confirmation must not be autosaved as a second deliverable after the requested file exists')
  assert.match(agentLoop, /Existing saved deliverable verified without auto-closing the final phase/, 'a valid saved artifact must remain inside the model-authored final phase until its full outcome is complete')
  assert.match(agentLoop, /readFileInSandbox\(conversationId, existingDeliverablePath\)[\s\S]*outputVerifier\.verify[\s\S]*continueFinalPhaseAfterVerifiedArtifact/, 'the final text boundary must read back and verify the real saved file without assuming one file finishes every possible outcome')
  ...
  assert.match(agentLoop, /if \(requestLooksBrief \|\| compactNamedDataArtifact\) \{[\s\S]*return 50/, 'small explicitly concise or field-scoped saved artifacts must not trigger an unnecessary append solely to hit a generic report-length floor')
  assert.match(agentLoop, /final\\s\+\(\?:response\|reply\|answer\|message\|handoff\)[\s\S]*compactNamedDataArtifact/, 'brief final-handoff wording must be removed before deciding saved-artifact length')
  assert.match(agentLoop, /function savedDeliverableChunkEndsCleanly[\s\S]*\^\(\?:\[-\*\+\]\|\\d\+\\\.\)\\s\+\\S/, 'a complete Markdown list item must count as a clean concise deliverable ending')
  assert.match(agentLoop, /function finalSavedDeliverableTurn[\s\S]*isFinalDeliveryStep\(state\)[\s\S]*taskNeedsSavedFinalArtifact\(state, messages\)/, 'browser-led tasks with explicit file output must enter the final saved-deliverable lane')
  assert.match(agentLoop, /function shouldUseTextSavedFinalDeliverable[\s\S]*return false/, 'ignored file-tool turns must retry a native model-authored file call instead of entering a runtime-named save fallback')
  ...
  assert.match(agentLoop, /FINAL_SAVED_DELIVERABLE_REQUEST_TIMEOUT_MS = 60_000/, 'saved final deliverables must have a bounded startup window that tolerates provider variance')
  assert.match(agentLoop, /FINAL_SAVED_DELIVERABLE_ITERATION_TIMEOUT_MS = 120_000/, 'saved final deliverables must allow provider-buffered native file arguments to finish')
  assert.match(agentLoop, /FINAL_SAVED_DELIVERABLE_INACTIVITY_TIMEOUT_MS = 60_000/, 'provider-buffered report generation must not be repeatedly cancelled at fifteen seconds')
  assert.match(agentLoop, /finalSavedDeliverableTurn\(state, messages\)[\s\S]*iterationTimeoutMs: Math\.max\(state\.tierTimeouts\.iterationTimeoutMs, FINAL_SAVED_DELIVERABLE_ITERATION_TIMEOUT_MS\)[\s\S]*inactivityTimeoutMs: Math\.max\(state\.tierTimeouts\.inactivityTimeoutMs, FINAL_SAVED_DELIVERABLE_INACTIVITY_TIMEOUT_MS\)/, 'saved final-deliverable timeouts must be minimum allowances instead of shrinking to generic research timeouts')
  assert.match(agentLoop, /if \(deliverableResult && recoveredPartialWrite\)[\s\S]*deliverableContentForVerification\(conversationId, deliverableResult\)[\s\S]*deliverableVerificationAccepted\(state, verification\)[\s\S]*Recovered deliverable passed whole-file verification/, 'every recovered final-file stream must be verified immediately so a complete report is not forced into another append')
  ...
  assert.match(agentLoop, /FINAL SAVED DELIVERABLE TOOL CALL ONLY[\s\S]*Write one coherent complete deliverable in the first create_file call whenever it fits/, 'saved final deliverables must push one coherent first file write')
  ...
  assert.match(agentLoop, /finalDecision === 'error'[\s\S]*The final file write took too long to start/, 'nudgeable final saved deliverable timeouts must error instead of resetting forever when no file exists')
  ...
  assert.match(agentState, /call\.name === 'read_document'[\s\S]*const target = parsed\.url \|\| parsed\.source[\s\S]*read_document:\$\{String\(target\)\.toLowerCase\(\)\.trim\(\)\}/, 'read_document loop detection must use canonical url targets while preserving legacy source compatibility')
...
export async function runLedgerSmoke() {
  ...
  assert.ok(buildPrompt.includes('one complete create_website action'), 'build prompts must require the reliable bundled website path')
  assert.ok(buildPrompt.includes('editable source set') && buildPrompt.includes('previewable index.html'), 'build prompts must preserve separate website sources and a clickable bundle')
  ...
  assert.equal(
    sanitizeNarrationText("gather more details from the page to confirm the movie she plays in.The embedded media page appears to be a short clip featuring a woman, but I couldn't extract its title or description.", { requireSignal: true }),
    null,
  )
  assert.equal(
  ...
  assert.equal(
    sanitizeNarrationText('Found the embedded media page did not expose a title or description from that URL.The source still confirms it was a media page.', { requireSignal: true }),
  ...
  assert.match(finalStepMessage, /Fill a genuinely critical evidence gap if necessary, but do not restart broad research/, 'final step guidance must prevent broad carryover without blocking a task-aware evidence repair')
  ...
  assert.ok(
    !cadenceActions.some((action) =>
      action.type === 'terminate' ||
      action.message?.content?.includes('NARRATION CADENCE') ||
      action.message?.content?.includes('previous progress update was not valid'),
    ),
    'policy evaluation must never block ordinary task work on narration generation or repair',
  )

  const phaseAdvanceWithoutNarrationState = createInitialState(false, timeouts)
  phaseAdvanceWithoutNarrationState.taskStrategy = 'analysis'
  phaseAdvanceWithoutNarrationState.currentPhase = 'analysis'
  phaseAdvanceWithoutNarrationState.currentPlanItems = ['Analyze collected evidence', 'Write final answer']
  phaseAdvanceWithoutNarrationState.currentStepIdx = 0
  phaseAdvanceWithoutNarrationState.visibleToolActionsSinceLastNarration = 4
  phaseAdvanceWithoutNarrationState.stepToolCallCount = 4
  phaseAdvanceWithoutNarrationState.stepIterationCount = 4
  const phaseAdvanceWithoutNarrationActions = policy.evaluate(
    phaseAdvanceWithoutNarrationState,
    new Map(),
    '',
    true,
    30,
  ...
  const overdueNarrationState = createInitialState(false, timeouts)
  overdueNarrationState.currentPlanItems = ['Gather source evidence', 'Write final answer']
  overdueNarrationState.currentStepIdx = 0
  overdueNarrationState.visibleToolActionsSinceLastNarration = 4
  const overdueNarrationActions = policy.evaluate(
    overdueNarrationState,
  ...
  assert.ok(
    !overdueNarrationActions.some((action) => action.message?.content?.includes('NARRATION REQUIRED BEFORE NEXT ACTION')),
    'overdue narration must not block a useful tool call while the native progress field is due',
  )
  assert.equal(overdueNarrationState.forceTextNextIteration, false, 'cadence narration must not turn into a blocking model retry')
  ...
  buildState.currentPlanItems = ['Design the research findings website', 'Verify the bundled preview']
  buildState.currentPlanScopes = ['Render the gathered research evidence in a responsive summary layout.', 'Inspect the finished local website.']
  buildState.currentStepIdx = 0
  updatePhase(buildState)
  assert.equal(buildState.currentPhase, 'build', 'an explicit build title must win over research words in its scope')
  assert.equal(isConcreteBuildStep(buildState), true)
  buildState.currentPlanItems = ['Scaffold Next.js project with Tailwind CSS and dependencies', 'Gather bird image assets']
  buildState.currentPlanScopes = ['Create package.json, app files, and Tailwind setup.', 'Search for source images only.']
  ...
  assert.ok(JSON.stringify(deliverableRevisionResults[0]?.result || {}).includes('Use edit_file for structural/content repairs'))
