...
async function assertSourceContracts() {
  ...
  assert.match(agentLoop, /uploadedAttachments\(messages\)\.filter\(\(attachment\) => !\([\s\S]*attachment\.contentEncoding === 'data-url'[\s\S]*assistantSupportsNativeAttachment\(attachment\)/, 'only Gemini-supported native uploads may bypass the visible attachment preflight step')
  ...
  assert.match(planManager, /PLANNER_CONTROL_REASONING = \{ effort: 'low' as const, exclude: true \}/, 'planner JSON control calls must use Gemini low reasoning')
  assert.match(planManager, /PLANNER_ACK_REASONING = \{ effort: 'minimal' as const, exclude: true \}/, 'startup acknowledgement must use Gemini minimal reasoning')
  ...
  assert.doesNotMatch(llm, /process\.env\.OPENROUTER_REASONING_EFFORT|process\.env\.OPENROUTER_REASONING_EXCLUDE/, 'stale environment variables must not override Gemini per-turn reasoning levels')
  ...
  assert.match(llm, /function providerReasoningPayload[\s\S]*requestedEffort === 'xhigh'[\s\S]*requestedEffort === 'none'[\s\S]*_reasoning\?\.enabled === false[\s\S]*reasoning:\s*\{[\s\S]*effort,[\s\S]*exclude:\s*true/, 'Gemini calls must use supported native reasoning levels, hide internal traces, and keep mandatory reasoning enabled')
  assert.doesNotMatch(llm, /forcedToolChoice/, 'Gemini must retain reasoning for provider-compatible required and named tool choices')
  ...
  assert.match(agentLoop, /PRESENTATION_REASONING = \{ effort: 'minimal' as const, exclude: true \}/, 'acknowledgement, narration, and handoff turns must use Gemini minimal reasoning')
  assert.match(agentLoop, /FAST_ACTION_REASONING = \{ effort: 'minimal' as const, exclude: true \}/, 'Gemini action selection must use minimal reasoning')
  assert.match(agentLoop, /TASK_REASONING = \{ effort: 'low' as const, exclude: true \}/, 'ordinary Gemini task turns must receive low reasoning')
  assert.match(agentLoop, /DEEP_TASK_REASONING = \{ effort: 'medium' as const, exclude: true \}/, 'final synthesis and build turns must receive Gemini medium reasoning')
  ...
  assert.match(agentLoop, /const requestReasoning = useCompactNarration[\s\S]*PRESENTATION_REASONING[\s\S]*isFinalInlineAnswerTurn[\s\S]*DEEP_TASK_REASONING[\s\S]*fastActionTurn[\s\S]*FAST_ACTION_REASONING[\s\S]*TASK_REASONING/, 'Gemini reasoning must adapt by turn type without slowing visible narration')
