...
async function assertSourceContracts() {
  ...
  assert.match(planManager, /PLANNER_ACK_MAX_TOKENS = 256/, 'planner acknowledgement repair must leave room for hidden medium reasoning while keeping startup generation bounded')
  ...
  assert.match(normalPlannerStartup, /plannerDeadlineAtMs = Date\.now\(\) \+ PLANNER_OVERALL_DEADLINE_MS[\s\S]*this\.planPromise = start\(\)[\s\S]*this\.attemptPlanCall\(0,\s*true\)/, 'normal startup must use the planner response for both its model-authored acknowledgement and plan')
  assert.doesNotMatch(normalPlannerStartup, /scheduleAcknowledgementCall|emitModelGeneratedAcknowledgement/, 'normal startup must not launch a second billable acknowledgement stream whose terminal usage could be lost on cancellation')
  ...
  assert.match(planManager, /startPlanCall\(\): void \{[\s\S]*attemptPlanCall\(0,\s*true\)/, 'normal startup must let one model planner response supply both acknowledgement and plan without a display race')
  ...
  assert.match(planManager, /PLANNER_ACK_REASONING = \{ effort: 'medium' as const, exclude: true \}/, 'startup acknowledgement must use medium reasoning')
  ...
  assert.match(llm, /ASSISTANT_REASONING_EFFORT = 'medium'[\s\S]*function providerReasoningPayload[\s\S]*effort:\s*ASSISTANT_REASONING_EFFORT[\s\S]*exclude:\s*true/, 'GLM calls must pin reasoning to medium and hide internal traces')
