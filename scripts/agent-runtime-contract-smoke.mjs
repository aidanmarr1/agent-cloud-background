...
async function assertSourceContracts() {
  ...
  assert.doesNotMatch(chatRoute, /taskStartPromise = workerStartupPlanPromise\.then|workerStartupPlanPromise[\s\S]*enqueueTaskJob/, 'external-worker route must not wait for startup planning before enqueueing')
  assert.match(chatRoute, /taskStartPromise = Promise\.resolve\(\)\.then\(\(\) => \{[\s\S]*return enqueueTaskJob\([\s\S]*payload: taskPayload/, 'external-worker route must enqueue the durable job immediately and attach the visible route plan later')
  ...
  assert.match(chatRoute, /ROUTE_STARTUP_PLAN_WORKER_HANDOFF_WAIT_MS = 5_500/, 'worker startup-plan handoff must not miss a route plan because of a narrow preface window')
  assert.match(chatRoute, /await taskStartPromise[\s\S]*await attachTaskJobStartupPlan\(creditRunId,\s*plan\)[\s\S]*return \[\{ type: 'plan'/, 'route plan must be persisted into the queued job before the visible plan event is emitted')
