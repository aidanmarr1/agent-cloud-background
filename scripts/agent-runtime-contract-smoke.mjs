...
async function assertSourceContracts() {
  ...
  assert.match(chatServerSync, /REFRESH_INTERVAL_MS\s*=\s*30_000/, 'background account-task polling must stay slow enough not to contend with active task persistence')
  ...
  assert.match(llm, /ASSISTANT_SUPPORTS_IMAGE_INPUT = true/, 'the pinned Qwen route must advertise native image input')
  assert.match(llm, /ASSISTANT_SUPPORTS_VIDEO_INPUT = true/, 'the pinned Qwen route must advertise native video input')
  assert.match(llm, /ASSISTANT_SUPPORTS_FILE_INPUT = false/, 'PDFs must be inspected through the sandbox because Qwen does not advertise native file input')
  assert.match(llm, /ASSISTANT_SUPPORTS_AUDIO_INPUT = false/, 'audio must be inspected through the sandbox because Qwen does not advertise native audio input')
  ...
  assert.match(agentLoop, /function assistantSupportsNativeAttachment[\s\S]*ASSISTANT_SUPPORTS_IMAGE_INPUT[\s\S]*ASSISTANT_SUPPORTS_VIDEO_INPUT[\s\S]*ASSISTANT_SUPPORTS_AUDIO_INPUT[\s\S]*ASSISTANT_SUPPORTS_FILE_INPUT/, 'native attachment routing must match Qwen’s advertised modalities')
  assert.match(agentLoop, /uploadedAttachments\(messages\)\.filter\(\(attachment\) => !\([\s\S]*attachment\.contentEncoding === 'data-url'[\s\S]*assistantSupportsNativeAttachment\(attachment\)/, 'only Qwen-supported native uploads may bypass the visible attachment preflight step')
  ...
  assert.match(planManager, /PLANNER_CONTROL_REASONING = \{ max_tokens: 256, exclude: true \}/, 'planner JSON control calls must use a bounded Qwen reasoning budget')
  assert.match(planManager, /PLANNER_ACK_REASONING = \{ enabled: false, exclude: true \}/, 'startup acknowledgement must avoid hidden reasoning latency')
  ...
  assert.match(agentConfig, /MODEL_MAX_PROMPT_TOKENS = 934_464[\s\S]*MODEL_MAX_COMPLETION_TOKENS = 65_536[\s\S]*MAX_CONTEXT_MESSAGES = 4_096/, 'Qwen3.7 Flash must use its published context and completion capacity instead of an eight-message cap')
  ...
  assert.match(chatRoute, /const rawMessages = validation\.data\.messages[\s\S]*const directChat = shouldUseDirectChat\(rawMessages\)[\s\S]*const scopedTaskMessages = scopeAgentTaskMessages\(safeRawMessages\)[\s\S]*const messagesPromise = useExternalWorker \|\| directChat \|\| !hasUnhydratedAttachments\(scopedTaskMessages\)/, 'chat route should scope payloads and decide routing before attachment hydration')
  ...
  assert.match(chatRoute, /const creditsPromise = assertServerCreditsAvailable\(userId\)[\s\S]*const workerAvailabilityPromise = timedRoutePromise[\s\S]*const attachmentAccessPromise = timedRoutePromise[\s\S]*Promise\.all\(\[[\s\S]*creditsPromise,[\s\S]*messagesPromise,[\s\S]*workerAvailabilityPromise,[\s\S]*attachmentAccessPromise,/, 'credit, hydration, worker, and attachment-access preflights should settle in parallel after authorization')
  ...
  assert.doesNotMatch(llm, /PINNED_OPENROUTER_PROVIDER|exactOpenRouterProviderRoute/, 'runtime must keep OpenRouter balanced price-and-speed routing')
  assert.doesNotMatch(llm, /process\.env\.OPENROUTER_REASONING_EFFORT|process\.env\.OPENROUTER_REASONING_EXCLUDE/, 'stale environment variables must not override Qwen per-turn reasoning budgets')
  ...
  assert.match(llm, /function providerReasoningPayload[\s\S]*forcedToolChoice[\s\S]*toolChoice === 'required'[\s\S]*typeof toolChoice === 'object'[\s\S]*_reasoning\?\.enabled === false \|\| forcedToolChoice[\s\S]*max_tokens:\s*safeBudget[\s\S]*exclude:\s*true/, 'Qwen calls must use explicit per-turn reasoning budgets, hide internal traces, and disable thinking for provider-incompatible forced tool choices')
  assert.doesNotMatch(llm, /providerCompatibleToolChoice/, 'Qwen must preserve runtime-selected required or named tool choices')
  ...
  assert.match(agentLoop, /FAST_SOURCE_ACTION_REQUEST_TIMEOUT_MS = 45_000/, 'source-action starts must tolerate Qwen provider startup')
  assert.match(agentLoop, /FAST_ACTION_REQUEST_TIMEOUT_MS = 45_000/, 'non-source action starts must tolerate Qwen provider startup')
  ...
  assert.match(agentLoop, /PRESENTATION_REASONING = \{ enabled: false, exclude: true \}/, 'acknowledgement, narration, and handoff presentation turns must avoid hidden reasoning latency')
  assert.match(agentLoop, /FAST_ACTION_REASONING = \{ max_tokens: 192, exclude: true \}/, 'Qwen action selection must use a small bounded reasoning budget')
  assert.match(agentLoop, /TASK_REASONING = \{ max_tokens: 1_024, exclude: true \}/, 'ordinary Qwen task turns must receive a substantive reasoning budget')
  assert.match(agentLoop, /DEEP_TASK_REASONING = \{ max_tokens: 2_048, exclude: true \}/, 'final synthesis and build turns must receive the larger Qwen reasoning budget')
  ...
  assert.match(agentLoop, /const FINAL_INLINE_ANSWER_REQUEST_TIMEOUT_MS = 60_000/, 'final inline answers must tolerate Qwen provider startup')
  ...
  assert.match(agentLoop, /FINAL_SAVED_DELIVERABLE_REQUEST_TIMEOUT_MS = 60_000/, 'saved final deliverables must have a bounded startup window that tolerates Qwen provider variance')
  ...
  assert.match(agentLoop, /const requestReasoning = useCompactNarration[\s\S]*PRESENTATION_REASONING[\s\S]*isFinalInlineAnswerTurn[\s\S]*DEEP_TASK_REASONING[\s\S]*fastActionTurn[\s\S]*FAST_ACTION_REASONING[\s\S]*TASK_REASONING/, 'Qwen reasoning must adapt by turn type without slowing visible narration')
