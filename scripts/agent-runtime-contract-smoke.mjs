...
async function assertSourceContracts() {
  ...
  assert.match(agentLoop, /fastActionTurn[\s\S]*MINIMAL_THINKING_REASONING/, 'between-tool action turns must use the shared lowest-reasoning fast lane')
  ...
  assert.match(agentLoop, /lastToolResults = await toolPipeline\.executeAll[\s\S]*narratedResult\?\.acceptedForExecution === true && !narratedResult\.isError[\s\S]*acceptProgressNarration[\s\S]*this\.emitter\.progressUpdate\(acceptedNarration\.text,[\s\S]*afterToolId: narratedResult\.tc\.id/, 'native narration must be validated and emitted only after the billed matching action succeeds')
  ...
  assert.match(agentLoop, /const requestReasoning = useCompactNarration[\s\S]*MINIMAL_THINKING_REASONING[\s\S]*isFinalInlineAnswerTurn[\s\S]*TASK_REASONING/, 'all task and final-answer turns must use the configured lowest reasoning mode')
