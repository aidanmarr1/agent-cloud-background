...
async function assertSourceContracts() {
  ...
  assert.match(agentLoop, /FAST_SOURCE_ACTION_REQUEST_TIMEOUT_MS = 4_500/, 'source-action starts must use a realistic provider-start window')
  assert.match(agentLoop, /FAST_ACTION_REQUEST_TIMEOUT_MS = 4_500/, 'non-source action starts must use a realistic provider-start window')
  assert.match(agentLoop, /FAST_ACTION_RETRY_REQUEST_TIMEOUT_MS = 5_000/, 'hot action retries must avoid stacked 2s false failures')
  ...
  assert.match(agentLoop, /const FINAL_INLINE_ANSWER_REQUEST_TIMEOUT_MS = 6_000/, 'final inline answers must get a realistic provider-start window without falling into deadline finalization')
