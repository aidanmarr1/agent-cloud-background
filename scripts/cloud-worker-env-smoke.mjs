...
requireExact('LLM_PROVIDER', 'deepseek', 'direct DeepSeek model provider')
requireRealEnv('DEEPSEEK_API_KEY', 'DeepSeek API key for task execution', validateNonShortToken)
requireExact('DEEPSEEK_MODEL', 'deepseek-v4-flash-vision-exp', 'pinned DeepSeek V4 Flash Vision Exp model')
requireUnset('OPENROUTER_API_KEY', 'OpenRouter routing must remain disabled')
requireUnset('OPENROUTER_MODEL', 'OpenRouter routing must remain disabled')
...
for (const name of [
  ...
  'AGENT_E2B_VCPU_COUNT',
  'AGENT_E2B_MEMORY_GIB',
