...
async function assertSourceContracts() {
  ...
  assert.match(modelPricing, /outputUsdPer1M:\s*0\.28/, 'DeepSeek v4 Flash output pricing must match DeepSeek')
  assert.match(modelPricing, /contextTokens:\s*1_000_000/, 'DeepSeek v4 Flash context window must match DeepSeek')
  assert.match(modelPricing, /maxCompletionTokens:\s*384_000/, 'DeepSeek v4 Flash max output token cap must match DeepSeek')
