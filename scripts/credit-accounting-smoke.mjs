...
async function assertSourceContracts() {
  ...
  assert.match(modelPricing, /DEFAULT_OPENROUTER_MODEL = 'openai\/gpt-5\.4-mini'/, 'default model must be GPT-5.4 Mini through OpenRouter')
  assert.match(modelPricing, /inputUsdPer1M:\s*0\.75/, 'GPT-5.4 Mini input pricing must match OpenRouter')
  assert.match(modelPricing, /cacheHitInputUsdPer1M:\s*0\.075/, 'GPT-5.4 Mini cache-read pricing must match OpenRouter')
  ...
  assert.match(modelPricing, /maxCompletionTokens:\s*65_536/, 'GPT-5.4 Mini max output token cap must match OpenRouter')
