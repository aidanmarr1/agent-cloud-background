...
async function assertSourceContracts() {
  ...
  assert.match(modelPricing, /inputUsdPer1M:\s*0\.375/, 'Gemini input pricing must match the standard Google Vertex endpoint')
  assert.match(modelPricing, /cacheHitInputUsdPer1M:\s*0\.0375/, 'Gemini cache-read pricing must match the standard Google Vertex endpoint')
  assert.match(modelPricing, /outputUsdPer1M:\s*1\.875/, 'Gemini output pricing must match the standard Google Vertex endpoint')
  assert.match(modelPricing, /internalReasoningUsdPer1M:\s*1\.875/, 'Gemini reasoning pricing must match output pricing')
  ...
  assert.match(modelPricing, /maxCompletionTokens:\s*65_536/, 'Gemini output cap must preserve the full configured budget')
