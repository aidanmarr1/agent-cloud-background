...
async function assertSourceContracts() {
  ...
  assert.match(planManager, /isSafeStreamingPlannerAckDraft\(draft, request\)[\s\S]*this\.emitter\.textDelta\(draft\)[\s\S]*fetchGenerationUsage\(generationId[\s\S]*recordCompletionUsage\(streamedUsage, 'ack'\)[\s\S]*this\.acknowledgementEmitted = true/, 'the separate acknowledgement must stream safe model text immediately while exact usage still fences plan and first-step release')
