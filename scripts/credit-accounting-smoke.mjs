...
async function assertSourceContracts() {
  ...
  assert.match(modelPricing, /DEFAULT_OPENROUTER_MODEL = 'google\/gemini-3\.5-flash'/, 'default model must be Gemini 3.5 Flash through OpenRouter')
  assert.match(modelPricing, /inputUsdPer1M:\s*1\.50/, 'Gemini 3.5 Flash input pricing must match OpenRouter')
  assert.match(modelPricing, /cacheHitInputUsdPer1M:\s*0\.15/, 'Gemini 3.5 Flash cache-read pricing must match OpenRouter')
  assert.match(modelPricing, /outputUsdPer1M:\s*9\.00/, 'Gemini 3.5 Flash output pricing must match OpenRouter')
  assert.match(modelPricing, /contextTokens:\s*1_048_576/, 'Gemini 3.5 Flash context window must match OpenRouter')
  assert.match(modelPricing, /maxCompletionTokens:\s*65_536/, 'Gemini 3.5 Flash max output token cap must match OpenRouter')
