...
async function assertSourceContracts() {
  ...
  assert.match(modelPricing, /DEFAULT_OPENROUTER_MODEL = 'google\/gemini-3\.8-flash'/, 'default model must be Gemini 3.8 Flash')
  assert.match(modelPricing, /inputUsdPer1M:\s*0\.75/, 'Gemini input pricing must match the published OpenRouter standard rates')
  assert.match(modelPricing, /cacheHitInputUsdPer1M:\s*0\.075/, 'Gemini cache-read pricing must match the published OpenRouter standard rates')
  assert.match(modelPricing, /outputUsdPer1M:\s*3\.75/, 'Gemini output pricing must match the published OpenRouter standard rates')
  assert.match(modelPricing, /internalReasoningUsdPer1M:\s*3\.75/, 'Gemini reasoning pricing must match output pricing')
  assert.match(modelPricing, /contextPriceTiers:\s*\[\]/, 'Gemini must not inherit unrelated long-context price tiers')
  assert.match(modelPricing, /contextTokens:\s*1_048_576/, 'Gemini context window must match Gemini')
  assert.match(modelPricing, /maxCompletionTokens:\s*65_536/, 'Gemini provider output cap must match the published OpenRouter standard rates')
  ...
  assert.match(serverCredits, /refundServerToolCharge/, 'failed prepaid external-tool requests must have an idempotent server refund path')
  assert.match(serverCredits, /tool-refund:[\s\S]*amount: -amount[\s\S]*category: 'adjustment'/, 'tool refunds must restore the exact whole-number prepaid charge through the ledger')
  ...
  assert.match(toolPipeline, /searchRecovery[\s\S]*serverToolCharge[\s\S]*refundUnavailableServerToolCharge\(tc\.id, tc\.name\)/, 'a failed or timed-out search provider request must refund its accepted prepaid charge')
  ...
  assert.match(llm, /stream_options: \{ include_usage: true \}/, 'Gemini streams must request token usage')
  assert.doesNotMatch(llm, /GENERATION_URL|api\.deepseek\.com/, 'usage recovery must not call a different provider or poll generation metadata')
  ...
  assert.match(planManager, /await this\.assertCreditRunway\('ack'\)[\s\S]*createStreamingCompletion/, 'precomputed-plan acknowledgement calls must preflight credits before streaming provider work')
  ...
  assert.match(normalPlannerStartup, /scheduleAcknowledgementCall\(\)[\s\S]*attemptPlanCall\(0,\s*true\)/, 'normal startup must meter its dedicated acknowledgement and structured plan as separate intentional control requests')
  assert.match(planManager, /await this\.assertCreditRunway\('initial'\)[\s\S]*streamInitialPlannerResponse\(params, true\)/, 'the structured startup planner must preflight credits before provider work')
  ...
  assert.match(planManager, /isSafeStreamingPlannerAckDraft\(draft, request\)[\s\S]*this\.emitter\.textDelta\(draft\)[\s\S]*fetchGenerationUsage\(generationId[\s\S]*recordCompletionUsage\(streamedUsage, 'ack'\)[\s\S]*this\.acknowledgementEmitted = true/, 'the precomputed-plan acknowledgement must stream safe model text immediately while exact usage still fences first-step release')
  assert.match(planManager, /streamInitialPlannerResponse[\s\S]*stream_options:\s*\{ include_usage: true \}[\s\S]*fetchGenerationUsage\(generationId[\s\S]*recordCompletionUsage\(usage, 'initial'\)/, 'the structured planner must recover and durably record exact usage before plan or first-step release')
  assert.match(planManager, /if \(state\.planEmitted && this\.planPromise && !this\.skipAcknowledgement\) \{[\s\S]*await this\.acknowledgementPromise[\s\S]*if \(this\.acknowledgementUsageError\) throw this\.acknowledgementUsageError/, 'precomputed first-step work must remain fenced on acknowledgement usage settlement')
...
export async function runCreditPricingSmoke() {
  ...
  assert.ok(Math.abs((estimateUsageCost({ at: new Date('2026-09-09T02:00:00Z'), prompt_tokens: 1000, completion_tokens: 1000 }) || 0) - 0.0045) < 1e-12)
  assert.ok(Math.abs((estimateUsageCost({ at: new Date('2026-09-09T02:00:00Z'), prompt_tokens: 50_000, completion_tokens: 1000 }) || 0) - 0.04125) < 1e-12)
  assert.ok(Math.abs((estimateUsageCost({ at: new Date('2026-09-09T02:00:00Z'), prompt_tokens: 500_000, completion_tokens: 1000 }) || 0) - 0.37875) < 1e-12)
  assert.ok(Math.abs((estimateUsageCost({ at: new Date('2026-09-09T02:00:00Z'),
  ...
  assert.equal(estimateUsageCost({ at: new Date('2026-09-09T04:00:00Z'), prompt_tokens: 1000, completion_tokens: 1000 }), 0.0045)
  assert.equal(estimateUsageCost({ at: new Date('2026-09-12T02:00:00Z'), prompt_tokens: 1000, completion_tokens: 1000 }), 0.0045)

  assert.equal(estimateUsageCost({ prompt_tokens: 1000, completion_tokens: 1000, prompt_tokens_details: { cached_tokens: 1000 } }), 0.003825)
