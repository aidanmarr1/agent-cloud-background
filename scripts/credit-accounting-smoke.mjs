...
async function assertSourceContracts() {
  ...
  assert.match(modelPricing, /cacheHitInputUsdPer1M:\s*0\.30/, 'cache pricing must stay conservative when no discounted rate is guaranteed')
  assert.match(modelPricing, /outputUsdPer1M:\s*2\.50/, 'Gemini 3.5 Flash Lite output pricing must match OpenRouter')
  assert.match(modelPricing, /contextTokens:\s*1_000_000/, 'Gemini 3.5 Flash Lite context window must match OpenRouter')
  assert.match(modelPricing, /maxCompletionTokens:\s*65_536/, 'Gemini 3.5 Flash Lite max output token cap must match OpenRouter')
