...
async function assertSourceContracts() {
  ...
  assert.match(modelPricing, /DEFAULT_OPENROUTER_MODEL = 'google\/gemini-3\.1-flash-lite'/, 'default model must be Gemini 3.1 Flash Lite through OpenRouter')
  assert.match(modelPricing, /inputUsdPer1M:\s*0\.25/, 'Gemini 3.1 Flash Lite input pricing must match OpenRouter')
  assert.match(modelPricing, /cacheHitInputUsdPer1M:\s*0\.025/, 'Gemini 3.1 Flash Lite cache-read pricing must match OpenRouter')
  assert.match(modelPricing, /outputUsdPer1M:\s*1\.50/, 'Gemini 3.1 Flash Lite output pricing must match OpenRouter')
  assert.match(modelPricing, /contextTokens:\s*1_048_576/, 'Gemini 3.1 Flash Lite context window must match OpenRouter')
  assert.match(modelPricing, /maxCompletionTokens:\s*65_536/, 'Gemini 3.1 Flash Lite max output token cap must match OpenRouter')
