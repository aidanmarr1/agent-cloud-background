...
async function assertSourceContracts() {
  ...
  assert.doesNotMatch(normalPlannerStartup, /scheduleAcknowledgementCall/, 'normal startup must use one metered planner stream instead of charging a redundant acknowledgement request')
  assert.match(planManager, /await this\.assertCreditRunway\('initial'\)[\s\S]*streamInitialPlannerResponse\(params, true\)/, 'the single streamed startup planner must preflight credits before provider work')
  ...
  assert.match(planManager, /streamInitialPlannerResponse[\s\S]*stream_options:\s*\{ include_usage: true \}[\s\S]*fetchGenerationUsage\(generationId[\s\S]*recordCompletionUsage\(usage, 'initial'\)/, 'the ordinary single-stream planner must recover and durably record exact usage before plan or first-step release')
