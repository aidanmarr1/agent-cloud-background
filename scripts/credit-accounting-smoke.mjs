...
async function assertSourceContracts() {
  ...
  assert.match(modelPricing, /DEFAULT_OPENROUTER_MODEL = 'qwen\/qwen3\.7-flash'/, 'default model must be Qwen 3.7 Flash through OpenRouter balanced routing')
  assert.match(modelPricing, /inputUsdPer1M:\s*0\.03/, 'Qwen 3.7 Flash input pricing must match OpenRouter')
  assert.match(modelPricing, /cacheHitInputUsdPer1M:\s*0\.006/, 'Qwen 3.7 Flash cache pricing must match OpenRouter')
  assert.match(modelPricing, /outputUsdPer1M:\s*0\.13/, 'Qwen 3.7 Flash output pricing must match OpenRouter')
  assert.match(modelPricing, /contextTokens:\s*1_000_000/, 'Qwen 3.7 Flash context window must match OpenRouter')
  assert.match(modelPricing, /maxCompletionTokens:\s*65_536/, 'Qwen 3.7 Flash max output token cap must stay bounded')
