...
async function assertSourceContracts() {
  ...
  assert.match(modelPricing, /inputUsdPer1M:\s*0\.03/, 'Qwen base input pricing must match OpenRouter')
  assert.match(modelPricing, /cacheHitInputUsdPer1M:\s*0\.006/, 'Qwen base cache-read pricing must match OpenRouter')
  assert.match(modelPricing, /outputUsdPer1M:\s*0\.13/, 'Qwen base output pricing must match OpenRouter')
  assert.match(modelPricing, /internalReasoningUsdPer1M:\s*0\.13/, 'Qwen reasoning pricing must match output pricing')
  assert.match(modelPricing, /minPromptTokens:\s*32_000[\s\S]*inputUsdPer1M:\s*0\.10[\s\S]*outputUsdPer1M:\s*0\.40/, 'Qwen 32k context pricing must match OpenRouter')
  assert.match(modelPricing, /minPromptTokens:\s*256_000[\s\S]*inputUsdPer1M:\s*0\.20[\s\S]*outputUsdPer1M:\s*0\.80/, 'Qwen 256k context pricing must match OpenRouter')
  assert.match(modelPricing, /contextTokens:\s*1_000_000/, 'Qwen context window must match OpenRouter')
  assert.match(modelPricing, /maxCompletionTokens:\s*65_536/, 'Qwen output cap must match OpenRouter')
...
export async function runCreditPricingSmoke() {
  ...
  assert.ok(Math.abs((estimateUsageCost({ prompt_tokens: 50_000, completion_tokens: 1000 }) || 0) - 0.0054) < 1e-12)
