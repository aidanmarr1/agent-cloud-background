...
async function assertSourceContracts() {
  ...
  assert.match(modelPricing, /cacheHitInputUsdPer1M:\s*0\.15/, 'Gemini 3.6 Flash cache pricing must match OpenRouter')
  assert.match(modelPricing, /outputUsdPer1M:\s*7\.50/, 'Gemini 3.6 Flash output pricing must match OpenRouter')
  assert.match(modelPricing, /contextTokens:\s*1_000_000/, 'Gemini 3.6 Flash context window must match OpenRouter')
