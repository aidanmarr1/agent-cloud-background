...
async function assertSourceContracts() {
  ...
  assert.match(modelPricing, /cacheHitInputUsdPer1M:\s*0\.05/, 'Gemini 3 Flash Preview cache-read pricing must match OpenRouter')
  assert.match(modelPricing, /outputUsdPer1M:\s*3\.00/, 'Gemini 3 Flash Preview output pricing must match OpenRouter')
  assert.match(modelPricing, /contextTokens:\s*1_000_000/, 'Gemini 3 Flash Preview context window must match OpenRouter')
