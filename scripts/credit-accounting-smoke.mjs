...
async function assertSourceContracts() {
  ...
  assert.match(modelPricing, /DEFAULT_OPENROUTER_MODEL = 'google\/gemini-3\.6-flash:exacto'/, 'default model must be Gemini 3.6 Flash through OpenRouter Exacto')
  assert.match(modelPricing, /inputUsdPer1M:\s*1\.50/, 'Gemini 3.6 Flash input pricing must match OpenRouter')
  assert.match(modelPricing, /cacheHitInputUsdPer1M:\s*1\.50/, 'fallback cache pricing must conservatively use input list price')
  ...
  assert.match(modelPricing, /maxCompletionTokens:\s*65_536/, 'Gemini 3.6 Flash max output token cap must stay bounded')
