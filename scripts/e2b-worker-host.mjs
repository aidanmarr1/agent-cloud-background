...
const DEFAULT_REPO = 'https://github.com/aidanmarr1/agent-cloud-background.git'
...
function workerEnvs(workerId) {
  return {
    ...
    DEEPSEEK_API_KEY: requireEnv('DEEPSEEK_API_KEY', 'task LLM calls'),
    DEEPSEEK_MODEL: envOrDefault('DEEPSEEK_MODEL', 'deepseek-v4-flash'),
    DEEPSEEK_REASONING_EFFORT: envOrDefault('DEEPSEEK_REASONING_EFFORT', 'minimal'),
    DEEPSEEK_THINKING_ENABLED: envOrDefault('DEEPSEEK_THINKING_ENABLED', 'false'),
...
const repo = readArg('--repo') || env('E2B_WORKER_REPO_URL') || DEFAULT_REPO
const branch = readArg('--branch') || env('E2B_WORKER_BRANCH') || 'main'
...
const workerId = readArg('--worker-id') || env('E2B_WORKER_ID') || `e2b-worker-host-${randomUUID().slice(0, 8)}`
...
console.log('Secret values are sent to E2B as process env vars but are not printed.')
...
git clone --depth 1 --branch ${shellQuote(branch)} ${shellQuote(repo)} ${shellQuote(workdir)}
