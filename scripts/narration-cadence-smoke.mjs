...
async function assertSourceContracts() {
  ...
  assert.match(streamProcessor, /preferred[\s\S]*cadence boundary[\s\S]*display-only and must never block useful work/, 'ordinary cadence must remain a preference rather than an execution gate')
  assert.match(streamProcessor, /rawUpdate === undefined[\s\S]*if \(allowMissing\)[\s\S]*return true/, 'a missing progress update must fail open once the native action envelope is complete')
  assert.match(streamProcessor, /Invalid or duplicate narration stays invisible[\s\S]*return allowMissing/, 'invalid narration must be hidden without discarding the model-selected action')
