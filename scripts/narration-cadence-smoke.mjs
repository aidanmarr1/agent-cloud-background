...
async function assertSourceContracts() {
  ...
  assert.match(agentLoop, /reasoning:\s*MINIMAL_THINKING_REASONING[\s\S]*requestTimeoutMs:\s*NARRATION_SIDECAR_REQUEST_TIMEOUT_MS[\s\S]*retryMaxAttempts:\s*0/, 'the narration lane must use Qwen 3.7 Flash’s lowest supported reasoning effort in a fast bounded request')
