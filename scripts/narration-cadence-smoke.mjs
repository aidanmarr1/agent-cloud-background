...
async function assertSourceContracts() {
  ...
  assert.match(dispatcher, /if \(isComputerPanelTool\(event\.name\) && this\.isActiveConversation\(\)\)[\s\S]*setComputerPanelActiveItemId\(panelFocusIdForTool\(event\.name,\s*event\.id\)\)/, 'every newly visible Computer action must take focus immediately')
  assert.doesNotMatch(dispatcher, /browserLiveActive|hasStreamingLiveBrowser/, 'late browser frames must not pin focus away from a newer task-stream action')
  assert.match(dispatcher, /if \(event\.name === 'read_file' \|\| event\.name === 'read_attachment'\)[\s\S]*upsertComputerPanelItem/, 'attachment reads must create a live Computer placeholder')
  assert.equal((dispatcher.match(/setComputerPanelActiveItemId\(panelFocusIdForTool/g) || []).length, 1, 'only tool starts may establish live Computer focus')
  assert.match(dispatcher, /Parallel results may finish[\s\S]*completion must not steal focus from a newer action/, 'out-of-order tool results must preserve the newest live action')
  ...
  assert.match(agentLoop, /const compactNarrationReview =[\s\S]*reviewProgressNarration[\s\S]*rejectedCompactNarrationEmission[\s\S]*rejectedModelEmission/, 'invalid compact narration must be withheld and unbilled before it reaches the task stream')
  assert.match(agentLoop, /phaseEndNarrationWasPending[\s\S]*state\.forcedNarrationRepairAttempts \+= 1[\s\S]*Retry immediately from the newest completed evidence/, 'a short phase must retry the LLM-authored update instead of silently advancing without narration')
  assert.match(agentLoop, /FORCED_NARRATION_MAX_TOKENS = 128/, 'phase narration must have enough output room for a natural completed-result update and next-step marker')
  ...
  assert.doesNotMatch(agentState.slice(agentState.indexOf('export function advanceStep')), /visibleToolActionsSinceLastNarration = 0/, 'phase changes must preserve outstanding narration cadence')
  ...
  assert.match(agentLoop, /function shouldUseNaturalCadenceNarration[\s\S]*visibleToolActionsSinceLastNarration < Math\.min[\s\S]*return true/, 'runtime must open the deterministic cadence window')
  assert.match(agentLoop, /hardWindowOpen =[\s\S]*NARRATION_MAX_VISIBLE_ACTION_GAP[\s\S]*state\.forceTextNextIteration \|\| state\.exactExtractionGuardPending[\s\S]*!hardWindowOpen/, 'internal text/extraction guards must never suppress narration once three actions have settled')
  assert.match(agentLoop, /const cadenceNarrationInMainTurn =[\s\S]*shouldUseNaturalCadenceNarration\(state\)[\s\S]*beginNarrationCadenceAttempt\(state\)/, 'cadence must be armed on the next ordinary action request once three visible actions complete')
  ...
  assert.match(config, /NARRATION_MAX_VISIBLE_ACTION_GAP\s*=\s*3/, 'three actions is the maximum silent gap')
  assert.match(streamProcessor, /envelopeComplete && hardCadenceBoundary[\s\S]*missing_progress_update/, 'a fourth action must wait for a model-authored update')
  assert.doesNotMatch(agentLoop, /const effectiveCadenceNarrationInMainTurn =\s*!allowPhaseDecision/, 'completion eligibility must not disable tool narration')
  assert.match(agentLoop, /allowTextOnlyCompletion: allowPhaseDecision/, 'narration must not force extra tools to complete a phase')
  ...
  assert.match(config, /inactivityTimeoutMs:\s*IS_OLLAMA \? 120_000 : 8_000/, 'API inactivity timeout must tolerate normal provider jitter while the iteration watchdog bounds frozen turns')
  ...
  assert.match(prompts, /Do not narrate with fewer than 3 new visible actions except at phase completion, and never go past 3 visible actions/, 'agent prompt must enforce a maximum three-action narration gap')
  ...
  assert.match(narrationMemory, /Detailed narration rules are injected once in the turn prompt/, 'tool schemas must use the shared narration guidance without repeating it for every tool')
...
export function runNarrationSmoke() {
  ...
  assert.deepEqual(Object.keys(cadenceSchemas[0].function.parameters.properties), ['progress_update', 'query'], 'the short narration field must arrive first so hard-boundary validation adds no file-stream delay')
  assert.deepEqual(cadenceSchemas[0].function.parameters.required, ['query', 'progress_update'])
  ...
  assert.equal(phaseResetState.narrationWorkLogFrontier, null, 'unnarrated work must remain available after a phase change')

  state.visibleToolActionsSinceLastNarration = 3
  assert.equal(visibleNarrationActionHeadroom(state), 0)
  ...
  assert.equal(beginNarrationCadenceAttempt(state), true, 'a failed attempt stays due at the hard boundary')
  finishNarrationCadenceAttempt(state)
