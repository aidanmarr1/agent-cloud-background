...
async function assertSourceContracts() {
  ...
  assert.match(policyEngine, /three visible action pills have completed without a valid user-facing progress paragraph/, 'overdue narration recovery must open at the 3-action point')
  ...
  assert.match(policyEngine, /needsPhaseNarrationBeforeAdvance/, 'backend must require at least one accepted LLM narration before a phase advances')
  ...
  assert.match(agentLoop, /goalCheck\.allMet[\s\S]*?pauseForPhaseEndNarrationBeforeAutoAdvance[\s\S]*?planManager\.handleStepAdvance\(state\)/, 'goal-complete auto-advance must pause for phase-end narration')
  ...
  assert.match(agentLoop, /function shouldUseNaturalCadenceNarration[\s\S]*visibleToolActionsSinceLastNarration < 3[\s\S]*return true/, 'runtime must open the 3-action cadence window')
  ...
  assert.match(toolPipeline, /After 4 completed visible actions it becomes a hard gate/, 'tool execution must hard-gate the fifth visible action until narration is accepted')
  assert.match(toolPipeline, /this visible tool call was skipped because 4 visible actions/, 'tool execution must block the fifth visible action when narration is overdue')
  ...
  assert.match(config, /inactivityTimeoutMs:\s*IS_OLLAMA \? 120_000 : 2_000/, 'API inactivity timeout must recover quickly instead of leaving the UI in invisible thinking')
