...
async function assertSourceContracts() {
  ...
  assert.match(agentLoop, /lastStreamResult\.cadenceProgressUpdate[\s\S]*acceptProgressNarration\(state,[\s\S]*resetCadence:\s*true/, 'committed model-authored progress must reset cadence only after billing succeeds')
  ...
  assert.match(agentLoop, /Put one natural completed-result update in that field, sized to the newest evidence rather than a fixed template/, 'native narration must let the evidence determine update length and form')
