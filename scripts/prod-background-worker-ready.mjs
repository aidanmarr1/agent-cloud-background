...
const timeoutMs = Number.parseInt(readArg('--timeout-ms') || process.env.AGENT_WORKER_READY_TIMEOUT_MS || '30000', 10)

if (!secret) {
  throw new Error('Missing AGENT_INTERNAL_HEALTH_SECRET or AUTH_SECRET for signed background worker readiness request.')
}
...
const timeout = setTimeout(() => abort.abort(), Number.isFinite(timeoutMs) && timeoutMs > 0 ? timeoutMs : 30_000)
...
try {
  ...
  if (abort.signal.aborted) {
    console.log(JSON.stringify({
      ok: false,
      error: `Timed out after ${Number.isFinite(timeoutMs) && timeoutMs > 0 ? timeoutMs : 30_000}ms waiting for the deployed background-worker readiness endpoint.`,
      url: `${baseUrl}${path}`,
    }, null, 2))
    process.exitCode = 1
  } else {
...
const hint = parsedOk
  ...
  : text && !contentType.includes('application/json')
    ? 'Readiness endpoint did not return JSON. Check that the deployed URL points at this app and includes the latest build.'
    : body === null
      ? 'Readiness endpoint returned an empty body. Rebuild/redeploy the web service, then retry.'
      : undefined
