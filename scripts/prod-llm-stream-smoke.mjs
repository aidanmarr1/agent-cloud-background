...
const mode = process.argv[3] || 'research'
const prompt = process.argv.slice(4).join(' ').trim()
const path = '/api/internal/llm-stream-smoke'
const secret = process.env.AGENT_INTERNAL_HEALTH_SECRET || process.env.AUTH_SECRET

if (!secret) {
  throw new Error('Missing AGENT_INTERNAL_HEALTH_SECRET or AUTH_SECRET for signed LLM stream smoke request.')
}
