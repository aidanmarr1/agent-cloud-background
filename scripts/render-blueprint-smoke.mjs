...
for (const service of [web, worker]) {
  ...
  requireValue(service, 'DEEPSEEK_MODEL', 'deepseek-v4.1-flash-expires-on-0910')
  assert.equal(service.env.has('OPENROUTER_API_KEY'), false, `${service.name} must not retain an OpenRouter credential`)
  assert.equal(service.env.has('OPENROUTER_MODEL'), false, `${service.name} must not retain an OpenRouter route`)
