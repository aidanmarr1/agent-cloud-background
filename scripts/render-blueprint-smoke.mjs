...
for (const service of [web, worker]) {
  ...
  requireValue(service, 'OPENROUTER_MODEL', 'meta/muse-spark-1.2')
  assert.equal(service.env.has('OPENROUTER_REASONING_EFFORT'), false, `${service.name} must use per-turn reasoning levels instead of a stale effort label`)
