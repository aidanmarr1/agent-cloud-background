...
for (const service of [web, worker]) {
  ...
  requireValue(service, 'OPENROUTER_MODEL', 'google/gemini-3.6-flash')
  assert.equal(service.env.has('OPENROUTER_REASONING_EFFORT'), false, `${service.name} must use Gemini per-turn reasoning levels instead of a stale effort label`)
