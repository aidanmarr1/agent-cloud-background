...
for (const service of [web, worker]) {
  ...
  requireValue(service, 'OPENROUTER_MODEL', 'google/gemini-3.8-flash')
  assert.equal(service.env.has('DEEPSEEK_API_KEY'), false, `${service.name} must not retain a DeepSeek credential`)
  assert.equal(service.env.has('DEEPSEEK_MODEL'), false, `${service.name} must not retain an expired DeepSeek route`)
  requireValue(service, 'OPENROUTER_REASONING_EFFORT', 'low')
  requireValue(service, 'OPENROUTER_PROVIDER_SORT', 'throughput')
