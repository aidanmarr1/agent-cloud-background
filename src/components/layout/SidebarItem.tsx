  ...
  return (
    <div className="relative w-full flex justify-center">
      ...
      <button
        ...
        className={`group relative h-10 rounded-[12px] flex items-center transition-all duration-300 ease-out active:scale-[0.96] ${
      ...
      >
        ...
        {badge !== undefined && badge > 0 && !expanded && (
          <span className="absolute -top-0.5 -right-0.5 min-w-[16px] h-[16px] rounded-full bg-accent-blue text-text-on-accent text-[9px] font-bold flex items-center justify-center px-1 leading-none ring-2 ring-bg-secondary">
            {badge > 99 ? '99+' : badge}
          </span>
        )}
      </button>
      ...
      {!expanded && hovered && (
        ...
        >
          <div
            aria-hidden="true"
            className="absolute -left-[5px] top-1/2 -translate-y-1/2 w-[10px] h-[10px] menu-surface border-l border-b border-border-primary rotate-45"
          />
          <div
            className="relative menu-surface border border-border-primary text-[11.5px] px-3 h-8 rounded-xl whitespace-nowrap flex items-center gap-2"
            style={{ boxShadow: 'var(--shadow-xl)' }}
          >
            <span className="text-text-primary font-semibold tracking-[0]">{tooltip}</span>
            {shortcut && (
              <kbd className="text-text-muted text-[10px] font-mono font-medium bg-bg-secondary border border-border-primary rounded-md px-1.5 h-5 flex items-center tabular-nums">{shortcut}</kbd>
            )}
          </div>
        </div>
      )}
