...
export function Toggle({ enabled, onChange, label = 'Toggle setting' }: {
  ...
  return (
    <button
      ...
      aria-label={label}
      className={`relative h-5 w-9 flex-shrink-0 rounded-full border p-[2px] transition-colors duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-active/35 ${
    ...
    >
      <div
        className={`h-[14px] w-[14px] rounded-full transition-transform duration-200 ease-out ${
          enabled ? 'translate-x-4 bg-toggle-thumb' : 'translate-x-0 bg-text-tertiary'
        }`}
        style={{
