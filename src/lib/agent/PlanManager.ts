...
const PLANNER_FAST_JSON_MAX_TOKENS = 1_600
const PLANNER_SIMPLE_JSON_MAX_TOKENS = 1_200
const PLANNER_MEDIUM_JSON_MAX_TOKENS = 2_000
const PLANNER_JSON_MAX_TOKENS = 2_800
...
const PLANNER_RELAXED_JSON_REQUEST_TIMEOUT_MS = 30_000
const PLANNER_REPAIR_REQUEST_TIMEOUT_MS = 45_000
...
function nonDeliverableStepGuidance(
  ...
  if (state.taskStrategy === 'browse') {
    return `RULES:\n- Complete this website phase's requested outcome using appropriate available tools. For live interactions, use the current browser state.\n- If the user gave a URL/domain, use that target directly unless the task or an observed blocker calls for another route. Reuse page evidence from earlier phases; a phase boundary does not require a fresh navigation or read.\n- Before browser_type, confirm a fresh input/textarea/contenteditable [N] is focused or click it first. Treat visible validation/errors as page state to fix before submit/advance/success.\n- Cover each requested item, field, or choice. For option groups, use the current selection and visual metadata to identify what remains; already-selected options need no extra click.\n- If the phase is satisfied, state the concrete finding and emit <next_step/>. Remaining controls do not require more work. Otherwise identify the missing outcome and choose a useful action; do not cycle through find/read/screenshot merely to reconfirm the same state.\n- Report concrete blockers and unresolved gaps honestly. Do not infer success from an available button or turn a listed price into an unverified checkout total.`
  }
...
export class PlanManager {
  ...
  private dedicatedAcknowledgementForPlan = false
  ...
  startPlanCall(): void {
    ...
    // Start the short model-authored opening first, then start structured
    // planning immediately alongside it. The plan may finish first internally,
    // but its visible event and every substantive action remain fenced on the
    // acknowledgement. Keeping these as separate control requests prevents a
    // malformed/empty JSON response from erasing both pieces of startup UI.
    this.dedicatedAcknowledgementForPlan = true
    this.scheduleAcknowledgementCall()
  ...
  async recoverFromPlannerFailure(state: AgentStateData): Promise<boolean> {
    ...
    const repaired = await this.repairPlannerResponse(
      `USER REQUEST:\n${request}\n\nThe earlier planner response was unavailable. Produce a fresh task-specific plan from the request.`,
      PLANNER_QUALITY_ERROR,
    )
    if (!repaired || this.emitter.isClosed) return false

    try {
      const recovered = await this.emitParsedPlan(state, repaired)
      if (recovered) {
        console.warn('[AgentDiagnostics] Recovered planner startup with a model-authored visible plan')
      }
      return recovered
    } catch (error) {
      if (!isPlannerQualityError(error)) throw error
      return false
    }
  }
  ...
  private async emitModelGeneratedAcknowledgement(taskShape: string, priorInvalidAck?: string): Promise<boolean> {
    ...
    // Use the same complete compact task window as planning. Previously this
...
- Use 0 steps only for a trivial non-tool answer. Otherwise use enough task-specific steps for the requested work.`,
  ...
  private emitAcknowledgementFromStreamingPlan(raw: string): void {
    if (
      this.dedicatedAcknowledgementForPlan ||
...
Generate an updated compact list of remaining task modules (including a revised current module if needed). Return ONLY a JSON array of strings. You own the new plan shape, wording, boundaries, and dependency-aware order. Group tool calls and source pages beneath meaningful workstreams or outcomes; retain separate evidence-angle, entity, region, assessment/synthesis, build, or verification modules only when the remaining work genuinely needs them. The final remaining module must name the concrete user-facing answer, action state, artifact, report, export, or handoff, and may combine final work with delivery. Avoid context-free labels such as "Deliver results". Keep every module actionable and specific.`,
      ...
      if (jsonMatch) {
        ...
        if (Array.isArray(newSteps) && newSteps.length > 0 && newSteps.every(s => typeof s === 'string')) {
          ...
          const modelRemaining = { titles: newSteps, scopes: newSteps.map(() => null) }
          const updatedPlan = [...completedSteps, ...modelRemaining.titles]
          ...
          if (state.currentPlanScopes) {
            ...
            state.currentPlanScopes = [...completedScopes, ...modelRemaining.scopes]
          }
          state.replanCount++
          ...
          const remainingStepCount = modelRemaining.titles.length
