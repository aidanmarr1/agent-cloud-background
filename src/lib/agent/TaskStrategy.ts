...
const STRATEGIES: Record<TaskType, TaskStrategyConfig> = {
  research: {
    ...
    stepGuidance: {
      research: 'Do more work inside each phase instead of adding more phase titles. Use strong sources, not snippet-only coverage. Use web_search for discovery, then default to read_document/http extraction for normal research pages. Use browser navigation/content primarily for dynamic or scripted state, interaction/action work, screenshots, or exact details that need visibly rendered confirmation. One search can satisfy a narrow lookup, but explanatory, cultural, historical, technical, current, or contested phases need a real evidence packet: targeted searches, opened primary/academic/specialist pages, concrete extracted details, and enough comparison/caveat coverage to make the phase useful.',
      deliverable: 'Create a coherent professional report when the user asks for research/report output. Choose its title, organization, prose/bullets/tables, synthesis, conclusions, and source presentation from the subject, audience, evidence, and explicit request rather than a fixed template. Connect facts into reasons, examples, tradeoffs, and a useful bottom line, and state source gaps plainly.',
    },
