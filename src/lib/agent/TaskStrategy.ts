...
const STRATEGIES: Record<TaskType, TaskStrategyConfig> = {
  research: {
    ...
    stepGuidance: {
      ...
      deliverable: 'Create one coherent professional report when the user asks for research/report output. Use a specific title, compact metadata when useful, an opening synthesis, substantive topic-specific sections, a conclusion, and References/Sources with inline [n] citations where evidence requires them. Adapt headings and numbering to the subject instead of forcing a template. Synthesize the why, examples, tradeoffs, and bottom line instead of stacking source notes, and state source gaps plainly.',
    },
