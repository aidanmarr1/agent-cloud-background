...
export function partialAppendRecoveryCountForPath(
  ...
  return state.partialFileWriteRecoveries.filter(
    recovery => recovery.path === path && recovery.toolName === 'append_file',
  ).length
}
...
import { serializeToolMessageContent } from './ToolMessageSerialization'
import {
...
export interface ToolExecutionResult {
  ...
  result: unknown
  isError: boolean
  /** True only when this call passed preflight and reached a handler or usable cache result. */
...
function isDocumentLikeTool(toolName: string): boolean {
  return toolName === 'read_document' || toolName === 'http_request'
}
...
const LOCAL_ABORT_RECOVERY_TOOLS = new Set(['web_search', 'image_search', 'read_document'])

function isAbortError(error: unknown): boolean {
  return error instanceof Error && error.name === 'AbortError'
}

function shouldUnwindToolExecution(error: unknown, taskSignal: AbortSignal | null): boolean {
  if (taskSignal?.aborted) return true
  return error instanceof Error && error.message === 'Tool execution aborted'
}

function isRecoverableToolLocalAbort(
  toolName: string,
  error: unknown,
  taskSignal: AbortSignal | null,
): boolean {
  return (
    LOCAL_ABORT_RECOVERY_TOOLS.has(toolName) &&
    isAbortError(error) &&
    !shouldUnwindToolExecution(error, taskSignal)
  )
}
...
function documentTimeoutRecoveryResult(
  toolName: string,
  args: Record<string, unknown>,
  errMsg: string,
  locallyAborted = false,
): Record<string, unknown> | null {
  if (!isDocumentLikeTool(toolName) || (!locallyAborted && !/\btimed out\b/i.test(errMsg))) return null
...
function repairRuntimeDisplayContractArgs(
  ...
  if (state.currentStepIdx === 0 && parsedStep === 0) {
    ...
    // plan_step_index is display/ordering metadata, not model authority over
    // the active phase. Normalize it to the runtime-owned step here; the
    // semantic phase guard below still blocks a tool whose actual purpose
    // belongs to a previous or future step. This avoids burning paid turns on
    // a label/index-only repair loop.
...
function uploadedAttachmentToolBlockReason(
  ...
  if (toolName === 'read_document' || toolName === 'http_request' || toolName === 'browser_navigate' || toolName === 'browse_page') {
    const target = [args.url, args.source, args.path]
      .map(value => typeof value === 'string' ? value : '')
      .find(Boolean) || ''
    const match = uploadedAttachmentMatch(state, target)
    if (!match) return null
...
function researchUrlFromToolCall(toolName: string, args: Record<string, unknown>, result?: unknown): string | undefined {
  ...
  const value = toolName === 'read_document'
    ? args.url || args.source || resultObj?.url || resultObj?.source
...
const RECOVERABLE_TRUNCATED_READ_ONLY_TOOL_SCHEMAS = {
  web_search: {
    allowedKeys: new Set(['action_label', 'plan_step_index', 'query']),
    requiredStringKey: 'query',
    requireDisplayEnvelope: false,
  },
  read_document: {
    allowedKeys: new Set(['action_label', 'plan_step_index', 'url']),
    requiredStringKey: 'url',
    requireDisplayEnvelope: true,
  },
} as const

/**
 * Recover the single conservative streamed-JSON case where a flat read-only
 * source-call object is complete except for its final `}`. The required query
 * or URL must already be a fully closed JSON string. Unknown keys, duplicate
 * keys, unterminated values, trailing commas, and every other malformed shape
 * remain rejected. read_document additionally requires a strict visible label
 * and the active plan step because it runs directly against the supplied URL.
 */
export function repairTruncatedFlatToolArguments(
  toolName: string,
  argumentsText: string,
  expectedPlanStepIndex?: number,
): string | null {
  const schema = RECOVERABLE_TRUNCATED_READ_ONLY_TOOL_SCHEMAS[
    toolName as keyof typeof RECOVERABLE_TRUNCATED_READ_ONLY_TOOL_SCHEMAS
  ]
  ...
  if (keys.length === 0 || keys.some(key => !schema.allowedKeys.has(key))) return null
  for (const key of keys) {
    const occurrences = argumentsText.match(new RegExp(`"${key}"\\s*:`, 'g'))?.length || 0
    if (occurrences !== 1) return null
  }

  const requiredValue = args[schema.requiredStringKey]
  if (typeof requiredValue !== 'string' || !requiredValue.trim()) return null
  if (args.action_label !== undefined && typeof args.action_label !== 'string') return null
  if (args.plan_step_index !== undefined && !Number.isFinite(args.plan_step_index)) return null
  if (schema.requireDisplayEnvelope) {
    if (!strictActionLabelFromArgs(args)) return null
    if (!Number.isInteger(expectedPlanStepIndex)) return null
    if (args.plan_step_index !== expectedPlanStepIndex) return null
  }
...
function buildToolStartArgsForExecution(toolName: string, args: Record<string, unknown>): Record<string, unknown> {
  ...
  return sanitizeToolStartArgs(toolName, { ...args, ...displayContractArgs })
}
...
export class ToolPipeline {
  ...
  private creditAttempt = 1
  private userId: string | undefined
  private ensureBrowserFrameStream: (() => void) | undefined
  private researchActivityAppender: (entry: ResearchActivityEntry) => Promise<void>
  private inflightToolHandlers = new Map<Promise<unknown>, { name: string; nonIdempotent: boolean }>()
  ...
  constructor(
    ...
    opts?: {
      ...
      researchActivityAppender?: (entry: ResearchActivityEntry) => Promise<void>
    },
  ) {
    ...
    this.creditAttempt = Number.isFinite(Number(opts?.creditAttempt))
      ? Math.max(1, Math.floor(Number(opts?.creditAttempt)))
      : 1
    this.userId = opts?.userId
    this.ensureBrowserFrameStream = opts?.ensureBrowserFrameStream
    this.researchActivityAppender = opts?.researchActivityAppender ?? appendResearchActivityEntry
    opts?.registerInflightToolDrain?.((timeoutMs) => this.drainInflightTools(timeoutMs))
  }

  trackInflightOperation<T>(
    promise: Promise<T>,
    toolName: string,
    args: Record<string, unknown>,
  ): Promise<T> {
    this.inflightToolHandlers.set(promise, {
      name: toolName,
      nonIdempotent: isNonIdempotentToolCall(toolName, args),
    })
    void promise.then(
      () => this.inflightToolHandlers.delete(promise),
      () => this.inflightToolHandlers.delete(promise),
    )
    return promise
  }

  private async drainInflightTools(timeoutMs: number): Promise<InflightToolDrainResult> {
    const snapshot = [...this.inflightToolHandlers.keys()]
    ...
    const pending = [...this.inflightToolHandlers.values()]
    return {
      settled: pending.length === 0,
      pendingCount: pending.length,
      nonIdempotentPending: pending.some((entry) => entry.nonIdempotent),
      pendingToolNames: [...new Set(pending.map((entry) => entry.name))],
    }
  }

  private recordResearchActivity(state: AgentStateData, input: {
    tool: string
    ...
    // Hidden activity is advisory telemetry. Start its durable write outside
    // the tool-result critical path, but register the caught promise with the
    // same execution drain used by the task's pre-terminal/cancellation fence.
    // This keeps tool turns fast without allowing a write to outlive terminal
    // ownership or turning a telemetry outage into a task failure.
    const persistence = Promise.resolve()
      .then(() => this.researchActivityAppender(entry))
      .catch((error) => {
    ...
    this.trackInflightOperation(persistence, 'research_activity_persistence', {})
  }
  ...
  private async persistGeneratedTaskFile(toolName: string, args: Record<string, unknown>, result: unknown): Promise<void> {
    ...
    if (!persisted) {
      throw new Error(`Generated file "${path}" could not be copied to durable task storage.`)
    }
  }

  private async ensureDurableBrowserScreenshot(result: unknown): Promise<void> {
    if (!result || typeof result !== 'object' || Array.isArray(result)) return
    const browserResult = result as Record<string, unknown>
    const screenshotPath = typeof browserResult.screenshotPath === 'string'
      ? browserResult.screenshotPath
      : ''
    if (!screenshotPath || !this.userId || !this.conversationId) {
      if (typeof browserResult.screenshotUrl === 'string' && browserResult.screenshotUrl.startsWith('/api/sandbox/')) {
        delete browserResult.screenshotUrl
      }
      return
    }
    const persisted = await persistSandboxTaskFile({
    ...
    if (persisted) {
      browserResult.screenshotUrl = this.durableImageUrl(screenshotPath)
    } else if (typeof browserResult.screenshotUrl === 'string' && browserResult.screenshotUrl.startsWith('/api/sandbox/')) {
      delete browserResult.screenshotUrl
    }
  }
  ...
  private async recordCachedToolProgress(
    ...
    // A cache hit replays evidence already counted when it was first gathered;
    // it must not inflate research breadth/depth or conceal a duplicate-source
    // loop from the paid-turn no-progress boundary.
    if (RESEARCH_TOOLS.has(tc.name)) {
      state.suppressedResearchToolName = tc.name
      state.stepLoopDetections += 1
      state.lastLoopSignal = tc.name === 'web_search'
        ? { type: 'search_duplicate', tool: tc.name }
        : { type: 'cross_tool_cycle', tool: tc.name }
    }
  ...
  private createAbortPromise(): { promise: Promise<never>; cleanup: () => void } | null {
    if (!this.signal) return null
    if (this.signal.aborted) {
      return { promise: Promise.reject(this.abortError()), cleanup: () => undefined }
    }
  ...
  private async maybeLaunchWebsiteAfterWrite(
    ...
    const previewStartArgs = { url: 'local-preview', path: filePath, previewBuild: true }
    this.emitter.toolStart(navigateId, 'browser_navigate', previewStartArgs)
    await this.emitter.flush?.()
    this.throwIfAborted()

    let launch: Awaited<ReturnType<typeof buildLocalWebsiteLaunch>>
    try {
      launch = await this.trackInflightOperation(
        Promise.resolve().then(() => buildLocalWebsiteLaunch(this.conversationId!, filePath)),
        'browser_navigate',
        previewStartArgs,
      )
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : String(err)
      this.emitter.toolResult(navigateId, 'browser_navigate', { error: errorMessage, url: '' } as never)
    ...
    try {
      browserResult = await this.trackInflightOperation(
        browserNavigate(this.conversationId, launch.url),
        'browser_navigate',
        { url: launch.url },
      )
    } catch (err) {
    ...
    browserResult = await this.trackInflightOperation(
      applyVisualQualityGate(browserResult, `local website preview for ${filePath}`),
      'browser_navigate',
      previewStartArgs,
    )
    ...
    this.emitter.toolResult(
      navigateId,
      'browser_navigate',
      sanitizeToolResultForEvent('browser_navigate', browserResultWithServer) as never,
    )
  ...
  private async maybeLaunchNextWebsiteAfterWrite(
    ...
    const previewStartArgs = { url: 'tsx-preview', path: `${status.appDir}/page.tsx`, previewBuild: true }
    this.emitter.toolStart(navigateId, 'browser_navigate', previewStartArgs)
    await this.emitter.flush?.()
    this.throwIfAborted()
    ...
    try {
      launch = await this.trackInflightOperation(
        Promise.resolve().then(() => buildTsxWebsitePreviewLaunch(this.conversationId!)),
        'browser_navigate',
        previewStartArgs,
      )
    } catch (err) {
    ...
    browserResult = await this.trackInflightOperation(
      applyVisualQualityGate(browserResult, `Next.js/TSX preview for ${status.appDir}/page.tsx`),
      'browser_navigate',
      previewStartArgs,
    )
    ...
    this.emitter.toolResult(
      navigateId,
      'browser_navigate',
      sanitizeToolResultForEvent('browser_navigate', browserResultWithPreview) as never,
    )
  ...
  private async blockBrowserActionPreflight(
    ...
    await this.ensureDurableBrowserScreenshot(result)
  ...
  private parallelSourceExtractionBatch(allCalls: ToolCallData[], state: AgentStateData): ToolCallData[] | null {
    if (allCalls.length < 2) return null
    if (!allCalls.every(tc => PARALLEL_SOURCE_EXTRACTION_TOOLS.has(tc.name))) return null
    const cadenceRoom = Math.max(1, visibleNarrationActionHeadroom(state))
    return allCalls.slice(0, Math.min(MAX_PARALLEL_SOURCE_EXTRACTIONS, cadenceRoom))
  }
  ...
  async executeAll(
    ...
    const parallelSourceBatch = coalescedSequence ? null : this.parallelSourceExtractionBatch(allCalls, state)
    ...
    if (parallelSourceBatch) {
      // Selecting a source batch is the decision boundary for the latest
      // search set. Consume it before execution so cached, blocked, malformed,
      // or failed reads cannot reopen the same result set on the next model
      // turn and cause an extraction loop.
      if (state.stepSearchQueries.size > 0) {
        state.stepLastSourceExtractionSearchCount = state.stepSearchQueries.size
      }
    ...
    for (const tc of executionBatch) {
      this.throwIfAborted()
      if (
        PARALLEL_SOURCE_EXTRACTION_TOOLS.has(tc.name) &&
        state.stepSearchQueries.size > 0
      ) {
        state.stepLastSourceExtractionSearchCount = state.stepSearchQueries.size
      }
  ...
  private async executeSingle(
    ...
    const closeVisibleProvisionalStart = (result: Record<string, unknown>, toolName = tc.name): void => {
      ...
      this.emitter.toolResult(tc.id, toolName, sanitizeToolResultForEvent(toolName, result) as never)
      state.visibleNarrationToolStartIds.delete(tc.id)
      // INTERNAL_RECOVERY means the provisional UI action never reached an
      // accepted tool result. Retract its optimistic cadence count just as the
      // live-directive supersession path does; cached/real failure results still
      // count as completed visible actions.
      if (isInternalRecoveryResult(result)) {
    ...
    if (!toolAllowedByExplicitTaskConstraint(explicitTaskToolConstraint, tc.name)) {
      const errorResult = {
        error: `INTERNAL_RECOVERY: ${tc.name} was blocked because it violates the user's explicit exclusive/forbidden tool instruction. Do not show this internal message to the user. Choose only a permitted named tool; if the required scope is unavailable under safety or permission checks, report that concrete blocker without substituting.`,
      }
      this.logger?.warn('Blocked a tool call that violated the explicit user method constraint', {
        tool: tc.name,
        constraint: explicitTaskToolConstraint,
    ...
    const argumentsForParsing = repairedArguments ?? tc.arguments
    try {
      args = JSON.parse(argumentsForParsing)
      if (repairedArguments) {
        tc.arguments = repairedArguments
        console.warn('[ToolPipeline] Repaired streamed read-only source arguments missing only the final object brace', {
          tool: tc.name,
      ...
      if ((tc.name === 'create_file' || tc.name === 'append_file') && this.conversationId) {
        ...
        if (partial && partial.content.trim().length >= minChars) {
          ...
          const priorPartialRecovery =
            tc.name === 'append_file' &&
            state.partialFileWriteRecoveryPending?.path === partial.path
              ? state.partialFileWriteRecoveryPending
              : null
          const recovery = {
            path: partial.path,
            toolName: tc.name,
            // A clipped append continues the recovered file already on disk.
            // Preserve the cumulative recovery boundary so the next model turn
            // does not mistake the newest chunk for the whole saved file and
            // repeat earlier sections.
            chars: (priorPartialRecovery?.chars || 0) + partial.content.length,
            lines:
              (priorPartialRecovery?.lines || 0) +
              recoveredLines -
              (priorPartialRecovery && priorPartialRecovery.lines > 0 && recoveredLines > 0 ? 1 : 0),
          }
          const appendRecoveryCount =
            tc.name === 'append_file'
              ? partialAppendRecoveryCountForPath(state, partial.path) + 1
              : 0
          ...
          // A recovery write is still a side effect. Do not touch the sandbox
          // until its tool_start checkpoint is durable for crash ambiguity
          // detection by the next worker attempt.
          await this.emitter.flush?.()
          this.throwIfAborted()

          const recoveryWrite = tc.name === 'create_file'
            ? createFileInSandbox(this.conversationId, partial.path, partial.content)
            : appendFileInSandbox(this.conversationId, partial.path, partial.content)
          const result = await this.trackInflightOperation(recoveryWrite, tc.name, recoveredArgs)

          if (result.size !== undefined) {
            try {
              await this.persistGeneratedTaskFile(tc.name, recoveredArgs, result)
            } catch (error) {
              this.emitter.toolResult(tc.id, tc.name, {
                error: 'The recovered file write could not be saved durably.',
                path: partial.path,
              } as never)
              throw error
            }
            ...
            trackSuccessfulToolExecution(state, tc.name)
            ...
            return {
              ...
              result: {
                ...
                partialWriteRecoveryLimitReached:
                  tc.name === 'append_file' &&
          ...
          this.emitter.toolResult(tc.id, tc.name, result as never)
          return { tc, result, isError: true, acceptedForExecution: true, durationMs: Date.now() - startTime }
        }
      ...
      const errorResult = {
        error: `INTERNAL_RECOVERY: malformed tool arguments for ${tc.name}. Retry this action with one strict JSON object tool call.`,
        rawPreview: argPreview,
      }
      closeVisibleProvisionalStart(errorResult)
      return {
        tc,
        result: errorResult,
        isError: true,
    ...
    const displayRepairs = repairRuntimeDisplayContractArgs(args, state)
    ...
    const narrationCadenceReason = isRequiredSavedDeliverableWrite(tc.name, args, state)
      ? null
    ...
    const nonIdempotentExecution = isNonIdempotentToolCall(tc.name, args)
    countVisibleToolActionForNarration(tc.id, tc.name, startArgs, state)
    if (!alreadyVisibleFromStream || nonIdempotentExecution) {
      // A provisional streamed start may not yet contain the method/action
      // that makes this call mutating. Re-persist the same ID with final,
      // sanitized args before execution; recovery treats this as an upsert.
    ...
    const toolContext: ToolContext = {
      ...
      onTerminalOutput: (streamType, data) => {
        this.emitter.terminalOutput(tc.id, streamType, redactTerminalOutputSecrets(data))
      },
    }
    ...
    // occasional repeated search request. Cached repeats still update tool and
    // loop state, but do not count as newly gathered research evidence.
    if (tc.name === 'web_search') {
      const query = ((args.query as string) || '').toLowerCase().trim()
      if (query) {
    ...
    const executeFn = async () => {
      ...
      if (nonIdempotentExecution) {
        await this.emitter.flush?.()
        this.throwIfAborted()
      }

      abortPromise = this.createAbortPromise()
      ...
      if (abortPromise) operations.push(abortPromise.promise)
    ...
    try {
      ...
      if (searchRecovery) {
        ...
        const sideEffectWarning = isTimeout && isNonIdempotentToolCall(tc.name, args)
    ...
    if (tc.name.startsWith('browser_') && result && typeof result === 'object') {
      ...
      if (isManagedLocalBrowserUrl(browserResult.url) && browserResult.screenshotBase64) {
        result = await this.trackInflightOperation(
          applyVisualQualityGate(browserResult, 'managed local website preview'),
          tc.name,
          args,
        )
      }
    }
    if (tc.name.startsWith('browser_')) await this.ensureDurableBrowserScreenshot(result)
    ...
    if (this.cache) {
      ...
      if (tc.name === 'create_file' || tc.name === 'edit_file' || tc.name === 'append_file' || tc.name === 'export_pdf' || tc.name === 'delete_file') {
        ...
        // Arbitrary code can mutate any workspace path, so path-targeted
        // invalidation cannot prove cached reads or directory listings fresh.
        this.cache.clear()
      } else if (tc.name === 'image_search') {
        const downloaded = result && typeof result === 'object' && !Array.isArray(result)
          ? (result as { downloaded?: unknown }).downloaded
          : undefined
        if (Array.isArray(downloaded)) {
          for (const filePath of downloaded) {
            if (typeof filePath === 'string' && filePath) this.cache.invalidateForFile(filePath)
          }
        }
        this.cache.invalidateForTool('list_files')
      }
    }
    ...
    return { tc, result, isError: !!isError, acceptedForExecution: true, durationMs }
  }
  ...
  buildToolResultMessages(results: ToolExecutionResult[], state?: AgentStateData): ChatMessageParam[] {
    ...
    for (const { tc, result } of results) {
      ...
      let serializableResult: unknown = result
      try {
      ...
      if (resultStr.length > MAX_TOOL_RESULT_CHARS) {
        if (tc.name === 'web_search' && Array.isArray(result)) {
          ...
          truncationNote = 'Page content compacted; current controls and completion signals retained when available.'
        } else if (tc.name === 'execute_command' || tc.name === 'run_code') {
          const compacted = compactCommandResultForModel(resultObj, resultStr, MAX_TOOL_RESULT_CHARS)
          try { compactedResult = JSON.parse(compacted) } catch { compactedResult = { result_preview: compacted } }
          truncationNote = 'Command output compacted; beginning, ending, exit code, and errors retained when available.'
        } else {
          truncationNote = 'Result compacted to the model context limit.'
        }
      }
      ...
      for (const imgPath of imageFiles) {
        if (!state.emittedImageArtifacts.has(imgPath)) {
          ...
          this.emitter.artifactCreated({
            ...
            content: '',
            type: 'image' as const,
            imageUrl: this.durableImageUrl(imgPath),
  ...
  private durableImageUrl(filePath: string): string {
    return `/api/files?conversationId=${encodeURIComponent(this.conversationId || '')}&file=${encodeURIComponent(filePath)}&inline=1`
  }
}
