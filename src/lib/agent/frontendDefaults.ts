 ...
 * and app builds should use one self-contained HTML file by default. Framework
 * structures remain available when the user explicitly asks for one.
 */
...
const FRAMEWORK_FRONTEND_REQUEST_PATTERN = /\b(?:next\.?js|react|tsx|jsx|vite|astro|svelte(?:kit)?|vue|nuxt|remix|angular|gatsby)\b/i
...
export function requestExplicitlyWantsFrameworkFrontend(text: string): boolean {
  return FRAMEWORK_FRONTEND_REQUEST_PATTERN.test(text)
}
...
export function shouldDefaultFrontendToNextTsx(text: string): boolean {
  ...
  return requestExplicitlyWantsFrameworkFrontend(text)
}

export function shouldDefaultFrontendToStandaloneHtml(text: string): boolean {
  return isFrontendArtifactRequest(text) && !shouldDefaultFrontendToNextTsx(text)
}
