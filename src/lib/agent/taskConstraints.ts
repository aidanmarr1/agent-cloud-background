...
export function requestedOutputFilePaths(text: string): string[] {
  ...
  const namedDeliverable = /\b(?:create|write|save|generate|produce|return|deliver|make)\s+[^.!?;\n]{0,80}?\b(?:report|document|file|output)\s+(?:to|as|at|named|called)\s+[`"']?((?:\.?\/?[\w-]+\/)*[\w-]+\.[a-zA-Z0-9]{1,10})(?=[`"'\s,;.!?]|$)/gi
  return [...new Set([
    ...text.matchAll(pattern),
    ...text.matchAll(namedDeliverable),
