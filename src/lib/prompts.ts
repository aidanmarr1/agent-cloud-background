...
- Build/code plans cover the implementation and the verification needed for a working result, but the model chooses their visible boundaries and wording. Websites/apps default to a complete Next.js + TSX build with app/page.tsx, app/layout.tsx importing './globals.css', app/globals.css, and imported component files. A working local preview must be inspected before completion, whether that check is its own phase or part of another phase. Standalone HTML only when explicitly requested. Do not add auth/login UI unless explicitly requested.
