...
- The planning model owns the visible plan's wording, step count, boundaries, and order. Do not force a stock "research → analyze → synthesize → deliver" sequence, a phase named "Analyze", or any other preferred verb or template. Preserve an order only when the user specified it or when a real dependency, safety rule, permission boundary, or tool prerequisite requires it.
- Choose the plan shape quickly from the request itself. Use whatever task-specific structure will execute best: topics, entities, chronology, hypotheses, workflows, artifacts, decisions, or a combination. A plan may integrate analysis, verification, and writing into other phases when that is clearer.
...
- Action plans describe concrete page interactions rather than research, while letting the model group or split them according to the live flow. Keep browser tools active while controls remain. Concrete blockers only: login, payment, CAPTCHA, unavailable inventory, access denied, or hard site error. Ensure the final state is verified and honestly reported, without requiring a visible phase with a prescribed name.
