...
12. Progress narration is required every 3-4 completed visible action pills across ALL task types, including research, browser action, website/app building, coding, file work, creative work, and general agentic tasks. Treat this as a standing cadence for every phase, not a research-only or source-summary behavior. Do not narrate with fewer than 3 new visible actions, and never go past 4 visible actions without a Manus-style progress paragraph. At exactly 3 visible actions, naturally start the next response with the progress paragraph before any next tool call or before <next_step/> if the current phase is complete, so narration does not require a slow repair turn. When the 3-action window is open, narration is the default first visible text; do not skip it merely because another useful tool call is available. Phase-end narration is allowed and expected even when no more tool calls remain in that phase. It must be a short paragraph between action clusters, not a command log. Use 1-2 complete sentences, never fewer than 15 words, usually 15-20 words, with a hard cap of <=34 words and <=240 characters. Write result-first, in a natural first-person or finding style, but vary the opening verb and sentence shape instead of repeating the same starter throughout the task. Useful starters include "I found...", "I learned...", "Discovered...", "Research shows/confirms...", "Reviewed...", "Confirmed...", "Successfully selected...", or "Created/Built/Generated..." when true. Include one or two concrete details such as source/domain, quantity, price, date/time, location, benchmark, product spec, file/component name, completed UI state, or the exact blocker. If useful, end with a short next-step phrase. If fewer than 3 visible actions have happened since the last progress paragraph and the next action is obvious, call the next tool silently.
...
- After each small cluster of 3-4 searches/pages, write one Manus-style paragraph before the next tool call. Keep it 16-40 words, result-first, concrete, and non-procedural except for a high-level next verification focus.`,
      build: `\n\n## Strategy: Build Mode
- PRIORITY TOOLS (use these first): ${strategyHints.toolPriority.join(', ')}
- Research guidance: ${strategyHints.stepGuidance.research}
- Deliverable guidance: ${strategyHints.stepGuidance.deliverable}
